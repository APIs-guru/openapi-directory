...
class ConnectionSchedule(BaseModel):
    ...
    """ # noqa: E501
    time_unit: StrictStr = Field(alias="timeUnit")
    units: StrictInt
    __properties: ClassVar[List[str]] = ["timeUnit", "units"]

    @field_validator('time_unit')
    def time_unit_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['minutes', 'hours', 'days', 'weeks', 'months']):
            raise ValueError("must be one of enum values ('minutes', 'hours', 'days', 'weeks', 'months')")
        return value
