...
class GitHubIntegration(BaseModel):
    ...
    owner: StrictStr = Field(description="The owner of the GitHub repository to synchronize with")
    token: Optional[SecretStr] = Field(default=None, description="A personal access token for accessing the target GitHub repository. You can generate tokens here: https://github.com/settings/tokens. Write-only property. Required to create and update the integration. The token must have the _public_repo_ scope if the target repository is public, or the _repo_ scope if it is private. ")
    __properties: ClassVar[List[str]] = ["enabled", "id", "name", "branch", "ignoredPaths", "managedPaths", "outputFile", "outputFolder", "providedPaths", "repository", "syncMethod", "target", "configType", "owner", "token"]
