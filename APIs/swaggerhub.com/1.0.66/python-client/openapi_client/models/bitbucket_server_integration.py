...
class BitbucketServerIntegration(BaseModel):
    """
    ...
    host: StrictStr = Field(description="The Bitbucket Server host to connect to. Make sure to include `http://` or `https://` at the beginning.")
    owner: StrictStr = Field(description="The owner of the repository to synchronize with")
    password: Optional[SecretStr] = Field(default=None, description="A [personal access token](https://support.smartbear.com/swaggerhub/docs/integrations/bitbucket-server-sync.html#token) for accessing the target Bitbucket Server repository. This token must have permissions for _Projects: Read_ and _Repositories: Write_. Write-only property. Required to create and update the integration.")
    username: StrictStr = Field(description="Bitbucket username to authenticate with")
    __properties: ClassVar[List[str]] = ["enabled", "id", "name", "branch", "ignoredPaths", "managedPaths", "outputFile", "outputFolder", "providedPaths", "repository", "syncMethod", "target", "configType", "host", "owner", "password", "username"]
    ...
    def config_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['BITBUCKET_SERVER']):
            raise ValueError("must be one of enum values ('BITBUCKET_SERVER')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BitbucketServerIntegration from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BitbucketServerIntegration from a dict"""
        if obj is None:
            return None
