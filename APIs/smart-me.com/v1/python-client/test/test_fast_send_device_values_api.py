# coding: utf-8

"""
    smart-me

    With the smart-me REST API you get Access to all your devices in the smart-me Cloud and you can add your own devices. So its an easy way to add the smart-me Cloud support to your Hardware or Software Product.

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.fast_send_device_values_api import FastSendDeviceValuesApi


class TestFastSendDeviceValuesApi(unittest.TestCase):
    """FastSendDeviceValuesApi unit test stubs"""

    def setUp(self) -> None:
        self.api = FastSendDeviceValuesApi()

    def tearDown(self) -> None:
        pass

    def test_fast_send_device_values_get(self) -> None:
        """Test case for fast_send_device_values_get

        Force a device to send the data every second (if supported). This for about 30s.              Don't use this call to force a device to send the data every second for a longer time.
        """
        pass


if __name__ == '__main__':
    unittest.main()
