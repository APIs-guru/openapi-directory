...
class Unsupported(BaseModel):
    ...
    message_type: StrictStr = Field(description="The type of message to send. Will be `unsupported` if the type of message received from user is not supported by the channel.")
    __properties: ClassVar[List[str]] = ["client_ref", "message_type"]

    @field_validator('message_type')
    def message_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['unsupported']):
            raise ValueError("must be one of enum values ('unsupported')")
        return value
