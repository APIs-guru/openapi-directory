...
class VoiceWebhooksInner(BaseModel):
    """
    VoiceWebhooksInner
    """ # noqa: E501
    endpoint: StrictStr = Field(description="`answer_url`: The URL where your webhook delivers the Nexmo Call Control Object that governs this call. As soon as your user answers a call Nexmo makes a request to answer_url. `event_url`: Nexmo sends event information asynchronously to this URL when status changes.")
    endpoint_type: StrictStr
    http_method: StrictStr = Field(description="The HTTP method used to send event information to the `event_url` or `answer_url`.")
    ...
    def endpoint_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['answer_url', 'event_url']):
            raise ValueError("must be one of enum values ('answer_url', 'event_url')")
        return value
