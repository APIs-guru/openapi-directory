...
class CreateCallRequestAnswerUrl(BaseModel):
    """
    ...
    answer_method: Optional[StrictStr] = Field(default='GET', description="The HTTP method used to send event information to answer_url.")
    answer_url: List[StrictStr] = Field(description="The webhook endpoint where you provide the [Nexmo Call Control Object](/voice/voice-api/ncco-reference) that governs this call. ")
    __properties: ClassVar[List[str]] = ["event_method", "event_url", "from", "length_timer", "machine_detection", "random_from_number", "ringing_timer", "to", "answer_method", "answer_url"]
    ...
    @field_validator('answer_method')
    def answer_method_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
