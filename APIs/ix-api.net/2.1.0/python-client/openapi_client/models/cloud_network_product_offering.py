...
class CloudNetworkProductOffering(BaseModel):
    """
    ...
    service_provider_pop: Optional[Annotated[str, Field(strict=True, max_length=16)]] = Field(description="The datacenter description of the partner NNI to the service provider. ")
    service_provider_region: Optional[StrictStr] = Field(description="The service provider offers the network service for a specific region. ")
    service_provider_workflow: StrictStr = Field(description="When the workflow is `provider_first` the subscriber creates a circuit with the cloud provider and provides a `cloud_key` for filtering the product-offerings.  If the workflow is `exchange_first` the IX will create the cloud circuit on the provider side. ")
    type: StrictStr
