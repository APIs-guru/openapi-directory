...
class ExchangeLanNetworkService(BaseModel):
    """
    ...
    metro_area_network: StrictStr = Field(description="Id of the `MetroAreaNetwork` where the exchange lan network service is directly provided.  Same as `service_metro_area_network` on the related `ProductOffering`. ")
    name: Annotated[str, Field(strict=True, max_length=40)] = Field(description="Exchange-dependent service name, will be shown on the invoice.")
    ...
    subnet_v4: StrictStr = Field(description="IPv4 subnet in [dot-decimal notation](https://en.wikipedia.org/wiki/Dot-decimal_notation) CIDR notation. ")
    subnet_v6: StrictStr = Field(description="IPv6 subnet in hexadecimal colon separated CIDR notation. ")
    type: StrictStr
