...
class RouteServerNetworkFeaturePartial(BaseModel):
    ...
    """ # noqa: E501
    address_families: Optional[List[StrictStr]] = Field(default=None, description="When creating a route server feature config, remember to specify which address family or families to use:  If the route server network feature only supports `af_inet`, then the `as_set_v4` in the network feature config is required.  If only `af_inet6` is supported, then the `as_set_v6` is required.  If both `af_inet` and `af_inet6` are supported, either `as_set_v4` or `as_set_v6` is required, but both can be provided in the network service config. ")
    asn: Optional[Annotated[int, Field(strict=True, ge=0)]] = None
    available_bgp_session_types: Optional[List[StrictStr]] = Field(default=None, description="The route server provides the following session modes. ")
    flags: Optional[List[IXPSpecificFeatureFlag]] = Field(default=None, description="A list of IXP specific feature flags. This can be used to see if e.g. RPKI hard filtering is available.")
    fqdn: Optional[Annotated[str, Field(strict=True, max_length=80)]] = Field(default=None, description="The FQDN of the route server. ")
    id: Optional[StrictStr] = None
    ip_v4: Optional[StrictStr] = Field(default=None, description="IPv4 address in [dot-decimal notation](https://en.wikipedia.org/wiki/Dot-decimal_notation) notation. ")
    ip_v6: Optional[StrictStr] = Field(default=None, description="IPv6 address in hexadecimal colon separated notation. ")
    looking_glass_url: Optional[StrictStr] = Field(default=None, description="The url of the looking glass. ")
    name: Optional[Annotated[str, Field(strict=True, max_length=80)]] = None
    ...
    session_mode: Optional[StrictStr] = Field(default=None, description="When creating a route server feature config, remember to specify the same session_mode as the route server. ")
    type: StrictStr
