...
EVENTDATA_ONE_OF_SCHEMAS = ["AccountEventPayload", "MessageEventPayload"]

class EventData(BaseModel):
    """
    EventData
    """
    # data type: AccountEventPayload
    oneof_schema_1_validator: Optional[AccountEventPayload] = None
    # data type: MessageEventPayload
    oneof_schema_2_validator: Optional[MessageEventPayload] = None
    actual_instance: Optional[Union[AccountEventPayload, MessageEventPayload]] = None
    one_of_schemas: Set[str] = { "AccountEventPayload", "MessageEventPayload" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        error_messages = []
        match = 0
        # validate data type: AccountEventPayload
        if not isinstance(v, AccountEventPayload):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AccountEventPayload`")
        else:
            match += 1
        # validate data type: MessageEventPayload
        if not isinstance(v, MessageEventPayload):
            error_messages.append(f"Error! Input type `{type(v)}` is not `MessageEventPayload`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in EventData with oneOf schemas: AccountEventPayload, MessageEventPayload. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in EventData with oneOf schemas: AccountEventPayload, MessageEventPayload. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into AccountEventPayload
        try:
            instance.actual_instance = AccountEventPayload.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into MessageEventPayload
        try:
            instance.actual_instance = MessageEventPayload.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into EventData with oneOf schemas: AccountEventPayload, MessageEventPayload. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into EventData with oneOf schemas: AccountEventPayload, MessageEventPayload. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AccountEventPayload, MessageEventPayload]]:
