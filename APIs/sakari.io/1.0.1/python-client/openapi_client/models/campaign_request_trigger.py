...
class CampaignRequestTrigger(BaseModel):
    """
    CampaignRequestTrigger
    """ # noqa: E501
    code: Optional[StrictStr] = Field(default=None, description="Campaign type specifies how it sources contacts and what event triggers its execution  Sort order   * `M` - Manual   * `S` - Scheduled   * `FU` - File Upload ")
    ...
    def code_validate_enum(cls, value):
        ...
        if value not in set(['M', 'S', 'FU']):
            raise ValueError("must be one of enum values ('M', 'S', 'FU')")
        return value
