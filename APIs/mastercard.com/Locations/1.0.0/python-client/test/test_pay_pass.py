# coding: utf-8

"""
    Locations API

    The Locations API provides access to MasterCard's ATM and Merchant location database

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.pay_pass import PayPass

class TestPayPass(unittest.TestCase):
    """PayPass unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PayPass:
        """Test PayPass
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PayPass`
        """
        model = PayPass()
        if include_optional:
            return PayPass(
                concession = True,
                drive_thru = True,
                fuel_pump = True,
                pharmacy = True,
                register = True,
                ticketing = True,
                toll_booth = True,
                vending_machine = True
            )
        else:
            return PayPass(
        )
        """

    def testPayPass(self):
        """Test PayPass"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
