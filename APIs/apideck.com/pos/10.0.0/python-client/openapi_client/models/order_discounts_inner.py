...
class OrderDiscountsInner(BaseModel):
    """
    OrderDiscountsInner
    """ # noqa: E501
    ...
    product_id: Optional[StrictStr] = Field(default=None, description="A unique identifier for an object.")
    scope: Optional[StrictStr] = None
    type: StrictStr
    __properties: ClassVar[List[str]] = ["amount", "currency", "id", "name", "product_id", "scope", "type"]
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['percentage', 'flat_fee']):
            raise ValueError("must be one of enum values ('percentage', 'flat_fee')")
        return value
