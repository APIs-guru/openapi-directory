# coding: utf-8

from datetime import date, datetime

from typing import List, Dict, Type

from openapi_server.models.base_model import Model
from openapi_server.models.assets_correlation_matrix_post_request_one_of1 import AssetsCorrelationMatrixPostRequestOneOf1
from openapi_server.models.assets_variance_post_request_one_of import AssetsVariancePostRequestOneOf
from openapi_server.models.assets_volatility_post_request_one_of import AssetsVolatilityPostRequestOneOf
from openapi_server.models.assets_volatility_post_request_one_of_assets_inner import AssetsVolatilityPostRequestOneOfAssetsInner
from openapi_server import util


class AssetsVolatilityPostRequest(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, assets: List[AssetsVolatilityPostRequestOneOfAssetsInner]=None, assets_covariance_matrix: List[List[float]]=None):
        """AssetsVolatilityPostRequest - a model defined in OpenAPI

        :param assets: The assets of this AssetsVolatilityPostRequest.
        :param assets_covariance_matrix: The assets_covariance_matrix of this AssetsVolatilityPostRequest.
        """
        self.openapi_types = {
            'assets': List[AssetsVolatilityPostRequestOneOfAssetsInner],
            'assets_covariance_matrix': List[List[float]]
        }

        self.attribute_map = {
            'assets': 'assets',
            'assets_covariance_matrix': 'assetsCovarianceMatrix'
        }

        self._assets = assets
        self._assets_covariance_matrix = assets_covariance_matrix

    @classmethod
    def from_dict(cls, dikt: dict) -> 'AssetsVolatilityPostRequest':
        """Returns the dict as a model

        :param dikt: A dict.
        :return: The _assets_volatility_post_request of this AssetsVolatilityPostRequest.
        """
        return util.deserialize_model(dikt, cls)

    @property
    def assets(self):
        """Gets the assets of this AssetsVolatilityPostRequest.


        :return: The assets of this AssetsVolatilityPostRequest.
        :rtype: List[AssetsVolatilityPostRequestOneOfAssetsInner]
        """
        return self._assets

    @assets.setter
    def assets(self, assets):
        """Sets the assets of this AssetsVolatilityPostRequest.


        :param assets: The assets of this AssetsVolatilityPostRequest.
        :type assets: List[AssetsVolatilityPostRequestOneOfAssetsInner]
        """
        if assets is None:
            raise ValueError("Invalid value for `assets`, must not be `None`")
        if assets is not None and len(assets) < 1:
            raise ValueError("Invalid value for `assets`, number of items must be greater than or equal to `1`")

        self._assets = assets

    @property
    def assets_covariance_matrix(self):
        """Gets the assets_covariance_matrix of this AssetsVolatilityPostRequest.

        assetsCovarianceMatrix[i][j] is the covariance between the asset i and the asset j

        :return: The assets_covariance_matrix of this AssetsVolatilityPostRequest.
        :rtype: List[List[float]]
        """
        return self._assets_covariance_matrix

    @assets_covariance_matrix.setter
    def assets_covariance_matrix(self, assets_covariance_matrix):
        """Sets the assets_covariance_matrix of this AssetsVolatilityPostRequest.

        assetsCovarianceMatrix[i][j] is the covariance between the asset i and the asset j

        :param assets_covariance_matrix: The assets_covariance_matrix of this AssetsVolatilityPostRequest.
        :type assets_covariance_matrix: List[List[float]]
        """
        if assets_covariance_matrix is None:
            raise ValueError("Invalid value for `assets_covariance_matrix`, must not be `None`")
        if assets_covariance_matrix is not None and len(assets_covariance_matrix) < 2:
            raise ValueError("Invalid value for `assets_covariance_matrix`, number of items must be greater than or equal to `2`")

        self._assets_covariance_matrix = assets_covariance_matrix
