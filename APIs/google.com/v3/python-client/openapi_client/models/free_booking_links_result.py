...
class FreeBookingLinksResult(BaseModel):
    ...
    **DEPRECATED:** Use `PropertyPerformanceResult` with `PropertyPerformanceReportService` instead. Represents a result from querying for free booking link stats for an account.
    """ # noqa: E501
    click_count: Optional[StrictStr] = Field(default=None, description="The total number of clicks that were recorded for this result.", alias="clickCount")
    ...
    partner_hotel_display_name: Optional[StrictStr] = Field(default=None, description="Partner's hotel name. Only present if `partnerHotelDisplayName` is specified in `aggregateBy` in the request.", alias="partnerHotelDisplayName")
    partner_hotel_id: Optional[StrictStr] = Field(default=None, description="Partner's hotel ID. Only present if `partnerHotelId` is specified in `aggregateBy` in the request.", alias="partnerHotelId")
    ...
    __properties: ClassVar[List[str]] = ["clickCount", "date", "deviceType", "partnerHotelDisplayName", "partnerHotelId", "userRegionCode"]
