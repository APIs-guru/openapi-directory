...
class Request(BaseModel):
    ...
    """ # noqa: E501
    extractor: Optional[StrictStr] = Field(default=None, description="[optional] Text extractor to be used when analyzing HTML document")
    id: Optional[StrictStr] = Field(default=None, description="Unique identifier of the document, it's optional")
    language: Optional[StrictStr] = Field(default=None, description="[optional] The language of the document, auto-detection will be used if omitted")
    options: Optional[Dict[str, Any]] = Field(default=None, description="[optional] Additional options for the internal modules (key-value pairs)")
    return_text_info: Optional[StrictBool] = Field(default=None, description="[optional] Indicates whether to return the source text within the response object", alias="returnTextInfo")
    text: Optional[StrictStr] = Field(default=None, description="The raw text to be analyzed, mutually exclusive with the 'url' parameter")
    url: Optional[StrictStr] = Field(default=None, description="URL of a document to be analysed, mutually exclusive with the 'text' parameter")
    __properties: ClassVar[List[str]] = ["extractor", "id", "language", "options", "returnTextInfo", "text", "url"]

    @field_validator('extractor')
    def extractor_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['default', 'article', 'keep-everything']):
            raise ValueError("must be one of enum values ('default', 'article', 'keep-everything')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "returnTextInfo": obj.get("returnTextInfo"),
            "text": obj.get("text"),
            "url": obj.get("url")
        })
        return _obj
