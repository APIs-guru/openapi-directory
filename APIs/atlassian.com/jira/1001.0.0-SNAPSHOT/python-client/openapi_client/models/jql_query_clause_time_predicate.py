...
class JqlQueryClauseTimePredicate(BaseModel):
    ...
    operator: StrictStr = Field(description="The operator between the field and the operand.")
    __properties: ClassVar[List[str]] = ["operand", "operator"]

    @field_validator('operator')
    def operator_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['before', 'after', 'from', 'to', 'true', 'during', 'by']):
            raise ValueError("must be one of enum values ('before', 'after', 'from', 'to', 'true', 'during', 'by')")
        return value
