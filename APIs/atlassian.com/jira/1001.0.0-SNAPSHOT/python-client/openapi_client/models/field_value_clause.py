...
class FieldValueClause(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["field", "operand", "operator"]

    @field_validator('operator')
    def operator_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['=', '!=', '>', '<', '>=', '<=', 'in', 'not in', '~', '~=', 'is', 'is not']):
            raise ValueError("must be one of enum values ('=', '!=', '>', '<', '>=', '<=', 'in', 'not in', '~', '~=', 'is', 'is not')")
        return value
