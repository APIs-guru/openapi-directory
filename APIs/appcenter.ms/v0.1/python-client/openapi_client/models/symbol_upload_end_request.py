...
class SymbolUploadEndRequest(BaseModel):
    ...
    """ # noqa: E501
    status: StrictStr = Field(description="The desired operation for the symbol upload")
    __properties: ClassVar[List[str]] = ["status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['committed', 'aborted']):
            raise ValueError("must be one of enum values ('committed', 'aborted')")
        return value
