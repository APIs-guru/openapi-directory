...
class ReleasesUpdateReleaseUploadStatusRequest(BaseModel):
    """
    ReleasesUpdateReleaseUploadStatusRequest
    """ # noqa: E501
    upload_status: StrictStr = Field(description="The new status of the release upload")
    __properties: ClassVar[List[str]] = ["upload_status"]

    @field_validator('upload_status')
    def upload_status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['uploadFinished', 'uploadCanceled']):
            raise ValueError("must be one of enum values ('uploadFinished', 'uploadCanceled')")
        return value
