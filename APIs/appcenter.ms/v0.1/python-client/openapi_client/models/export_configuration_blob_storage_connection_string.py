...
class ExportConfigurationBlobStorageConnectionString(BaseModel):
    ...
    """ # noqa: E501
    connection_string: StrictStr = Field(description="Connection string for blob storage account")
    blob_path_format_kind: Optional[StrictStr] = Field(default=None, description="The path to the blob when enum set to 'WithoutAppId' is 'year/month/day/hour/minute' and when set to 'WithAppId' is 'appId/year/month/day/hour/minute'")
    ...
    __properties: ClassVar[List[str]] = ["blob_path_format_kind", "backfill", "export_entities", "resource_group", "resource_name", "type"]

    @field_validator('blob_path_format_kind')
    def blob_path_format_kind_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['WithoutAppId', 'WithAppId']):
            raise ValueError("must be one of enum values ('WithoutAppId', 'WithAppId')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "blob_path_format_kind": obj.get("blob_path_format_kind"),
