...
class PatchReleaseUploadStatusResponse(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["id", "upload_status"]

    @field_validator('upload_status')
    def upload_status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['uploadStarted', 'uploadFinished', 'uploadCanceled', 'readyToBePublished', 'malwareDetected', 'error']):
            raise ValueError("must be one of enum values ('uploadStarted', 'uploadFinished', 'uploadCanceled', 'readyToBePublished', 'malwareDetected', 'error')")
        return value
