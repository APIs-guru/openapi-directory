...
class AppInvitationsUpdatePermissionsRequest(BaseModel):
    """
    AppInvitationsUpdatePermissionsRequest
    """ # noqa: E501
    permissions: List[StrictStr] = Field(description="The permissions the user has for the app in the invitation")
    ...
    def permissions_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['manager', 'developer', 'viewer']):
                raise ValueError("each list item must be one of ('manager', 'developer', 'viewer')")
        return value
