# coding: utf-8

"""
    App Center Client

    Microsoft Visual Studio App Center API

    The version of the OpenAPI document: v0.1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.alerting_github_bugtracker_settings import AlertingGithubBugtrackerSettings

class TestAlertingGithubBugtrackerSettings(unittest.TestCase):
    """AlertingGithubBugtrackerSettings unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AlertingGithubBugtrackerSettings:
        """Test AlertingGithubBugtrackerSettings
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AlertingGithubBugtrackerSettings`
        """
        model = AlertingGithubBugtrackerSettings()
        if include_optional:
            return AlertingGithubBugtrackerSettings(
                github_label = '',
                github_repo_id = 56,
                github_repo_name = '',
                callback_url = '',
                owner_name = '',
                type = 'github'
            )
        else:
            return AlertingGithubBugtrackerSettings(
                github_repo_id = 56,
                github_repo_name = '',
                owner_name = '',
                type = 'github',
        )
        """

    def testAlertingGithubBugtrackerSettings(self):
        """Test AlertingGithubBugtrackerSettings"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
