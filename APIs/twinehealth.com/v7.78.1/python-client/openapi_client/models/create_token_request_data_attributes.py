...
class CreateTokenRequestDataAttributes(BaseModel):
    """
    CreateTokenRequestDataAttributes
    """ # noqa: E501
    client_id: StrictStr = Field(description="Contact Fitbit Plus API Support to get a client id and secret.")
    client_secret: Optional[StrictStr] = Field(default=None, description="Contact Fitbit Plus API Support to get a client id and secret. Secret is required if grant_type is \"client_credentials\"")
    ...
    __properties: ClassVar[List[str]] = ["client_id", "client_secret", "grant_type", "refresh_token"]

    @field_validator('grant_type')
    def grant_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['refresh_token', 'client_credentials']):
            raise ValueError("must be one of enum values ('refresh_token', 'client_credentials')")
        return value
