# coding: utf-8

"""
    iQualify Management API

    The iQualify API offers management responses for building learning experiences using your iQualify instance data.  Once you’ve registered with iQualify, you can request an API access token by navigating to the API access section of the \"Account Settings\" area.  Find out how to [Request your API access token](https://www.iqualify.com/help/connecting-iqualify-to-other-systems/api/how-to-access-and-manage-your-api-token) on our Knowledge base.  All endpoints are only accessible via https and are located at api.iqualify.com. For instance: you can find your current offerings by accessing the following URL:      https://api.iqualify.com/v1/offerings/current  

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.assessment_groups_api import AssessmentGroupsApi


class TestAssessmentGroupsApi(unittest.TestCase):
    """AssessmentGroupsApi unit test stubs"""

    def setUp(self) -> None:
        self.api = AssessmentGroupsApi()

    def tearDown(self) -> None:
        pass

    def test_offerings_offering_id_groups_get(self) -> None:
        """Test case for offerings_offering_id_groups_get

        Find assessment groups
        """
        pass

    def test_offerings_offering_id_groups_group_id_learners_get(self) -> None:
        """Test case for offerings_offering_id_groups_group_id_learners_get

        Find learners in an assessment group
        """
        pass

    def test_offerings_offering_id_groups_group_id_learners_post(self) -> None:
        """Test case for offerings_offering_id_groups_group_id_learners_post

        Add a learner to an assessment group
        """
        pass

    def test_offerings_offering_id_groups_group_id_learners_user_email_delete(self) -> None:
        """Test case for offerings_offering_id_groups_group_id_learners_user_email_delete

        Remove a learner from an assessment group
        """
        pass

    def test_offerings_offering_id_groups_post(self) -> None:
        """Test case for offerings_offering_id_groups_post

        Add an assessment group
        """
        pass


if __name__ == '__main__':
    unittest.main()
