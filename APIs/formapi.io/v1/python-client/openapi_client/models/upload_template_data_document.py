...
class UploadTemplateDataDocument(BaseModel):
    """
    ...
    storage: StrictStr
    __properties: ClassVar[List[str]] = ["id", "metadata", "storage"]

    @field_validator('storage')
    def storage_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['cache']):
            raise ValueError("must be one of enum values ('cache')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "metadata": UploadTemplateDataDocumentMetadata.from_dict(obj["metadata"]) if obj.get("metadata") is not None else None,
