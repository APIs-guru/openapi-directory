# TransportLineNotifyPatch

The TransportLineNotify resource is a collection of transports that carry dispatched alerts to the external LINE Notify service.

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**data_segment_code** | **str** | User-provided string on which to segment and filter data. Max 50 characters. | [optional] 
**line_notify_access_token** | **str** | The access token for the LINE Notify service. Stored in encrypted format. | 
**line_notify_sticker_id** | **str** | The sticker ID value for the LINE Notify service. | [optional] 
**line_notify_sticker_package_id** | **str** | The sticker package ID value for the LINE Notify service. | [optional] 
**transport_name** | **str** | The name of the transport. | 

## Example

```python
from openapi_client.models.transport_line_notify_patch import TransportLineNotifyPatch

# TODO update the JSON string below
json = "{}"
# create an instance of TransportLineNotifyPatch from a JSON string
transport_line_notify_patch_instance = TransportLineNotifyPatch.from_json(json)
# print the JSON string representation of the object
print(TransportLineNotifyPatch.to_json())

# convert the object into a dict
transport_line_notify_patch_dict = transport_line_notify_patch_instance.to_dict()
# create an instance of TransportLineNotifyPatch from a dict
transport_line_notify_patch_from_dict = TransportLineNotifyPatch.from_dict(transport_line_notify_patch_dict)
```
[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


