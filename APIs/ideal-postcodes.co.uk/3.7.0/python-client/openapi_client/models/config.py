...
class Config(BaseModel):
    ...
    Config
    """ # noqa: E501
    created_at: StrictStr = Field(description="Timestamp for when the config was updated", alias="createdAt")
    name: Annotated[str, Field(strict=True)] = Field(description="A unique name to identify the configuration payload")
    payload: Annotated[str, Field(strict=True)] = Field(description="A serialised payload of up to `4096` characters")
    updated_at: StrictStr = Field(description="Timestamp for when the config was created", alias="updatedAt")
    __properties: ClassVar[List[str]] = ["createdAt", "name", "payload", "updatedAt"]
