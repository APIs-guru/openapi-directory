...
FINANCENUMBER_ONE_OF_SCHEMAS = ["float", "str"]

class FinanceNumber(BaseModel):
    """
    ...
    # data type: float
    oneof_schema_2_validator: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Finance Number")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = FinanceNumber.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in FinanceNumber with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in FinanceNumber with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into FinanceNumber with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into FinanceNumber with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return instance
