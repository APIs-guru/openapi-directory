...
USARESOLVEADDRESSRESPONSERESULT_ONE_OF_SCHEMAS = ["UsaGlobalAddress", "UspsAddress"]

class UsaResolveAddressResponseResult(BaseModel):
    """
    UsaResolveAddressResponseResult
    """
    # data type: UspsAddress
    oneof_schema_1_validator: Optional[UspsAddress] = None
    # data type: UsaGlobalAddress
    oneof_schema_2_validator: Optional[UsaGlobalAddress] = None
    actual_instance: Optional[Union[UsaGlobalAddress, UspsAddress]] = None
    one_of_schemas: Set[str] = { "UsaGlobalAddress", "UspsAddress" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = UsaResolveAddressResponseResult.model_construct()
        error_messages = []
        match = 0
        # validate data type: UspsAddress
        if not isinstance(v, UspsAddress):
            error_messages.append(f"Error! Input type `{type(v)}` is not `UspsAddress`")
        else:
            match += 1
        # validate data type: UsaGlobalAddress
        if not isinstance(v, UsaGlobalAddress):
            error_messages.append(f"Error! Input type `{type(v)}` is not `UsaGlobalAddress`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in UsaResolveAddressResponseResult with oneOf schemas: UsaGlobalAddress, UspsAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in UsaResolveAddressResponseResult with oneOf schemas: UsaGlobalAddress, UspsAddress. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into UspsAddress
        try:
            instance.actual_instance = UspsAddress.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into UsaGlobalAddress
        try:
            instance.actual_instance = UsaGlobalAddress.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into UsaResolveAddressResponseResult with oneOf schemas: UsaGlobalAddress, UspsAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into UsaResolveAddressResponseResult with oneOf schemas: UsaGlobalAddress, UspsAddress. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], UsaGlobalAddress, UspsAddress]]:
