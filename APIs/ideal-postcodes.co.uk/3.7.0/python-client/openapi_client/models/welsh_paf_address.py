...
class WelshPafAddress(BaseModel):
    """
    ...
    def country_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Wales']):
            raise ValueError("must be one of enum values ('Wales')")
        return value
    ...
    def dataset_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pafw']):
            raise ValueError("must be one of enum values ('pafw')")
        return value

    @field_validator('language')
    def language_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['cy']):
            raise ValueError("must be one of enum values ('cy')")
        return value
