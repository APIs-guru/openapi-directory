...
class Licensee(BaseModel):
    ...
    Licensee
    """ # noqa: E501
    address: StrictStr = Field(description="Licensee's first, second and third line address as well as post town concatenated by commas")
    daily: LicenseeAllOfDaily
    name: StrictStr = Field(description="Licensee individual or organisation name")
    postcode: StrictStr = Field(description="Licensee's postcode")
    whitelist: List[StrictStr] = Field(description="A list of allowed URLs. An empty list means that whitelisting is disabled")
    created_at: StrictStr = Field(description="Timestamp for when the licensee was created", alias="createdAt")
    id: StrictStr = Field(description="An immutable ID provided for every licensee. Primarily used for paginated list requests. ")
    key: StrictStr = Field(description="Uniquely identifies a licensee for a key.   Required to perform paid lookups for a specific licensee. Typically begins `sk_`. ")
    __properties: ClassVar[List[str]] = ["address", "daily", "name", "postcode", "whitelist", "createdAt", "id", "key"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "daily": LicenseeAllOfDaily.from_dict(obj["daily"]) if obj.get("daily") is not None else None,
            "name": obj.get("name"),
