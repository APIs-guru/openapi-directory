...
class ApiKeyCurrentPurchase(BaseModel):
    """
    ApiKeyCurrentPurchase
    """ # noqa: E501
    consumed: Annotated[int, Field(strict=True, ge=0)] = Field(description="Number of consumed lookups off this purchase.")
    expires: Optional[StrictStr] = Field(description="`string` or `null` The date when this purchase will expire in simplified  extended ISO format (ISO 8601). This is typically 365 days from the time  of first use. This field will be `null` if the purchase has not yet been  used.")
    purchased: Annotated[int, Field(strict=True, ge=0)] = Field(description="Number of procured lookups from this purchase.")
    __properties: ClassVar[List[str]] = ["consumed", "expires", "purchased"]
