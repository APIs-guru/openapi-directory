...
class EcafAddress(BaseModel):
    ...
    def dataset_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ecaf']):
            raise ValueError("must be one of enum values ('ecaf')")
        return value
