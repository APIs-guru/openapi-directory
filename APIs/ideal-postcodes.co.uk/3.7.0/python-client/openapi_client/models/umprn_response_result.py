...
UMPRNRESPONSERESULT_ONE_OF_SCHEMAS = ["MrAddress"]

class UMPRNResponseResult(BaseModel):
    """
    UMPRNResponseResult
    """
    # data type: MrAddress
    oneof_schema_1_validator: Optional[MrAddress] = None
    actual_instance: Optional[Union[MrAddress]] = None
    one_of_schemas: Set[str] = { "MrAddress" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = UMPRNResponseResult.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in UMPRNResponseResult with oneOf schemas: MrAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in UMPRNResponseResult with oneOf schemas: MrAddress. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into UMPRNResponseResult with oneOf schemas: MrAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into UMPRNResponseResult with oneOf schemas: MrAddress. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], MrAddress]]:
