...
class ApiKeyIndividualLimit(BaseModel):
    """
    ApiKeyIndividualLimit
    """ # noqa: E501
    limit: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(description="`number` or `null` Limit set on the number of lookups that can be  performed from a single IP address. `null` means the limit is currently  disabled.")
