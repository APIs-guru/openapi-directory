...
GBRCLEANSEMATCHMATCH_ONE_OF_SCHEMAS = ["MrAddress", "NybAddress", "PafAddress", "PafAliasAddress", "WelshPafAddress"]

class GbrCleanseMatchMatch(BaseModel):
    """
    ...
    oneof_schema_3_validator: Optional[NybAddress] = None
    # data type: PafAliasAddress
    oneof_schema_4_validator: Optional[PafAliasAddress] = None
    # data type: WelshPafAddress
    oneof_schema_5_validator: Optional[WelshPafAddress] = None
    actual_instance: Optional[Union[MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress]] = None
    one_of_schemas: Set[str] = { "MrAddress", "NybAddress", "PafAddress", "PafAliasAddress", "WelshPafAddress" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = GbrCleanseMatchMatch.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GbrCleanseMatchMatch with oneOf schemas: MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GbrCleanseMatchMatch with oneOf schemas: MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GbrCleanseMatchMatch with oneOf schemas: MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GbrCleanseMatchMatch with oneOf schemas: MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress]]:
