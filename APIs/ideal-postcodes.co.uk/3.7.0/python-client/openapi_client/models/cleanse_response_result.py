...
CLEANSERESPONSERESULT_ONE_OF_SCHEMAS = ["GbrCleanseMatch", "GbrCleanseNoMatch"]

class CleanseResponseResult(BaseModel):
    """
    CleanseResponseResult
    """
    # data type: GbrCleanseMatch
    oneof_schema_1_validator: Optional[GbrCleanseMatch] = None
    # data type: GbrCleanseNoMatch
    oneof_schema_2_validator: Optional[GbrCleanseNoMatch] = None
    actual_instance: Optional[Union[GbrCleanseMatch, GbrCleanseNoMatch]] = None
    one_of_schemas: Set[str] = { "GbrCleanseMatch", "GbrCleanseNoMatch" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = CleanseResponseResult.model_construct()
        error_messages = []
        match = 0
        # validate data type: GbrCleanseMatch
        if not isinstance(v, GbrCleanseMatch):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GbrCleanseMatch`")
        else:
            match += 1
        # validate data type: GbrCleanseNoMatch
        if not isinstance(v, GbrCleanseNoMatch):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GbrCleanseNoMatch`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CleanseResponseResult with oneOf schemas: GbrCleanseMatch, GbrCleanseNoMatch. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in CleanseResponseResult with oneOf schemas: GbrCleanseMatch, GbrCleanseNoMatch. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into GbrCleanseMatch
        try:
            instance.actual_instance = GbrCleanseMatch.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into GbrCleanseNoMatch
        try:
            instance.actual_instance = GbrCleanseNoMatch.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into CleanseResponseResult with oneOf schemas: GbrCleanseMatch, GbrCleanseNoMatch. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CleanseResponseResult with oneOf schemas: GbrCleanseMatch, GbrCleanseNoMatch. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], GbrCleanseMatch, GbrCleanseNoMatch]]:
