...
class UsaResolveAddressResponse(BaseModel):
    """
    UsaResolveAddressResponse
    """ # noqa: E501
    code: StrictInt
    message: StrictStr
    result: UsaResolveAddressResponseResult
    __properties: ClassVar[List[str]] = ["code", "message", "result"]

    @field_validator('code')
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set([2000]):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "result": UsaResolveAddressResponseResult.from_dict(obj["result"]) if obj.get("result") is not None else None
        })
        return _obj
