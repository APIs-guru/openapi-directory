...
class Email(BaseModel):
    ...
    Email
    """ # noqa: E501
    catchall: StrictBool = Field(description="Returns `true` if this domain accepts all emails regardless of username")
    deliverable: StrictBool = Field(description="Returns `true` if the email can be delivered")
    disposable: StrictBool = Field(description="Returns `true` if email comes from a disposable email service like temp-mail")
    free: StrictBool = Field(description="Returns `true` if the email originates from a free service like Outlook or Gmail.")
    result: StrictStr
    role: StrictBool = Field(description="Returns `true` if email address represents an organisational role like `admin`, `support`, `postmaster` etc")
    __properties: ClassVar[List[str]] = ["catchall", "deliverable", "disposable", "free", "result", "role"]

    @field_validator('result')
    def result_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['deliverable', 'not_deliverable']):
            raise ValueError("must be one of enum values ('deliverable', 'not_deliverable')")
        return value
