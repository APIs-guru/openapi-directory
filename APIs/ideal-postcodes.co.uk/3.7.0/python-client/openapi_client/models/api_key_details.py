...
class ApiKeyDetails(BaseModel):
    ...
    ApiKeyDetails
    """ # noqa: E501
    allowed_urls: List[StrictStr] = Field(description="A list of allowed URLs. An empty list means that allowed URLs are disabled.")
    automated_topups: ApiKeyAutomatedTopup
    current_purchases: List[ApiKeyCurrentPurchase] = Field(description="Current balance purchases attached to key.")
    daily_limit: ApiKeyDailyLimit
    datasets: Optional[ApiKeyDatasets] = None
    individual_limit: ApiKeyIndividualLimit
    lookups_remaining: Annotated[int, Field(strict=True, ge=0)]
    notifications: Optional[ApiKeyNotifications] = None
    __properties: ClassVar[List[str]] = ["allowed_urls", "automated_topups", "current_purchases", "daily_limit", "datasets", "individual_limit", "lookups_remaining", "notifications"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of automated_topups
        if self.automated_topups:
            _dict['automated_topups'] = self.automated_topups.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in current_purchases (list)
        _items = []
        if self.current_purchases:
            for _item_current_purchases in self.current_purchases:
                if _item_current_purchases:
                    _items.append(_item_current_purchases.to_dict())
            _dict['current_purchases'] = _items
        ...
        # override the default output from pydantic by calling `to_dict()` of individual_limit
        if self.individual_limit:
            _dict['individual_limit'] = self.individual_limit.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "automated_topups": ApiKeyAutomatedTopup.from_dict(obj["automated_topups"]) if obj.get("automated_topups") is not None else None,
            "current_purchases": [ApiKeyCurrentPurchase.from_dict(_item) for _item in obj["current_purchases"]] if obj.get("current_purchases") is not None else None,
            "daily_limit": ApiKeyDailyLimit.from_dict(obj["daily_limit"]) if obj.get("daily_limit") is not None else None,
            "datasets": ApiKeyDatasets.from_dict(obj["datasets"]) if obj.get("datasets") is not None else None,
            "individual_limit": ApiKeyIndividualLimit.from_dict(obj["individual_limit"]) if obj.get("individual_limit") is not None else None,
            "lookups_remaining": obj.get("lookups_remaining"),
            "notifications": ApiKeyNotifications.from_dict(obj["notifications"]) if obj.get("notifications") is not None else None
        })
        return _obj
