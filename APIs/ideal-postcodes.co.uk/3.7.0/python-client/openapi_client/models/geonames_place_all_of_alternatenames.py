...
GEONAMESPLACEALLOFALTERNATENAMES_ONE_OF_SCHEMAS = ["str"]

class GeonamesPlaceAllOfAlternatenames(BaseModel):
    """
    GeonamesPlaceAllOfAlternatenames
    """
    # data type: str
    oneof_schema_1_validator: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=40)]] = Field(default=None, description="Alternate name for place")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = GeonamesPlaceAllOfAlternatenames.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GeonamesPlaceAllOfAlternatenames with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GeonamesPlaceAllOfAlternatenames with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GeonamesPlaceAllOfAlternatenames with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GeonamesPlaceAllOfAlternatenames with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance
