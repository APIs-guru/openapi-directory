...
PLACENATIVE_ONE_OF_SCHEMAS = ["GeonamesPlace"]

class PlaceNative(BaseModel):
    """
    Native representation of a place
    """
    # data type: GeonamesPlace
    oneof_schema_1_validator: Optional[GeonamesPlace] = None
    actual_instance: Optional[Union[GeonamesPlace]] = None
    one_of_schemas: Set[str] = { "GeonamesPlace" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PlaceNative.model_construct()
        error_messages = []
        match = 0
        # validate data type: GeonamesPlace
        if not isinstance(v, GeonamesPlace):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GeonamesPlace`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PlaceNative with oneOf schemas: GeonamesPlace. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PlaceNative with oneOf schemas: GeonamesPlace. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into GeonamesPlace
        try:
            instance.actual_instance = GeonamesPlace.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PlaceNative with oneOf schemas: GeonamesPlace. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PlaceNative with oneOf schemas: GeonamesPlace. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], GeonamesPlace]]:
