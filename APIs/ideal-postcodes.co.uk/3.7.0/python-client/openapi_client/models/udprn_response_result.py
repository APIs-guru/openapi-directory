...
UDPRNRESPONSERESULT_ONE_OF_SCHEMAS = ["NybAddress", "PafAddress"]

class UDPRNResponseResult(BaseModel):
    """
    UDPRNResponseResult
    """
    ...
    oneof_schema_2_validator: Optional[NybAddress] = None
    actual_instance: Optional[Union[NybAddress, PafAddress]] = None
    one_of_schemas: Set[str] = { "NybAddress", "PafAddress" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = UDPRNResponseResult.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in UDPRNResponseResult with oneOf schemas: NybAddress, PafAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in UDPRNResponseResult with oneOf schemas: NybAddress, PafAddress. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into UDPRNResponseResult with oneOf schemas: NybAddress, PafAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into UDPRNResponseResult with oneOf schemas: NybAddress, PafAddress. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], NybAddress, PafAddress]]:
