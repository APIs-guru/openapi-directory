...
class LicenseesResponseResult(BaseModel):
    """
    List of licensees
    """ # noqa: E501
    has_more: Optional[StrictBool] = Field(default=None, description="Returns true if there are more licensees listed after the maximum number of results as implied by `limit`", alias="hasMore")
    licensees: Optional[List[Licensee]] = None
    __properties: ClassVar[List[str]] = ["hasMore", "licensees"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in licensees (list)
        _items = []
        if self.licensees:
            for _item_licensees in self.licensees:
                if _item_licensees:
                    _items.append(_item_licensees.to_dict())
            _dict['licensees'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "licensees": [Licensee.from_dict(_item) for _item in obj["licensees"]] if obj.get("licensees") is not None else None
        })
        return _obj
