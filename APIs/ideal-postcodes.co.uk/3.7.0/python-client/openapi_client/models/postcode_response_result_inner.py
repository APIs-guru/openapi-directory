...
POSTCODERESPONSERESULTINNER_ONE_OF_SCHEMAS = ["AbAddress", "GbrGlobalAddress", "MrAddress", "NybAddress", "PafAddress", "PafAliasAddress", "WelshPafAddress"]

class PostcodeResponseResultInner(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PostcodeResponseResultInner.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PostcodeResponseResultInner with oneOf schemas: AbAddress, GbrGlobalAddress, MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PostcodeResponseResultInner with oneOf schemas: AbAddress, GbrGlobalAddress, MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PostcodeResponseResultInner with oneOf schemas: AbAddress, GbrGlobalAddress, MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PostcodeResponseResultInner with oneOf schemas: AbAddress, GbrGlobalAddress, MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        else:
            return instance
