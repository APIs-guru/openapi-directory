...
class GbrCleanseNoMatch(BaseModel):
    """
    GbrCleanseNoMatch
    """ # noqa: E501
    confidence: Union[StrictFloat, StrictInt]
    count: Union[StrictFloat, StrictInt]
    fit: Union[StrictFloat, StrictInt]
    locality_match: StrictStr
    match: Optional[Dict[str, Any]] = Field(description="Nearest matching address")
    organisation_match: StrictStr
    post_town_match: StrictStr
    postcode_match: StrictStr
    premise_match: StrictStr
    query: StrictStr = Field(description="Originally submitted query")
    thoroughfare_match: StrictStr
    ...
    def locality_match_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NO_MATCH']):
            raise ValueError("must be one of enum values ('NO_MATCH')")
        return value
