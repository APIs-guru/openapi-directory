...
class GbrCleanseMatch(BaseModel):
    """
    GbrCleanseMatch
    """ # noqa: E501
    confidence: Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]] = Field(description="A confidence score represented as number between 1 and 0. 1 indicates a full match. 0 indicates no complete matching elements. ")
    count: Union[Annotated[float, Field(le=10, strict=True, ge=0)], Annotated[int, Field(le=10, strict=True, ge=0)]] = Field(description="The number of addresses we matched to the input. We return the closest match by default. ")
    fit: Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]] = Field(description="A score represented as number between 1 and 0. Fit compares the address elements present in your query against the matching address elements. It does not incorporate elements you have not presented in the score. A partial address (e.g. 12 Pye Green Road) will have a fit of 1 even though it is missing post town and postcode. Its confidence score will be less than 1 however because it is missing some crucial elements. ")
    locality_match: StrictStr = Field(description="Match indicator for the locality")
    match: GbrCleanseMatchMatch
    organisation_match: StrictStr = Field(description="Match indicator for the organisation")
    post_town_match: StrictStr = Field(description="Match indicator for the post_town")
    postcode_match: StrictStr = Field(description="Match indicator for the postcode")
    premise_match: StrictStr = Field(description="Match indicator for the premise")
    query: StrictStr = Field(description="Originally submitted query")
    thoroughfare_match: StrictStr = Field(description="Match indicator for the street")
    __properties: ClassVar[List[str]] = ["confidence", "count", "fit", "locality_match", "match", "organisation_match", "post_town_match", "postcode_match", "premise_match", "query", "thoroughfare_match"]

    @field_validator('locality_match')
    def locality_match_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA']):
            raise ValueError("must be one of enum values ('FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA')")
        return value

    @field_validator('organisation_match')
    def organisation_match_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA']):
            raise ValueError("must be one of enum values ('FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA')")
        return value

    @field_validator('post_town_match')
    def post_town_match_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA']):
            raise ValueError("must be one of enum values ('FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA')")
        return value

    @field_validator('postcode_match')
    def postcode_match_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA']):
            raise ValueError("must be one of enum values ('FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA')")
        return value

    @field_validator('premise_match')
    def premise_match_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA']):
            raise ValueError("must be one of enum values ('FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA')")
        return value

    @field_validator('thoroughfare_match')
    def thoroughfare_match_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA']):
            raise ValueError("must be one of enum values ('FULL', 'PARTIAL', 'INCORRECT', 'MISSING', 'NA')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "count": obj.get("count"),
            "fit": obj.get("fit"),
            "locality_match": obj.get("locality_match"),
            "match": GbrCleanseMatchMatch.from_dict(obj["match"]) if obj.get("match") is not None else None,
            "organisation_match": obj.get("organisation_match"),
            "post_town_match": obj.get("post_town_match"),
            "postcode_match": obj.get("postcode_match"),
            "premise_match": obj.get("premise_match"),
            "query": obj.get("query"),
            "thoroughfare_match": obj.get("thoroughfare_match")
        })
        return _obj
