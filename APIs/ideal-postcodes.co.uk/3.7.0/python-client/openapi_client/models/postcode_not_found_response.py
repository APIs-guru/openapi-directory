...
class PostcodeNotFoundResponse(BaseModel):
    """
    PostcodeNotFoundResponse
    """ # noqa: E501
    code: StrictInt
    message: StrictStr
    suggestions: List[StrictStr] = Field(description="A list of alternate nearest matching postcodes you can try")
    __properties: ClassVar[List[str]] = ["code", "message", "suggestions"]

    @field_validator('code')
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set([4040]):
            raise ValueError("must be one of enum values (4040)")
        return value

    @field_validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Postcode not found']):
            raise ValueError("must be one of enum values ('Postcode not found')")
        return value
