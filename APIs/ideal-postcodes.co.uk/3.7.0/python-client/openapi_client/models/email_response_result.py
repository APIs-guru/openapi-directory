...
EMAILRESPONSERESULT_ONE_OF_SCHEMAS = ["Email", "UnknownEmail"]
...
class EmailResponseResult(BaseModel):
    ...
    """
    # data type: Email
    oneof_schema_1_validator: Optional[Email] = None
    # data type: UnknownEmail
    oneof_schema_2_validator: Optional[UnknownEmail] = None
    actual_instance: Optional[Union[Email, UnknownEmail]] = None
    one_of_schemas: Set[str] = { "Email", "UnknownEmail" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = EmailResponseResult.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, Email):
            ...
            match += 1
        # validate data type: UnknownEmail
        if not isinstance(v, UnknownEmail):
            error_messages.append(f"Error! Input type `{type(v)}` is not `UnknownEmail`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in EmailResponseResult with oneOf schemas: Email, UnknownEmail. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in EmailResponseResult with oneOf schemas: Email, UnknownEmail. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into UnknownEmail
        try:
            instance.actual_instance = UnknownEmail.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into EmailResponseResult with oneOf schemas: Email, UnknownEmail. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into EmailResponseResult with oneOf schemas: Email, UnknownEmail. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Email, UnknownEmail]]:
