...
class KeyUsageResult(BaseModel):
    """
    KeyUsageResult
    """ # noqa: E501
    daily_count: List[KeyUsageResultDailyCountInner] = Field(description="An array of objects representing number of paid lookups made on specific days, ordered by date. Each object contains a `date` attribute, which represents the day and a `count` attribute, which represents the number of paid lookups made on that day. ", alias="dailyCount")
    end: StrictStr = Field(description="End date in ISO 8601 format.")
    start: StrictStr = Field(description="Start date in ISO 8601 format.")
    total: StrictInt = Field(description="Total of paid lookups performed in specified period.")
    __properties: ClassVar[List[str]] = ["dailyCount", "end", "start", "total"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "dailyCount": [KeyUsageResultDailyCountInner.from_dict(_item) for _item in obj["dailyCount"]] if obj.get("dailyCount") is not None else None,
            "end": obj.get("end"),
            "start": obj.get("start"),
            "total": obj.get("total")
        })
