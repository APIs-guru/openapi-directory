...
class LicenseeEditable(BaseModel):
    """
    Licensee object which can be defined by user
    """ # noqa: E501
    address: Optional[StrictStr] = Field(default=None, description="Licensee's first, second and third line address as well as post town concatenated by commas")
    daily: Optional[LicenseeEditableDaily] = None
    name: Optional[StrictStr] = Field(default=None, description="Licensee individual or organisation name")
    postcode: Optional[StrictStr] = Field(default=None, description="Licensee's postcode")
    whitelist: Optional[List[StrictStr]] = Field(default=None, description="A list of allowed URLs. An empty list means that whitelisting is disabled")
    __properties: ClassVar[List[str]] = ["address", "daily", "name", "postcode", "whitelist"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "daily": LicenseeEditableDaily.from_dict(obj["daily"]) if obj.get("daily") is not None else None,
            "name": obj.get("name"),
