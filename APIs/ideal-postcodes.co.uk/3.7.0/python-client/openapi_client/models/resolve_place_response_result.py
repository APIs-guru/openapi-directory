...
RESOLVEPLACERESPONSERESULT_ONE_OF_SCHEMAS = ["Place"]

class ResolvePlaceResponseResult(BaseModel):
    """
    ResolvePlaceResponseResult
    """
    # data type: Place
    oneof_schema_1_validator: Optional[Place] = None
    actual_instance: Optional[Union[Place]] = None
    one_of_schemas: Set[str] = { "Place" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ResolvePlaceResponseResult.model_construct()
        error_messages = []
        match = 0
        # validate data type: Place
        if not isinstance(v, Place):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Place`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ResolvePlaceResponseResult with oneOf schemas: Place. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ResolvePlaceResponseResult with oneOf schemas: Place. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into Place
        try:
            instance.actual_instance = Place.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ResolvePlaceResponseResult with oneOf schemas: Place. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ResolvePlaceResponseResult with oneOf schemas: Place. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Place]]:
