...
PLACERESPONSERESULTHITSINNER_ONE_OF_SCHEMAS = ["PlaceSuggestion"]

class PlaceResponseResultHitsInner(BaseModel):
    """
    PlaceResponseResultHitsInner
    """
    # data type: PlaceSuggestion
    oneof_schema_1_validator: Optional[PlaceSuggestion] = None
    actual_instance: Optional[Union[PlaceSuggestion]] = None
    one_of_schemas: Set[str] = { "PlaceSuggestion" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PlaceResponseResultHitsInner.model_construct()
        error_messages = []
        match = 0
        # validate data type: PlaceSuggestion
        if not isinstance(v, PlaceSuggestion):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PlaceSuggestion`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PlaceResponseResultHitsInner with oneOf schemas: PlaceSuggestion. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PlaceResponseResultHitsInner with oneOf schemas: PlaceSuggestion. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into PlaceSuggestion
        try:
            instance.actual_instance = PlaceSuggestion.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PlaceResponseResultHitsInner with oneOf schemas: PlaceSuggestion. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PlaceResponseResultHitsInner with oneOf schemas: PlaceSuggestion. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], PlaceSuggestion]]:
