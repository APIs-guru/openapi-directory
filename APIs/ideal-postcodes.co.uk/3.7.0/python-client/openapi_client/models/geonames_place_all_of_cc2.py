...
GEONAMESPLACEALLOFCC2_ONE_OF_SCHEMAS = ["str"]

class GeonamesPlaceAllOfCc2(BaseModel):
    """
    GeonamesPlaceAllOfCc2
    """
    # data type: str
    oneof_schema_1_validator: Optional[Annotated[str, Field(min_length=2, strict=True, max_length=2)]] = Field(default=None, description="Alternate name for place")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = GeonamesPlaceAllOfCc2.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GeonamesPlaceAllOfCc2 with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GeonamesPlaceAllOfCc2 with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GeonamesPlaceAllOfCc2 with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GeonamesPlaceAllOfCc2 with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance
