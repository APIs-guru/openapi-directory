...
PAFUMPRN_ONE_OF_SCHEMAS = ["float", "str"]

class PafUmprn(BaseModel):
    """
    ...
    """
    # data type: str
    oneof_schema_1_validator: Optional[StrictStr] = Field(default=None, description="If premise is not multiple occupancy, this will be an empty string")
    # data type: float
    oneof_schema_2_validator: Optional[Union[Annotated[float, Field(le=99999999, strict=True, ge=0)], Annotated[int, Field(le=99999999, strict=True, ge=0)]]] = Field(default=None, description="If premise is multiple occupancy, this will be an integer")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PafUmprn.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PafUmprn with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PafUmprn with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PafUmprn with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PafUmprn with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return instance
