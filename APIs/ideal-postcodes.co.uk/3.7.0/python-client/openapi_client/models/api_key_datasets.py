...
class ApiKeyDatasets(BaseModel):
    """
    Indicates which datasets are available and added by default to the address responses
    """ # noqa: E501
    ecad: StrictBool = Field(description="IE Address File. Eircode Address Database")
    ecaf: StrictBool = Field(description="IE Base Address File. Eircode Address File")
    herewe: Optional[StrictBool] = Field(default=None, description="Western Europe Address File")
    mr: StrictBool = Field(description="UK Multiple Residence Dataset")
    nyb: StrictBool = Field(description="UK Not Yet Built Dataset")
    paf: StrictBool = Field(description="UK Main Address File (Postcode Address File)")
    pafa: StrictBool = Field(description="UK Property Alias dataset")
    pafw: StrictBool = Field(description="UK Welsh Language Dataset")
    usps: StrictBool = Field(description="US Address Dataset")
    __properties: ClassVar[List[str]] = ["ecad", "ecaf", "herewe", "mr", "nyb", "paf", "pafa", "pafw", "usps"]
