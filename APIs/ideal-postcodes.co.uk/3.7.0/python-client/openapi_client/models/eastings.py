...
EASTINGS_ONE_OF_SCHEMAS = ["float", "str"]

class Eastings(BaseModel):
    """
    ...
    # data type: float
    oneof_schema_2_validator: Optional[Union[Annotated[float, Field(le=999999, strict=True, ge=0)], Annotated[int, Field(le=999999, strict=True, ge=0)]]] = Field(default=None, description="Metres offset east from origin")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = Eastings.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in Eastings with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in Eastings with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into Eastings with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Eastings with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return instance
