...
APIKEYCONTEXT_ONE_OF_SCHEMAS = ["NoContext", "str"]
...
class ApiKeyContext(BaseModel):
    ...
    """
    # data type: str
    oneof_schema_1_validator: Optional[StrictStr] = Field(default=None, description="Limits search results within a geographical boundary or country.")
    # data type: NoContext
    oneof_schema_2_validator: Optional[NoContext] = None
    actual_instance: Optional[Union[NoContext, str]] = None
    one_of_schemas: Set[str] = { "NoContext", "str" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ApiKeyContext.model_construct()
        error_messages = []
        match = 0
        ...
        # validate data type: NoContext
        if not isinstance(v, NoContext):
            error_messages.append(f"Error! Input type `{type(v)}` is not `NoContext`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ApiKeyContext with oneOf schemas: NoContext, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ApiKeyContext with oneOf schemas: NoContext, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into NoContext
        try:
            instance.actual_instance = NoContext.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ApiKeyContext with oneOf schemas: NoContext, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ApiKeyContext with oneOf schemas: NoContext, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], NoContext, str]]:
