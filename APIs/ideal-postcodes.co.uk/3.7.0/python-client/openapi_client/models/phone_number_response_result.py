...
PHONENUMBERRESPONSERESULT_ONE_OF_SCHEMAS = ["InvalidPhoneNumber", "PhoneNumber"]

class PhoneNumberResponseResult(BaseModel):
    """
    PhoneNumberResponseResult
    """
    # data type: PhoneNumber
    oneof_schema_1_validator: Optional[PhoneNumber] = None
    # data type: InvalidPhoneNumber
    oneof_schema_2_validator: Optional[InvalidPhoneNumber] = None
    actual_instance: Optional[Union[InvalidPhoneNumber, PhoneNumber]] = None
    one_of_schemas: Set[str] = { "InvalidPhoneNumber", "PhoneNumber" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PhoneNumberResponseResult.model_construct()
        error_messages = []
        match = 0
        # validate data type: PhoneNumber
        if not isinstance(v, PhoneNumber):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PhoneNumber`")
        else:
            match += 1
        # validate data type: InvalidPhoneNumber
        if not isinstance(v, InvalidPhoneNumber):
            error_messages.append(f"Error! Input type `{type(v)}` is not `InvalidPhoneNumber`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PhoneNumberResponseResult with oneOf schemas: InvalidPhoneNumber, PhoneNumber. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PhoneNumberResponseResult with oneOf schemas: InvalidPhoneNumber, PhoneNumber. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into PhoneNumber
        try:
            instance.actual_instance = PhoneNumber.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into InvalidPhoneNumber
        try:
            instance.actual_instance = InvalidPhoneNumber.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PhoneNumberResponseResult with oneOf schemas: InvalidPhoneNumber, PhoneNumber. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PhoneNumberResponseResult with oneOf schemas: InvalidPhoneNumber, PhoneNumber. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], InvalidPhoneNumber, PhoneNumber]]:
