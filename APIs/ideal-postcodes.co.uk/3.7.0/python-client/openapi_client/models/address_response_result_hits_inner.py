...
ADDRESSRESPONSERESULTHITSINNER_ONE_OF_SCHEMAS = ["AbAddress", "MrAddress", "NybAddress", "PafAddress", "PafAliasAddress", "WelshPafAddress"]

class AddressResponseResultHitsInner(BaseModel):
    """
    AddressResponseResultHitsInner
    """
    ...
    oneof_schema_4_validator: Optional[WelshPafAddress] = None
    # data type: PafAliasAddress
    oneof_schema_5_validator: Optional[PafAliasAddress] = None
    # data type: AbAddress
    oneof_schema_6_validator: Optional[AbAddress] = None
    actual_instance: Optional[Union[AbAddress, MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress]] = None
    one_of_schemas: Set[str] = { "AbAddress", "MrAddress", "NybAddress", "PafAddress", "PafAliasAddress", "WelshPafAddress" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AddressResponseResultHitsInner.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AddressResponseResultHitsInner with oneOf schemas: AbAddress, MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AddressResponseResultHitsInner with oneOf schemas: AbAddress, MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AddressResponseResultHitsInner with oneOf schemas: AbAddress, MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AddressResponseResultHitsInner with oneOf schemas: AbAddress, MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AbAddress, MrAddress, NybAddress, PafAddress, PafAliasAddress, WelshPafAddress]]:
