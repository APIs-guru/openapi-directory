...
GBRGLOBALADDRESSNATIVE_ONE_OF_SCHEMAS = ["EcadAddress", "EcafAddress", "UspsAddress"]

class GbrGlobalAddressNative(BaseModel):
    """
    The native representation of a non-UK address
    """
    # data type: EcadAddress
    oneof_schema_1_validator: Optional[EcadAddress] = None
    # data type: EcafAddress
    oneof_schema_2_validator: Optional[EcafAddress] = None
    # data type: UspsAddress
    oneof_schema_3_validator: Optional[UspsAddress] = None
    # data type: UspsAddress
    oneof_schema_4_validator: Optional[UspsAddress] = None
    actual_instance: Optional[Union[EcadAddress, EcafAddress, UspsAddress]] = None
    one_of_schemas: Set[str] = { "EcadAddress", "EcafAddress", "UspsAddress" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = GbrGlobalAddressNative.model_construct()
        error_messages = []
        match = 0
        # validate data type: EcadAddress
        if not isinstance(v, EcadAddress):
            error_messages.append(f"Error! Input type `{type(v)}` is not `EcadAddress`")
        else:
            match += 1
        # validate data type: EcafAddress
        if not isinstance(v, EcafAddress):
            error_messages.append(f"Error! Input type `{type(v)}` is not `EcafAddress`")
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GbrGlobalAddressNative with oneOf schemas: EcadAddress, EcafAddress, UspsAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GbrGlobalAddressNative with oneOf schemas: EcadAddress, EcafAddress, UspsAddress. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into EcadAddress
        try:
            instance.actual_instance = EcadAddress.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into EcafAddress
        try:
            instance.actual_instance = EcafAddress.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GbrGlobalAddressNative with oneOf schemas: EcadAddress, EcafAddress, UspsAddress. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GbrGlobalAddressNative with oneOf schemas: EcadAddress, EcafAddress, UspsAddress. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], EcadAddress, EcafAddress, UspsAddress]]:
