...
class MrAddress(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["administrative_county", "building_name", "building_number", "country", "country_iso", "country_iso_2", "county", "county_code", "dataset", "delivery_point_suffix", "department_name", "dependant_locality", "dependant_thoroughfare", "district", "double_dependant_locality", "eastings", "id", "language", "latitude", "line_1", "line_2", "line_3", "longitude", "northings", "organisation_name", "po_box", "post_town", "postal_county", "postcode", "postcode_inward", "postcode_outward", "postcode_type", "premise", "su_organisation_indicator", "sub_building_name", "thoroughfare", "traditional_county", "udprn", "umprn", "uprn", "ward"]

    @field_validator('country')
    def country_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['England', 'Scotland', 'Wales', 'Northern Ireland', 'Jersey', 'Guernsey', 'Isle of Man']):
            raise ValueError("must be one of enum values ('England', 'Scotland', 'Wales', 'Northern Ireland', 'Jersey', 'Guernsey', 'Isle of Man')")
        return value

    @field_validator('country_iso')
    def country_iso_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['GBR', 'IMN', 'JEY', 'GGY']):
            raise ValueError("must be one of enum values ('GBR', 'IMN', 'JEY', 'GGY')")
        return value

    @field_validator('country_iso_2')
    def country_iso_2_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['GB', 'IM', 'JE', 'GG']):
            raise ValueError("must be one of enum values ('GB', 'IM', 'JE', 'GG')")
        return value

    @field_validator('dataset')
    def dataset_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['mr']):
            raise ValueError("must be one of enum values ('mr')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MrAddress from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MrAddress from a dict"""
        if obj is None:
            return None
