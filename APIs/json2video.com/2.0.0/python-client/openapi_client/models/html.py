...
class Html(BaseModel):
    ...
    height: Optional[Annotated[int, Field(le=1080, strict=True, ge=-1)]] = Field(default=-1, description="Desired text element height. A value of <code>-1</code> means inherits scenes or movie height")
    html: StrictStr = Field(description="HTML snippet to render. Compatible with HTML5, CSS3 and Javascript")
    tailwindcss: Optional[StrictBool] = Field(default=False, description="Enables usage of TailwindCSS for the HTML snippet")
    type: StrictStr
    width: Optional[Annotated[int, Field(le=1920, strict=True, ge=-1)]] = Field(default=-1, description="Desired text element width. A value of <code>-1</code> means inherits scenes or movie width")
    __properties: ClassVar[List[str]] = ["cache", "comment", "duration", "extra-time", "fade-in", "fade-out", "start", "z-index", "chroma-key", "crop", "pan", "position", "rotate", "scale", "x", "y", "zoom", "height", "html", "tailwindcss", "type", "width"]
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['html']):
            raise ValueError("must be one of enum values ('html')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "html": obj.get("html"),
            "tailwindcss": obj.get("tailwindcss") if obj.get("tailwindcss") is not None else False,
            "type": obj.get("type"),
