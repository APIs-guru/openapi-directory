...
class SessionRead(BaseModel):
    ...
    SessionRead
    """ # noqa: E501
    acquiring_channel: Optional[StrictStr] = Field(default=None, description="The acquiring channel in which the session takes place. Ecommerce is default unless specified. Any other values should be defined in the agreement.")
    attachment: Optional[Attachment] = None
    authorization_token: Optional[StrictStr] = Field(default=None, description="Authorization token.")
    billing_address: Optional[Address] = None
    client_token: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=4096)]] = Field(default=None, description="Token to be passed to the JS client")
    ...
    design: Optional[StrictStr] = Field(default=None, description="Design package to use in the session. This can only by used if a custom design has been implemented for Klarna Payments and agreed upon in the agreement. It might have a financial impact. Delivery manager will provide the value for the parameter.")
    expires_at: Optional[datetime] = Field(default=None, description="Session expiration date")
    intent: Optional[StrictStr] = Field(default=None, description="Intent for the session. The field is designed to let partners inform Klarna of the purpose of the customer’s session.")
    ...
    order_amount: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="Total amount of the order including tax and any available discounts. The value should be in non-negative minor units. Eg: 25 Euros should be 2500.")
    order_lines: Optional[Annotated[List[OrderLine], Field(min_length=1, max_length=1000)]] = Field(default=None, description="The array containing list of line items that are part of this order. Maximum of 1000 line items could be processed in a single order.")
    ...
    purchase_country: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="The purchase country of the customer. The billing country always overrides purchase country if the values are different. Formatted according to ISO 3166 alpha-2 standard, e.g. GB, SE, DE, US, etc.")
    purchase_currency: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="The purchase currency of the order. Formatted according to ISO 4217 standard, e.g. USD, EUR, SEK, GBP, etc.")
    ...
    __properties: ClassVar[List[str]] = ["acquiring_channel", "attachment", "authorization_token", "billing_address", "client_token", "custom_payment_method_ids", "customer", "design", "expires_at", "intent", "locale", "merchant_data", "merchant_reference1", "merchant_reference2", "merchant_urls", "options", "order_amount", "order_lines", "order_tax_amount", "payment_method_categories", "purchase_country", "purchase_currency", "shipping_address", "status"]

    @field_validator('acquiring_channel')
    def acquiring_channel_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['ECOMMERCE', 'IN_STORE', 'TELESALES']):
            raise ValueError("must be one of enum values ('ECOMMERCE', 'IN_STORE', 'TELESALES')")
        return value
    ...
    def intent_validate_enum(cls, value):
        ...
        if value not in set(['buy', 'tokenize', 'buy_and_tokenize']):
            raise ValueError("must be one of enum values ('buy', 'tokenize', 'buy_and_tokenize')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "acquiring_channel": obj.get("acquiring_channel"),
            ...
            "customer": CustomerRead.from_dict(obj["customer"]) if obj.get("customer") is not None else None,
