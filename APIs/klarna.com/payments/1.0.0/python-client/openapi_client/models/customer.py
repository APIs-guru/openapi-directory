...
class Customer(BaseModel):
    ...
    last_four_ssn: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Last four digits of the customer's social security number. This value is available for US customers.")
    national_identification_number: Optional[StrictStr] = Field(default=None, description="The customer's national identification number. This value is available for EU customers utilizing national identification numbers.")
    ...
    type: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Type of customer in the session. If nothing is added, a B2C session will be the default. If it is a b2b-session, you should enter organization to trigger a B2B session.")
    vat_id: Optional[StrictStr] = Field(default=None, description="VAT ID. Only applicable for B2B customers.")
    __properties: ClassVar[List[str]] = ["date_of_birth", "gender", "last_four_ssn", "national_identification_number", "organization_entity_type", "organization_registration_id", "title", "type", "vat_id"]

    @field_validator('last_four_ssn')
    def last_four_ssn_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^([0-9]{4}|[0-9]{9})$", value):
            raise ValueError(r"must validate the regular expression /^([0-9]{4}|[0-9]{9})$/")
        return value
    ...
    def type_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(person|organization)$", value):
            raise ValueError(r"must validate the regular expression /^(person|organization)$/")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "last_four_ssn": obj.get("last_four_ssn"),
            "national_identification_number": obj.get("national_identification_number"),
