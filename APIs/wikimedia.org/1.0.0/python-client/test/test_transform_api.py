# coding: utf-8

"""
    Wikimedia

    This API provides cacheable and straightforward access to Wikimedia content and data, in machine-readable formats. ### Global Rules - Limit your clients to no more than 200 requests/s to this API.   Each API endpoint's documentation may detail more specific usage limits. - Set a unique `User-Agent` or `Api-User-Agent` header that   allows us to contact you quickly. Email addresses or URLs   of contact pages work well.  By using this API, you agree to Wikimedia's  [Terms of Use](https://wikimediafoundation.org/wiki/Terms_of_Use) and [Privacy Policy](https://wikimediafoundation.org/wiki/Privacy_policy). Unless otherwise specified in the endpoint documentation below, content accessed via this API is licensed under the [CC-BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/)  and [GFDL](https://www.gnu.org/copyleft/fdl.html) licenses, and you irrevocably agree to release modifications or additions made through this API under these licenses.  See https://www.mediawiki.org/wiki/REST_API for background and details. ### Endpoint documentation Please consult each endpoint's documentation for details on: - Licensing information for the specific type of content   and data served via the endpoint. - Stability markers to inform you about development status and   change policy, according to   [our API version policy](https://www.mediawiki.org/wiki/API_versioning). - Endpoint specific usage limits. 

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.transform_api import TransformApi


class TestTransformApi(unittest.TestCase):
    """TransformApi unit test stubs"""

    def setUp(self) -> None:
        self.api = TransformApi()

    def tearDown(self) -> None:
        pass

    def test_transform_html_from_from_lang_to_to_lang_post(self) -> None:
        """Test case for transform_html_from_from_lang_to_to_lang_post

        Machine-translate content
        """
        pass

    def test_transform_html_from_from_lang_to_to_lang_provider_post(self) -> None:
        """Test case for transform_html_from_from_lang_to_to_lang_provider_post

        Machine-translate content
        """
        pass

    def test_transform_list_languagepairs_get(self) -> None:
        """Test case for transform_list_languagepairs_get

        Lists the language pairs supported by the back-end
        """
        pass

    def test_transform_list_pair_from_to_get(self) -> None:
        """Test case for transform_list_pair_from_to_get

        Lists the tools available for a language pair
        """
        pass

    def test_transform_list_tool_tool_from_get(self) -> None:
        """Test case for transform_list_tool_tool_from_get

        Lists the tools and language pairs available for the given tool category
        """
        pass

    def test_transform_list_tool_tool_from_to_get(self) -> None:
        """Test case for transform_list_tool_tool_from_to_get

        Lists the tools and language pairs available for the given tool category
        """
        pass

    def test_transform_list_tool_tool_get(self) -> None:
        """Test case for transform_list_tool_tool_get

        Lists the tools and language pairs available for the given tool category
        """
        pass

    def test_transform_word_from_from_lang_to_to_lang_word_get(self) -> None:
        """Test case for transform_word_from_from_lang_to_to_lang_word_get

        Fetch the dictionary meaning of a word
        """
        pass

    def test_transform_word_from_from_lang_to_to_lang_word_provider_get(self) -> None:
        """Test case for transform_word_from_from_lang_to_to_lang_word_provider_get

        Fetch the dictionary meaning of a word
        """
        pass


if __name__ == '__main__':
    unittest.main()
