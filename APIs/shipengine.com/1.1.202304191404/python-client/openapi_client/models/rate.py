...
class Rate(BaseModel):
    ...
    rate_id: Annotated[str, Field(min_length=1, strict=True, max_length=25)] = Field(description="A string that uniquely identifies the rate")
    rate_type: RateType
    ...
    def rate_id_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^se(-[a-z0-9]+)+$", value):
            raise ValueError(r"must validate the regular expression /^se(-[a-z0-9]+)+$/")
        return value
