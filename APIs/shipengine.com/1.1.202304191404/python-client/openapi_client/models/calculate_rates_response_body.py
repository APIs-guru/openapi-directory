...
class CalculateRatesResponseBody(BaseModel):
    ...
    """ # noqa: E501
    advanced_options: AdvancedShipmentOptions = Field(description="Advanced shipment options.  These are entirely optional.")
    carrier_id: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=25)]] = Field(default=None, description="The carrier account that is billed for the shipping charges")
    confirmation: DeliveryConfirmation = Field(description="The type of delivery confirmation that is required for this shipment.")
    created_at: datetime = Field(description="The date and time that the shipment was created in ShipEngine.")
    customs: Optional[InternationalShipmentOptions] = Field(description="Customs information.  This is usually only needed for international shipments. ")
    ...
    insurance_provider: InsuranceProvider = Field(description="The insurance provider to use for any insured packages in the shipment. ")
    ...
    modified_at: datetime = Field(description="The date and time that the shipment was created or last modified.")
    ...
    packages: Annotated[List[Package], Field(min_length=1)] = Field(description="The packages in the shipment.  > **Note:** Some carriers only allow one package per shipment.  If you attempt to create a multi-package shipment for a carrier that doesn't allow it, an error will be returned. ")
    return_to: ShippingAddress = Field(description="The return address for this shipment.  Defaults to the `ship_from` address. ")
    ...
    ship_date: datetime = Field(description="The date that the shipment was (or will be) shippped.  ShipEngine will take the day of week into consideration. For example, if the carrier does not operate on Sundays, then a package that would have shipped on Sunday will ship on Monday instead. ")
    ship_from: Optional[ShippingAddress] = Field(default=None, description="The shipment's origin address. If you frequently ship from the same location, consider [creating a warehouse](https://www.shipengine.com/docs/reference/create-warehouse/).  Then you can simply specify the `warehouse_id` rather than the complete address each time. ")
    ship_to: Optional[ShippingAddress] = Field(default=None, description="The recipient's mailing address")
    ...
    shipment_status: ShipmentStatus = Field(description="The current status of the shipment")
    tags: Annotated[List[Tag], Field(min_length=0)] = Field(description="Arbitrary tags associated with this shipment.  Tags can be used to categorize shipments, and shipments can be queried by their tags. ")
    tax_identifiers: Optional[List[TaxIdentifier]] = None
    total_weight: Weight = Field(description="The combined weight of all packages in the shipment")
    ...
    __properties: ClassVar[List[str]] = ["advanced_options", "carrier_id", "confirmation", "created_at", "customs", "external_order_id", "external_shipment_id", "insurance_provider", "is_return", "items", "modified_at", "order_source_code", "origin_type", "packages", "return_to", "service_code", "ship_date", "ship_from", "ship_to", "shipment_id", "shipment_number", "shipment_status", "tags", "tax_identifiers", "total_weight", "warehouse_id", "rate_response"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CalculateRatesResponseBody from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of rate_response
        if self.rate_response:
            _dict['rate_response'] = self.rate_response.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CalculateRatesResponseBody from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "rate_response": RatesInformation.from_dict(obj["rate_response"]) if obj.get("rate_response") is not None else None
        })
        return _obj
