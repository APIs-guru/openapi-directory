...
class EICDetailsVulnerabilities(BaseModel):
    """
    Vulnerability information for residents of the property being supplied
    """ # noqa: E501
    dependency_type: StrictStr = Field(description="Is anyone in the property dependent on electricity for life support equipment?", alias="dependencyType")
    __properties: ClassVar[List[str]] = ["dependencyType"]

    @field_validator('dependency_type')
    def dependency_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Life Support', 'Sensitive Load']):
            raise ValueError("must be one of enum values ('Life Support', 'Sensitive Load')")
        return value
