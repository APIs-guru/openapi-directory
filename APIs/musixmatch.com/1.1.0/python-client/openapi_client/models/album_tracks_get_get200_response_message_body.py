...
class AlbumTracksGetGet200ResponseMessageBody(BaseModel):
    """
    AlbumTracksGetGet200ResponseMessageBody
    """ # noqa: E501
    track_list: Optional[List[Track]] = Field(default=None, description="A list of tracks")
    __properties: ClassVar[List[str]] = ["track_list"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in track_list (list)
        _items = []
        if self.track_list:
            for _item_track_list in self.track_list:
                if _item_track_list:
                    _items.append(_item_track_list.to_dict())
            _dict['track_list'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "track_list": [Track.from_dict(_item) for _item in obj["track_list"]] if obj.get("track_list") is not None else None
        })
        return _obj
