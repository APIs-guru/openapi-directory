# coding: utf-8

"""
    VictorOps

    This API allows you to interact with the VictorOps platform in various ways. Your account may be limited to a total number of API calls per month. Also, some of these API calls have rate limits.  NOTE: In this documentation when creating a sample curl request (clicking the TRY IT OUT! button), in some API viewing interfaces, the '@' in an email address may be encoded. Please note that the REST endpoints will not process the encoded version. Make sure that the encoded character '%40' is changed to its unencoded form before submitting the curl request. 

    The version of the OpenAPI document: 0.0.3
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.on_call_api import OnCallApi


class TestOnCallApi(unittest.TestCase):
    """OnCallApi unit test stubs"""

    def setUp(self) -> None:
        self.api = OnCallApi()

    def tearDown(self) -> None:
        pass

    def test_api_public_v1_oncall_current_get(self) -> None:
        """Test case for api_public_v1_oncall_current_get

        Get an organization's on-call users
        """
        pass

    def test_api_public_v1_policies_policy_oncall_user_patch(self) -> None:
        """Test case for api_public_v1_policies_policy_oncall_user_patch

        Create an on-call override (take on-call)
        """
        pass

    def test_api_public_v1_team_team_oncall_schedule_get(self) -> None:
        """Test case for api_public_v1_team_team_oncall_schedule_get

        Get a team's on-call schedule
        """
        pass

    def test_api_public_v1_team_team_oncall_user_patch(self) -> None:
        """Test case for api_public_v1_team_team_oncall_user_patch

        Create an on-call override (take on-call)
        """
        pass

    def test_api_public_v1_user_user_oncall_schedule_get(self) -> None:
        """Test case for api_public_v1_user_user_oncall_schedule_get

        Get a user's on-call schedule
        """
        pass

    def test_api_public_v2_team_team_oncall_schedule_get(self) -> None:
        """Test case for api_public_v2_team_team_oncall_schedule_get

        Get a team's on-call schedule
        """
        pass

    def test_api_public_v2_user_user_oncall_schedule_get(self) -> None:
        """Test case for api_public_v2_user_user_oncall_schedule_get

        Get a user's on-call schedule
        """
        pass


if __name__ == '__main__':
    unittest.main()
