# coding: utf-8

"""
    Interactive documentation for your Premium plan

      This interactive documentation is using your API key which is filled in automatically, you can find and change this in [your dashboard](https://www.meteosource.com/client).  Using the `GET` button, open your selected endpoint and read the introduction. You will find a detailed description of available parameters. You can complete the `Parameters` section and hit `Execute` to view a full API response. You can then inspect the JSON response, copy the `curl` command to run it on your machine, or obtain a URL of the request. In this way, you can easily build request command for the data you need. 

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.point_point_daily_stats_data import PointPointDailyStatsData

class TestPointPointDailyStatsData(unittest.TestCase):
    """PointPointDailyStatsData unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PointPointDailyStatsData:
        """Test PointPointDailyStatsData
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PointPointDailyStatsData`
        """
        model = PointPointDailyStatsData()
        if include_optional:
            return PointPointDailyStatsData(
                precipitation = openapi_client.models.point_point_daily_stats_precipitation_data.Point_PointDailyStatsPrecipitationData(
                    avg = 1.337, 
                    probability = 56, ),
                temperature = openapi_client.models.point_point_daily_stats_temperature_data.Point_PointDailyStatsTemperatureData(
                    avg = 1.337, 
                    avg_max = 1.337, 
                    avg_min = 1.337, 
                    record_max = 1.337, 
                    record_min = 1.337, ),
                wind = openapi_client.models.point_point_daily_stats_wind_data.Point_PointDailyStatsWindData(
                    avg_angle = 1.337, 
                    avg_dir = bytes(b'blah'), 
                    avg_speed = 1.337, 
                    max_gust = 1.337, 
                    max_speed = 1.337, )
            )
        else:
            return PointPointDailyStatsData(
                precipitation = openapi_client.models.point_point_daily_stats_precipitation_data.Point_PointDailyStatsPrecipitationData(
                    avg = 1.337, 
                    probability = 56, ),
                temperature = openapi_client.models.point_point_daily_stats_temperature_data.Point_PointDailyStatsTemperatureData(
                    avg = 1.337, 
                    avg_max = 1.337, 
                    avg_min = 1.337, 
                    record_max = 1.337, 
                    record_min = 1.337, ),
                wind = openapi_client.models.point_point_daily_stats_wind_data.Point_PointDailyStatsWindData(
                    avg_angle = 1.337, 
                    avg_dir = bytes(b'blah'), 
                    avg_speed = 1.337, 
                    max_gust = 1.337, 
                    max_speed = 1.337, ),
        )
        """

    def testPointPointDailyStatsData(self):
        """Test PointPointDailyStatsData"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
