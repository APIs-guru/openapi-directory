# coding: utf-8

"""
    Interactive documentation for your Premium plan

      This interactive documentation is using your API key which is filled in automatically, you can find and change this in [your dashboard](https://www.meteosource.com/client).  Using the `GET` button, open your selected endpoint and read the introduction. You will find a detailed description of available parameters. You can complete the `Parameters` section and hit `Execute` to view a full API response. You can then inspect the JSON response, copy the `curl` command to run it on your machine, or obtain a URL of the request. In this way, you can easily build request command for the data you need. 

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.point_point_hourly_data import PointPointHourlyData

class TestPointPointHourlyData(unittest.TestCase):
    """PointPointHourlyData unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PointPointHourlyData:
        """Test PointPointHourlyData
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PointPointHourlyData`
        """
        model = PointPointHourlyData()
        if include_optional:
            return PointPointHourlyData(
                cape = 1.337,
                cloud_cover = openapi_client.models.point_point_hourly_cloud_cover_data.Point_PointHourlyCloudCoverData(
                    high = 1.337, 
                    low = 1.337, 
                    middle = 1.337, 
                    total = 1.337, ),
                var_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                dew_point = 1.337,
                evaporation = 1.337,
                feels_like = 1.337,
                humidity = 56,
                icon = 56,
                irradiance = 1.337,
                lftx = 1.337,
                ozone = 1.337,
                precipitation = openapi_client.models.point_point_hourly_precipitation_data.Point_PointHourlyPrecipitationData(
                    convective = 1.337, 
                    rainspot = bytes(b'blah'), 
                    total = 1.337, 
                    type = bytes(b'blah'), ),
                pressure = 1.337,
                probability = openapi_client.models.point_point_hourly_prob_data.Point_PointHourlyProbData(
                    freeze = 1.337, 
                    precipitation = 56, 
                    storm = 1.337, ),
                snow_depth = 1.337,
                soil_temperature = 1.337,
                summary = '',
                sunshine_duration = 1.337,
                surface_temperature = 1.337,
                temperature = 1.337,
                uv_index = 1.337,
                visibility = 1.337,
                weather = '',
                wind = openapi_client.models.point_point_hourly_wind_data.Point_PointHourlyWindData(
                    angle = 1.337, 
                    dir = bytes(b'blah'), 
                    gusts = 1.337, 
                    speed = 1.337, ),
                wind_chill = 1.337
            )
        else:
            return PointPointHourlyData(
                cloud_cover = openapi_client.models.point_point_hourly_cloud_cover_data.Point_PointHourlyCloudCoverData(
                    high = 1.337, 
                    low = 1.337, 
                    middle = 1.337, 
                    total = 1.337, ),
                precipitation = openapi_client.models.point_point_hourly_precipitation_data.Point_PointHourlyPrecipitationData(
                    convective = 1.337, 
                    rainspot = bytes(b'blah'), 
                    total = 1.337, 
                    type = bytes(b'blah'), ),
                probability = openapi_client.models.point_point_hourly_prob_data.Point_PointHourlyProbData(
                    freeze = 1.337, 
                    precipitation = 56, 
                    storm = 1.337, ),
                wind = openapi_client.models.point_point_hourly_wind_data.Point_PointHourlyWindData(
                    angle = 1.337, 
                    dir = bytes(b'blah'), 
                    gusts = 1.337, 
                    speed = 1.337, ),
        )
        """

    def testPointPointHourlyData(self):
        """Test PointPointHourlyData"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
