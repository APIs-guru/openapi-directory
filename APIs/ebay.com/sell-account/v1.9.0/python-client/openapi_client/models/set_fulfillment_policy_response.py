...
class SetFulfillmentPolicyResponse(BaseModel):
    """
    Complex type that that gets populated with a response containing a fulfillment policy.
    """ # noqa: E501
    category_types: Optional[List[CategoryType]] = Field(default=None, description="This container indicates whether the fulfillment business policy applies to motor vehicle listings, or if it applies to non-motor vehicle listings.", alias="categoryTypes")
    ...
    fulfillment_policy_id: Optional[StrictStr] = Field(default=None, description="A unique eBay-assigned ID for a fulfillment business policy. This ID is generated when the policy is created.", alias="fulfillmentPolicyId")
    ...
    marketplace_id: Optional[StrictStr] = Field(default=None, description="The ID of the eBay marketplace to which this fulfillment business policy applies. For implementation help, refer to <a href='https://developer.ebay.com/api-docs/sell/account/types/ba:MarketplaceIdEnum'>eBay API documentation</a>", alias="marketplaceId")
    name: Optional[StrictStr] = Field(default=None, description="A seller-defined name for this fulfillment business policy. Names must be unique for policies assigned to the same marketplace. <br/><br/><b>Max length</b>: 64")
    ...
    __properties: ClassVar[List[str]] = ["categoryTypes", "description", "freightShipping", "fulfillmentPolicyId", "globalShipping", "handlingTime", "localPickup", "marketplaceId", "name", "pickupDropOff", "shipToLocations", "shippingOptions", "warnings"]
