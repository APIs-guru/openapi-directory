...
class ItemPriorityResponse(BaseModel):
    """
    ItemPriorityResponse
    """ # noqa: E501
    item_delta: Optional[List[ItemPriority]] = Field(default=None, description="The container for the array of items returned by the <b>getItemPriorityFeed</b> method. The data in the file is tab separated and the first row is the header, which labels the columns and indicates the order of the values on each line. The header labels match the fields that are described in the <a href=\"/api-docs/buy/feed/resources/item/methods/getItemPriorityFeed#h3-response-fields\">Response fields</a> section.", alias="itemDelta")
    __properties: ClassVar[List[str]] = ["itemDelta"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in item_delta (list)
        _items = []
        if self.item_delta:
            for _item_item_delta in self.item_delta:
                if _item_item_delta:
                    _items.append(_item_item_delta.to_dict())
            _dict['itemDelta'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "itemDelta": [ItemPriority.from_dict(_item) for _item in obj["itemDelta"]] if obj.get("itemDelta") is not None else None
        })
        return _obj
