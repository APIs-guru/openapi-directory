...
class ProgramInformationBatch(BaseModel):
    """
    An electronic program guide (EPG) batch operation to create or update metadata on one or more guide programs.
    """ # noqa: E501
    created_date: StrictStr = Field(description="The date and time the batch was created.", alias="createdDate")
    finished_date: Optional[StrictStr] = Field(default=None, description="The date and time the batch finished (either successful or failed).", alias="finishedDate")
    format: StrictStr = Field(description="The format of the metadata file defining the create or update actions to be performed on one or more EPG programs.")
    id: Annotated[int, Field(strict=True, ge=0)] = Field(description="The ID of the batch.")
    message: Optional[StrictStr] = Field(default=None, description="The human readable success or failure message.")
    name: Optional[StrictStr] = Field(default=None, description="The optional name of the batch for human reference.")
    program: Optional[ProgramInformationBatchProgram] = None
    status: StrictStr = Field(description="The current processing status.")
    uri: Optional[StrictStr] = Field(default=None, description="The URI to the metadata file defining the batch creates/updates.")
    __properties: ClassVar[List[str]] = ["createdDate", "finishedDate", "format", "id", "message", "name", "program", "status", "uri"]

    @field_validator('format')
    def format_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['radiodns']):
            raise ValueError("must be one of enum values ('radiodns')")
        return value

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['queued', 'processing', 'failed', 'successful']):
            raise ValueError("must be one of enum values ('queued', 'processing', 'failed', 'successful')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "name": obj.get("name"),
            "program": ProgramInformationBatchProgram.from_dict(obj["program"]) if obj.get("program") is not None else None,
