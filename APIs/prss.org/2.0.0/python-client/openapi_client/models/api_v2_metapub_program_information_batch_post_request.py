...
class ApiV2MetapubProgramInformationBatchPostRequest(BaseModel):
    """
    The batch operation definition.
    """ # noqa: E501
    format: StrictStr = Field(description="The format of the metadata file defining the create or update actions to be performed on one or more EPG programs. For more information on how RadioDNS EPG maps to ContentDepot <a href=\"/api/epg-cd-mapping.html\">click here </a>")
    name: Optional[StrictStr] = Field(default=None, description="An optional human readable name for the batch.")
    program: Optional[ApiV2MetapubProgramInformationBatchPostRequestProgram] = None
    uri: StrictStr = Field(description="The URI to the metadata file. Currently only the ```cddrive``` scheme is supported.")
    __properties: ClassVar[List[str]] = ["format", "name", "program", "uri"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "name": obj.get("name"),
            "program": ApiV2MetapubProgramInformationBatchPostRequestProgram.from_dict(obj["program"]) if obj.get("program") is not None else None,
            "uri": obj.get("uri")
        })
        return _obj
