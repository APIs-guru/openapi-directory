...
class GetOrganizationSensorReadingsHistory200ResponseInnerButton(BaseModel):
    """
    Reading for the 'button' metric. This will only be present if the 'metric' property equals 'button'.
    """ # noqa: E501
    press_type: Optional[StrictStr] = Field(default=None, description="Type of button press that occurred.", alias="pressType")
    __properties: ClassVar[List[str]] = ["pressType"]

    @field_validator('press_type')
    def press_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
