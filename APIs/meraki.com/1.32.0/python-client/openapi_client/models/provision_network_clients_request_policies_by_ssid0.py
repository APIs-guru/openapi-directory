...
class ProvisionNetworkClientsRequestPoliciesBySsid0(BaseModel):
    ...
    """ # noqa: E501
    device_policy: StrictStr = Field(description="The policy to apply to the specified client. Can be 'Allowed', 'Blocked', 'Normal' or 'Group policy'. Required.", alias="devicePolicy")
    ...
    def device_policy_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Allowed', 'Blocked', 'Group policy', 'Normal']):
            raise ValueError("must be one of enum values ('Allowed', 'Blocked', 'Group policy', 'Normal')")
        return value
