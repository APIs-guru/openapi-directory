...
class UpdateNetworkSwitchStackRoutingInterfaceDhcpRequest(BaseModel):
    """
    UpdateNetworkSwitchStackRoutingInterfaceDhcpRequest
    """ # noqa: E501
    boot_file_name: Optional[StrictStr] = Field(default=None, description="The PXE boot server file name for the DHCP server running on the switch stack interface", alias="bootFileName")
    boot_next_server: Optional[StrictStr] = Field(default=None, description="The PXE boot server IP for the DHCP server running on the switch stack interface", alias="bootNextServer")
    boot_options_enabled: Optional[StrictBool] = Field(default=None, description="Enable DHCP boot options to provide PXE boot options configs for the dhcp server running on the switch stack interface", alias="bootOptionsEnabled")
    dhcp_lease_time: Optional[StrictStr] = Field(default=None, description="The DHCP lease time config for the dhcp server running on switch stack interface ('30 minutes', '1 hour', '4 hours', '12 hours', '1 day' or '1 week')", alias="dhcpLeaseTime")
    dhcp_mode: Optional[StrictStr] = Field(default=None, description="The DHCP mode options for the switch stack interface ('dhcpDisabled', 'dhcpRelay' or 'dhcpServer')", alias="dhcpMode")
    dhcp_options: Optional[List[UpdateDeviceSwitchRoutingInterfaceDhcpRequestDhcpOptionsInner]] = Field(default=None, description="Array of DHCP options consisting of code, type and value for the DHCP server running on the switch stack interface", alias="dhcpOptions")
    dhcp_relay_server_ips: Optional[List[StrictStr]] = Field(default=None, description="The DHCP relay server IPs to which DHCP packets would get relayed for the switch stack interface", alias="dhcpRelayServerIps")
    ...
    dns_nameservers_option: Optional[StrictStr] = Field(default=None, description="The DHCP name server option for the dhcp server running on the switch stack interface ('googlePublicDns', 'openDns' or 'custom')", alias="dnsNameserversOption")
    fixed_ip_assignments: Optional[List[UpdateDeviceSwitchRoutingInterfaceDhcpRequestFixedIpAssignmentsInner]] = Field(default=None, description="Array of DHCP fixed IP assignments for the DHCP server running on the switch stack interface", alias="fixedIpAssignments")
    reserved_ip_ranges: Optional[List[UpdateDeviceSwitchRoutingInterfaceDhcpRequestReservedIpRangesInner]] = Field(default=None, description="Array of DHCP reserved IP assignments for the DHCP server running on the switch stack interface", alias="reservedIpRanges")
