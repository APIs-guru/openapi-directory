...
class UpdateNetworkSwitchStackRoutingInterfaceRequest(BaseModel):
    """
    UpdateNetworkSwitchStackRoutingInterfaceRequest
    """ # noqa: E501
    default_gateway: Optional[StrictStr] = Field(default=None, description="The next hop for any traffic that isn't going to a directly connected subnet or over a static route. This IP address must exist in a subnet with a routed interface.", alias="defaultGateway")
    interface_ip: Optional[StrictStr] = Field(default=None, description="The IP address this switch stack will use for layer 3 routing on this VLAN or subnet. This cannot be the same as the switch's management IP.", alias="interfaceIp")
    ipv6: Optional[UpdateNetworkSwitchStackRoutingInterfaceRequestIpv6] = None
    multicast_routing: Optional[StrictStr] = Field(default=None, description="Enable multicast support if, multicast routing between VLANs is required. Options are, 'disabled', 'enabled' or 'IGMP snooping querier'.", alias="multicastRouting")
    name: Optional[StrictStr] = Field(default=None, description="A friendly name or description for the interface or VLAN.")
    ospf_settings: Optional[UpdateNetworkSwitchStackRoutingInterfaceRequestOspfSettings] = Field(default=None, alias="ospfSettings")
    ...
    __properties: ClassVar[List[str]] = ["defaultGateway", "interfaceIp", "ipv6", "multicastRouting", "name", "ospfSettings", "subnet", "vlanId"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "ipv6": UpdateNetworkSwitchStackRoutingInterfaceRequestIpv6.from_dict(obj["ipv6"]) if obj.get("ipv6") is not None else None,
            "multicastRouting": obj.get("multicastRouting"),
            "name": obj.get("name"),
            "ospfSettings": UpdateNetworkSwitchStackRoutingInterfaceRequestOspfSettings.from_dict(obj["ospfSettings"]) if obj.get("ospfSettings") is not None else None,
