# coding: utf-8

"""
    Meraki Dashboard API

    The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 05 April, 2023 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

    The version of the OpenAPI document: 1.32.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.update_network_wireless_ssid_traffic_shaping_rules_request import UpdateNetworkWirelessSsidTrafficShapingRulesRequest

class TestUpdateNetworkWirelessSsidTrafficShapingRulesRequest(unittest.TestCase):
    """UpdateNetworkWirelessSsidTrafficShapingRulesRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> UpdateNetworkWirelessSsidTrafficShapingRulesRequest:
        """Test UpdateNetworkWirelessSsidTrafficShapingRulesRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `UpdateNetworkWirelessSsidTrafficShapingRulesRequest`
        """
        model = UpdateNetworkWirelessSsidTrafficShapingRulesRequest()
        if include_optional:
            return UpdateNetworkWirelessSsidTrafficShapingRulesRequest(
                default_rules_enabled = True,
                rules = [
                    openapi_client.models.update_network_wireless_ssid_traffic_shaping_rules_request_rules_inner.updateNetworkWirelessSsidTrafficShapingRules_request_rules_inner(
                        definitions = [
                            openapi_client.models.update_network_appliance_traffic_shaping_rules_request_rules_inner_definitions_inner.updateNetworkApplianceTrafficShapingRules_request_rules_inner_definitions_inner(
                                type = 'application', 
                                value = '', )
                            ], 
                        dscp_tag_value = 56, 
                        pcp_tag_value = 56, 
                        per_client_bandwidth_limits = openapi_client.models.update_network_appliance_traffic_shaping_rules_request_rules_inner_per_client_bandwidth_limits.updateNetworkApplianceTrafficShapingRules_request_rules_inner_perClientBandwidthLimits(
                            bandwidth_limits = openapi_client.models.update_network_appliance_traffic_shaping_rules_request_rules_inner_per_client_bandwidth_limits_bandwidth_limits.updateNetworkApplianceTrafficShapingRules_request_rules_inner_perClientBandwidthLimits_bandwidthLimits(
                                limit_down = 56, 
                                limit_up = 56, ), 
                            settings = '', ), )
                    ],
                traffic_shaping_enabled = True
            )
        else:
            return UpdateNetworkWirelessSsidTrafficShapingRulesRequest(
        )
        """

    def testUpdateNetworkWirelessSsidTrafficShapingRulesRequest(self):
        """Test UpdateNetworkWirelessSsidTrafficShapingRulesRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
