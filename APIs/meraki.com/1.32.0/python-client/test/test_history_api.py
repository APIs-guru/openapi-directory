# coding: utf-8

"""
    Meraki Dashboard API

    The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 05 April, 2023 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

    The version of the OpenAPI document: 1.32.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.history_api import HistoryApi


class TestHistoryApi(unittest.TestCase):
    """HistoryApi unit test stubs"""

    def setUp(self) -> None:
        self.api = HistoryApi()

    def tearDown(self) -> None:
        pass

    def test_get_device_camera_analytics_zone_history_3(self) -> None:
        """Test case for get_device_camera_analytics_zone_history_3

        Return historical records for analytic zones
        """
        pass

    def test_get_network_alerts_history_2(self) -> None:
        """Test case for get_network_alerts_history_2

        Return the alert history for this network
        """
        pass

    def test_get_organization_sensor_readings_history_2(self) -> None:
        """Test case for get_organization_sensor_readings_history_2

        Return all reported readings from sensors in a given timespan, sorted by timestamp
        """
        pass


if __name__ == '__main__':
    unittest.main()
