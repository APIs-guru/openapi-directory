...
class ProvisionNetworkClientsRequestPoliciesBySsid0(BaseModel):
    """
    The number for the SSID
    """ # noqa: E501
    device_policy: StrictStr = Field(description="The policy to apply to the specified client. Can be 'Whitelisted', 'Blocked', 'Normal' or 'Group policy'. Required.", alias="devicePolicy")
    ...
    def device_policy_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Blocked', 'Group policy', 'Normal', 'Whitelisted']):
            raise ValueError("must be one of enum values ('Blocked', 'Group policy', 'Normal', 'Whitelisted')")
        return value
