...
class TemplateDefinitionLayoutRepeatLayout(BaseModel):
    """
    Defines page size if layout is repeated on the page e.g sheet labels
    """ # noqa: E501
    format: Optional[StrictStr] = Field(default=None, description="Defines template page size")
    height: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Page height in units")
    width: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Page width in units")
    __properties: ClassVar[List[str]] = ["format", "height", "width"]
    ...
    def format_validate_enum(cls, value):
        ...
        if value not in set(['A4', 'letter', 'custom']):
            raise ValueError("must be one of enum values ('A4', 'letter', 'custom')")
        return value
