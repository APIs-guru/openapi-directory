...
class GetTemplates403Response(BaseModel):
    """
    GetTemplates403Response
    """ # noqa: E501
    error: Optional[StrictStr] = Field(default=None, description="Error description")
    status: Optional[StrictInt] = Field(default=None, description="HTTP Error code")
    ...
    def error_validate_enum(cls, value):
        ...
        if value not in set(['Your account has exceeded the monthly document generation limit.']):
            raise ValueError("must be one of enum values ('Your account has exceeded the monthly document generation limit.')")
        return value
