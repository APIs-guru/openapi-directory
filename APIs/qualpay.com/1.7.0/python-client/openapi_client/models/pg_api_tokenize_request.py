...
class PGApiTokenizeRequest(BaseModel):
    """
    ...
    single_use: Optional[StrictBool] = Field(default=None, description="<br><strong>Default: </strong>false<br><strong>Description: </strong>In a tokenize request, setting the single_use field to \"true\" will cause a single-use token to be generated. This token will expire in 10 minutes or when first used.")
    ...
    vendor_id: Optional[StrictInt] = Field(default=None, description="<strong>Format: </strong>Variable length, up to 12 N<br><strong>Description: </strong>Identifies the vendor to which this tokenize request applies.")
    __properties: ClassVar[List[str]] = ["avs_address", "avs_zip", "card_id", "card_number", "card_swipe", "cardholder_name", "client_ip", "cvv2", "dda_number", "developer_id", "echo_fields", "email_address", "exp_date", "loc_id", "merchant_id", "payload_apple_pay", "payload_google_pay", "profile_id", "report_data", "retry_attempt", "retry_id", "session_id", "single_use", "tr_number", "type_id", "user_id", "vendor_id"]
