...
class PGApiExpireTokenRequest(BaseModel):
    """
    PGApiExpireTokenRequest
    """ # noqa: E501
    card_id: StrictStr = Field(description="<strong>Format: </strong>Fixed length, 32 AN<br><strong>Description: </strong>Card ID received from a tokenization request.")
    ...
    vendor_id: Optional[StrictInt] = Field(default=None, description="<strong>Format: </strong>Variable length, up to 12 N<br><strong>Description: </strong>Identifies the vendor to which this expire token request applies.")
    __properties: ClassVar[List[str]] = ["card_id", "developer_id", "echo_fields", "loc_id", "merchant_id", "profile_id", "report_data", "retry_attempt", "retry_id", "session_id", "user_id", "vendor_id"]
