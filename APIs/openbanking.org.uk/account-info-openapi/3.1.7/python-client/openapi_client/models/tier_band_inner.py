...
class TierBandInner(BaseModel):
    ...
    application_frequency: StrictStr = Field(description="How often is interest applied to the BCA for this tier/band i.e. how often the financial institution pays accumulated interest to the customer's BCA.", alias="ApplicationFrequency")
    bank_interest_rate: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Bank Interest for the BCA product", alias="BankInterestRate")
    bank_interest_rate_type: Optional[StrictStr] = Field(default=None, description="Interest rate types, other than AER, which financial institutions may use to describe the annual interest rate payable to the BCA.", alias="BankInterestRateType")
    ...
    identification: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=35)]] = Field(default=None, description="Unique and unambiguous identification of a  Tier Band for a BCA.", alias="Identification")
    ...
    def application_frequency_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Daily', 'HalfYearly', 'Monthly', 'Other', 'Quarterly', 'PerStatementDate', 'Weekly', 'Yearly']):
            raise ValueError("must be one of enum values ('Daily', 'HalfYearly', 'Monthly', 'Other', 'Quarterly', 'PerStatementDate', 'Weekly', 'Yearly')")
        return value
