...
class FeeChargeCapInner(BaseModel):
    """
    ...
    min_max_type: StrictStr = Field(description="Min Max type", alias="MinMaxType")
    notes: Optional[List[Annotated[str, Field(min_length=1, strict=True, max_length=2000)]]] = Field(default=None, description="Free text for adding  extra details for fee charge cap", alias="Notes")
    ...
    def fee_type_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['Other', 'ServiceCAccountFee', 'ServiceCAccountFeeMonthly', 'ServiceCAccountFeeQuarterly', 'ServiceCFixedTariff', 'ServiceCBusiDepAccBreakage', 'ServiceCMinimumMonthlyFee', 'ServiceCOther']):
                raise ValueError("each list item must be one of ('Other', 'ServiceCAccountFee', 'ServiceCAccountFeeMonthly', 'ServiceCAccountFeeQuarterly', 'ServiceCFixedTariff', 'ServiceCBusiDepAccBreakage', 'ServiceCMinimumMonthlyFee', 'ServiceCOther')")
        return value
