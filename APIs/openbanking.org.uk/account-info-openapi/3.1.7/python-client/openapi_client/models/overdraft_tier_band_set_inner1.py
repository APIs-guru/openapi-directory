...
class OverdraftTierBandSetInner1(BaseModel):
    """
    ...
    notes: Optional[List[Annotated[str, Field(min_length=1, strict=True, max_length=2000)]]] = Field(default=None, description="Optional additional notes to supplement the overdraft Tier Band Set details", alias="Notes")
    overdraft_fees_charges: Optional[List[OverdraftFeesChargesInner2]] = Field(default=None, description="Overdraft fees and charges details", alias="OverdraftFeesCharges")
    overdraft_tier_band: Annotated[List[OverdraftTierBandInner1], Field(min_length=1)] = Field(description="Provides overdraft details for a specific tier or band", alias="OverdraftTierBand")
    ...
    def overdraft_type_validate_enum(cls, value):
        ...
        if value not in set(['Committed', 'OnDemand', 'Other']):
            raise ValueError("must be one of enum values ('Committed', 'OnDemand', 'Other')")
        return value

    @field_validator('tier_band_method')
    def tier_band_method_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Tiered', 'Whole', 'Banded']):
            raise ValueError("must be one of enum values ('Tiered', 'Whole', 'Banded')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "Notes": obj.get("Notes"),
            "OverdraftFeesCharges": [OverdraftFeesChargesInner2.from_dict(_item) for _item in obj["OverdraftFeesCharges"]] if obj.get("OverdraftFeesCharges") is not None else None,
            "OverdraftTierBand": [OverdraftTierBandInner1.from_dict(_item) for _item in obj["OverdraftTierBand"]] if obj.get("OverdraftTierBand") is not None else None,
