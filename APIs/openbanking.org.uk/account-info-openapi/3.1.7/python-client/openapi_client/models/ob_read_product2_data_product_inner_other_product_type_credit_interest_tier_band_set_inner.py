...
class OBReadProduct2DataProductInnerOtherProductTypeCreditInterestTierBandSetInner(BaseModel):
    """
    The group of tiers or bands for which credit interest can be applied.
    """ # noqa: E501
    calculation_method: Optional[OBInterestCalculationMethod1Code] = Field(default=None, alias="CalculationMethod")
    ...
    other_destination: Optional[OBOtherCodeType10] = Field(default=None, alias="OtherDestination")
    tier_band: Annotated[List[OBReadProduct2DataProductInnerOtherProductTypeCreditInterestTierBandSetInnerTierBandInner], Field(min_length=1)] = Field(alias="TierBand")
    tier_band_method: StrictStr = Field(description="The methodology of how credit interest is paid/applied. It can be:- 1. Banded Interest rates are banded. i.e. Increasing rate on whole balance as balance increases. 2. Tiered Interest rates are tiered. i.e. increasing rate for each tier as balance increases, but interest paid on tier fixed for that tier and not on whole balance. 3. Whole The same interest rate is applied irrespective of the product holder's account balance", alias="TierBandMethod")
    __properties: ClassVar[List[str]] = ["CalculationMethod", "Destination", "Notes", "OtherCalculationMethod", "OtherDestination", "TierBand", "TierBandMethod"]

    @field_validator('destination')
    def destination_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['INOT', 'INPA', 'INSC']):
            raise ValueError("must be one of enum values ('INOT', 'INPA', 'INSC')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of other_destination
        if self.other_destination:
            _dict['OtherDestination'] = self.other_destination.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "OtherDestination": OBOtherCodeType10.from_dict(obj["OtherDestination"]) if obj.get("OtherDestination") is not None else None,
            "TierBand": [OBReadProduct2DataProductInnerOtherProductTypeCreditInterestTierBandSetInnerTierBandInner.from_dict(_item) for _item in obj["TierBand"]] if obj.get("TierBand") is not None else None,
