...
class FeeChargeCapInner1(BaseModel):
    """
    ...
    def fee_type_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['ServiceCAccountFee', 'ServiceCAccountFeeMonthly', 'ServiceCOther', 'Other']):
                raise ValueError("each list item must be one of ('ServiceCAccountFee', 'ServiceCAccountFeeMonthly', 'ServiceCOther', 'Other')")
        return value
