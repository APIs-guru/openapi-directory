...
class JobPreparationTaskExecutionInformation(BaseModel):
    """
    ...
    last_retry_time: Optional[datetime] = Field(default=None, description="This property is set only if the Task was retried (i.e. retryCount is nonzero). If present, this is typically the same as startTime, but may be different if the Task has been restarted for reasons other than retry; for example, if the Compute Node was rebooted during a retry, then the startTime is updated but the lastRetryTime is not.", alias="lastRetryTime")
    ...
    __properties: ClassVar[List[str]] = ["containerInfo", "endTime", "exitCode", "failureInfo", "lastRetryTime", "result", "retryCount", "startTime", "state", "taskRootDirectory", "taskRootDirectoryUrl"]
