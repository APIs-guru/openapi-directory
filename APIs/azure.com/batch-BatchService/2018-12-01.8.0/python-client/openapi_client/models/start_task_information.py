...
class StartTaskInformation(BaseModel):
    """
    StartTaskInformation
    """ # noqa: E501
    container_info: Optional[TaskContainerExecutionInformation] = Field(default=None, alias="containerInfo")
    end_time: Optional[datetime] = Field(default=None, description="This is the end time of the most recent run of the start task, if that run has completed (even if that run failed and a retry is pending). This element is not present if the start task is currently running.", alias="endTime")
    ...
    start_time: datetime = Field(description="This value is reset every time the task is restarted or retried (that is, this is the most recent time at which the start task started running).", alias="startTime")
    state: StrictStr
    __properties: ClassVar[List[str]] = ["containerInfo", "endTime", "exitCode", "failureInfo", "lastRetryTime", "result", "retryCount", "startTime", "state"]
