...
class JobInformation(BaseModel):
    ...
    """ # noqa: E501
    error_message: Optional[List[JobErrorDetails]] = Field(default=None, description="The error message details for the job, if the job failed.", alias="errorMessage")
    properties: JobProperties
    state_audit_records: Optional[List[JobStateAuditRecord]] = Field(default=None, description="The job state audit records, indicating when various operations have been performed on this job.", alias="stateAuditRecords")
    ...
    tags: Optional[Dict[str, StrictStr]] = Field(default=None, description="The key-value pairs used to add additional metadata to the job information. (Only for use internally with Scope job type.)")
    type: StrictStr = Field(description="The job type of the current job (Hive, USql, or Scope (for internal use only)).")
    __properties: ClassVar[List[str]] = ["degreeOfParallelism", "degreeOfParallelismPercent", "endTime", "hierarchyQueueNode", "jobId", "logFilePatterns", "logFolder", "name", "priority", "related", "result", "startTime", "state", "submitTime", "submitter", "tags", "type"]
    ...
    def state_validate_enum(cls, value):
        ...
        if value not in set(['Accepted', 'Compiling', 'Ended', 'New', 'Queued', 'Running', 'Scheduling', 'Starting', 'Paused', 'WaitingForCapacity', 'Yielded', 'Finalizing']):
            raise ValueError("must be one of enum values ('Accepted', 'Compiling', 'Ended', 'New', 'Queued', 'Running', 'Scheduling', 'Starting', 'Paused', 'WaitingForCapacity', 'Yielded', 'Finalizing')")
        return value
