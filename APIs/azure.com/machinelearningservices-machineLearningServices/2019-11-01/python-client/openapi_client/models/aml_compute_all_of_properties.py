...
class AmlComputeAllOfProperties(BaseModel):
    ...
    remote_login_port_public_access: Optional[StrictStr] = Field(default='NotSpecified', description="State of the public SSH port. Possible values are: Disabled - Indicates that the public ssh port is closed on all nodes of the cluster. Enabled - Indicates that the public ssh port is open on all nodes of the cluster. NotSpecified - Indicates that the public ssh port is closed on all nodes of the cluster if VNet is defined, else is open all public nodes. It can be default only during cluster creation time, after creation it will be either enabled or disabled.", alias="remoteLoginPortPublicAccess")
    ...
    __properties: ClassVar[List[str]] = ["allocationState", "allocationStateTransitionTime", "currentNodeCount", "errors", "nodeStateCounts", "remoteLoginPortPublicAccess", "scaleSettings", "subnet", "targetNodeCount", "userAccountCredentials", "vmPriority", "vmSize"]
    ...
    @field_validator('remote_login_port_public_access')
    def remote_login_port_public_access_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['Enabled', 'Disabled', 'NotSpecified']):
            raise ValueError("must be one of enum values ('Enabled', 'Disabled', 'NotSpecified')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "remoteLoginPortPublicAccess": obj.get("remoteLoginPortPublicAccess") if obj.get("remoteLoginPortPublicAccess") is not None else 'NotSpecified',
