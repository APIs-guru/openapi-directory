...
class ServerConnectionPolicyProperties(BaseModel):
    """
    The properties of a server secure connection policy.
    """ # noqa: E501
    connection_type: StrictStr = Field(description="The server connection type.", alias="connectionType")
    __properties: ClassVar[List[str]] = ["connectionType"]

    @field_validator('connection_type')
    def connection_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Default', 'Proxy', 'Redirect']):
            raise ValueError("must be one of enum values ('Default', 'Proxy', 'Redirect')")
        return value
