...
class ApiProductsListByApis200ResponseValueInner(BaseModel):
    """
    Product profile.
    """ # noqa: E501
    ...
    id: Optional[StrictStr] = Field(default=None, description="Uniquely identifies the product within the current API Management service instance. The value is a valid relative URL in the format of /products/{productId} where {productId} is a product identifier.")
    name: Annotated[str, Field(min_length=1, strict=True, max_length=300)] = Field(description="Product name.")
    state: Optional[StrictStr] = Field(default='NotPublished', description="whether product is published or not. Published products are discoverable by users of developer portal. Non published products are visible only to administrators. Default state of Product is NotPublished.")
    subscription_required: Optional[StrictBool] = Field(default=True, description="Whether a product subscription is required for accessing APIs included in this product. If true, the product is referred to as \"protected\" and a valid subscription key is required for a request to an API included in the product to succeed. If false, the product is referred to as \"open\" and requests to an API included in the product can be made without a subscription key. If property is omitted when creating a new product it's value is assumed to be true.", alias="subscriptionRequired")
    ...
    __properties: ClassVar[List[str]] = ["approvalRequired", "description", "id", "name", "state", "subscriptionRequired", "subscriptionsLimit", "terms"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "id": obj.get("id"),
            "name": obj.get("name"),
            "state": obj.get("state") if obj.get("state") is not None else 'NotPublished',
            "subscriptionRequired": obj.get("subscriptionRequired") if obj.get("subscriptionRequired") is not None else True,
