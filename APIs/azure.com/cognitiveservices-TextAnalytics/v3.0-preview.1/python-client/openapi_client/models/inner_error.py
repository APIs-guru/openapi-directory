...
class InnerError(BaseModel):
    ...
    details: Optional[Dict[str, StrictStr]] = Field(default=None, description="Error details.")
    inner_error: Optional[InnerError] = Field(default=None, alias="innerError")
    message: StrictStr = Field(description="Error message.")
    target: Optional[StrictStr] = Field(default=None, description="Error target.")
    __properties: ClassVar[List[str]] = ["code", "details", "innerError", "message", "target"]

    @field_validator('code')
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['invalidParameterValue', 'invalidRequestBodyFormat', 'emptyRequest', 'missingInputRecords', 'invalidDocument', 'modelVersionIncorrect', 'invalidDocumentBatch', 'unsupportedLanguageCode', 'invalidCountryHint']):
            raise ValueError("must be one of enum values ('invalidParameterValue', 'invalidRequestBodyFormat', 'emptyRequest', 'missingInputRecords', 'invalidDocument', 'modelVersionIncorrect', 'invalidDocumentBatch', 'unsupportedLanguageCode', 'invalidCountryHint')")
        return value
...
InnerError.model_rebuild(raise_errors=False)
