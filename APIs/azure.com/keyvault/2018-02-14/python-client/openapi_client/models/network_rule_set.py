...
class NetworkRuleSet(BaseModel):
    ...
    """ # noqa: E501
    bypass: Optional[StrictStr] = Field(default=None, description="Tells what traffic can bypass network rules. This can be 'AzureServices' or 'None'.  If not specified the default is 'AzureServices'.")
    default_action: Optional[StrictStr] = Field(default=None, description="The default action when no rule from ipRules and from virtualNetworkRules match. This is only used after the bypass property has been evaluated.", alias="defaultAction")
    ip_rules: Optional[List[IPRule]] = Field(default=None, description="The list of IP address rules.", alias="ipRules")
    virtual_network_rules: Optional[List[VirtualNetworkRule]] = Field(default=None, description="The list of virtual network rules.", alias="virtualNetworkRules")
    ...
    def bypass_validate_enum(cls, value):
        ...
        if value not in set(['AzureServices', 'None']):
            raise ValueError("must be one of enum values ('AzureServices', 'None')")
        return value
