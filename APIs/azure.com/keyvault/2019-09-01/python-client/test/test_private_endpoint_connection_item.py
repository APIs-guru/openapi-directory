# coding: utf-8

"""
    KeyVaultManagementClient

    The Azure management API provides a RESTful set of web services that interact with Azure Key Vault.

    The version of the OpenAPI document: 2019-09-01
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.private_endpoint_connection_item import PrivateEndpointConnectionItem

class TestPrivateEndpointConnectionItem(unittest.TestCase):
    """PrivateEndpointConnectionItem unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PrivateEndpointConnectionItem:
        """Test PrivateEndpointConnectionItem
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PrivateEndpointConnectionItem`
        """
        model = PrivateEndpointConnectionItem()
        if include_optional:
            return PrivateEndpointConnectionItem(
                properties = openapi_client.models.private_endpoint_connection_properties.PrivateEndpointConnectionProperties(
                    private_endpoint = openapi_client.models.private_endpoint.PrivateEndpoint(
                        id = '', ), 
                    private_link_service_connection_state = openapi_client.models.private_link_service_connection_state.PrivateLinkServiceConnectionState(
                        action_required = '', 
                        description = '', 
                        status = 'Pending', ), 
                    provisioning_state = 'Succeeded', )
            )
        else:
            return PrivateEndpointConnectionItem(
        )
        """

    def testPrivateEndpointConnectionItem(self):
        """Test PrivateEndpointConnectionItem"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
