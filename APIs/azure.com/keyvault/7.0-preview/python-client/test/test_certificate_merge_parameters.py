# coding: utf-8

"""
    KeyVaultClient

    The key vault client performs cryptographic key operations and vault operations against the Key Vault service.

    The version of the OpenAPI document: 7.0-preview
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.certificate_merge_parameters import CertificateMergeParameters

class TestCertificateMergeParameters(unittest.TestCase):
    """CertificateMergeParameters unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CertificateMergeParameters:
        """Test CertificateMergeParameters
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CertificateMergeParameters`
        """
        model = CertificateMergeParameters()
        if include_optional:
            return CertificateMergeParameters(
                attributes = openapi_client.models.certificate_attributes.CertificateAttributes(
                    recovery_level = 'Purgeable', ),
                tags = {
                    'key' : ''
                    },
                x5c = [
                    'YQ=='
                    ]
            )
        else:
            return CertificateMergeParameters(
                x5c = [
                    'YQ=='
                    ],
        )
        """

    def testCertificateMergeParameters(self):
        """Test CertificateMergeParameters"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
