...
class YearlyRetentionSchedule(BaseModel):
    """
    Yearly retention schedule.
    """ # noqa: E501
    months_of_year: Optional[List[StrictStr]] = Field(default=None, description="List of months of year of yearly retention policy.", alias="monthsOfYear")
    ...
    retention_schedule_format_type: Optional[StrictStr] = Field(default=None, description="Retention schedule format for yearly retention policy.", alias="retentionScheduleFormatType")
    ...
    __properties: ClassVar[List[str]] = ["monthsOfYear", "retentionDuration", "retentionScheduleDaily", "retentionScheduleFormatType", "retentionScheduleWeekly", "retentionTimes"]

    @field_validator('months_of_year')
    def months_of_year_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['Invalid', 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']):
                raise ValueError("each list item must be one of ('Invalid', 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "monthsOfYear": obj.get("monthsOfYear"),
