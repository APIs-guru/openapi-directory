...
class ClusterProperties(BaseModel):
    ...
    """ # noqa: E501
    available_cluster_versions: Optional[List[ClusterVersionDetails]] = Field(default=None, description="The available cluster code version which the cluster can upgrade to, note that you must choose upgradeMode to manual to upgrade to", alias="availableClusterVersions")
    ...
    client_certificate_thumbprints: Optional[List[ClientCertificateThumbprint]] = Field(default=None, description="The client thumbprint details ,it is used for client access for cluster operation", alias="clientCertificateThumbprints")
    cluster_code_version: Optional[StrictStr] = Field(default=None, description="The ServiceFabric code version running in your cluster", alias="clusterCodeVersion")
    cluster_endpoint: Optional[StrictStr] = Field(default=None, description="The endpoint for the cluster connecting to servicefabric resource provider", alias="clusterEndpoint")
    cluster_id: Optional[StrictStr] = Field(default=None, description="The unique identifier for the cluster resource", alias="clusterId")
    cluster_state: Optional[StrictStr] = Field(default=None, description="The state for the cluster", alias="clusterState")
    ...
    fabric_settings: Optional[List[SettingsSectionDescription]] = Field(default=None, description="List of custom fabric settings to configure the cluster.", alias="fabricSettings")
    management_endpoint: StrictStr = Field(description="The http management endpoint of the cluster", alias="managementEndpoint")
    node_types: List[NodeTypeDescription] = Field(description="The list of node types that make up the cluster", alias="nodeTypes")
    ...
    reliability_level: Optional[StrictStr] = Field(default=None, description="Cluster reliability level indicates replica set size of system service", alias="reliabilityLevel")
    ...
    vm_image: Optional[StrictStr] = Field(default=None, description="The name of VM image VMSS has been configured with. Generic names such as Windows or Linux can be used.", alias="vmImage")
    __properties: ClassVar[List[str]] = ["availableClusterVersions", "azureActiveDirectory", "certificate", "clientCertificateCommonNames", "clientCertificateThumbprints", "clusterCodeVersion", "clusterEndpoint", "clusterId", "clusterState", "diagnosticsStorageAccountConfig", "fabricSettings", "managementEndpoint", "nodeTypes", "provisioningState", "reliabilityLevel", "reverseProxyCertificate", "upgradeDescription", "upgradeMode", "vmImage"]
    ...
    def cluster_state_validate_enum(cls, value):
        ...
        if value not in set(['WaitingForNodes', 'Deploying', 'BaselineUpgrade', 'UpdatingUserConfiguration', 'UpdatingUserCertificate', 'UpdatingInfrastructure', 'EnforcingClusterVersion', 'UpgradeServiceUnreachable', 'AutoScale', 'Ready']):
            raise ValueError("must be one of enum values ('WaitingForNodes', 'Deploying', 'BaselineUpgrade', 'UpdatingUserConfiguration', 'UpdatingUserCertificate', 'UpdatingInfrastructure', 'EnforcingClusterVersion', 'UpgradeServiceUnreachable', 'AutoScale', 'Ready')")
        return value
    ...
    def reliability_level_validate_enum(cls, value):
        ...
        if value not in set(['Bronze', 'Silver', 'Gold', 'Platinum']):
            raise ValueError("must be one of enum values ('Bronze', 'Silver', 'Gold', 'Platinum')")
        return value
