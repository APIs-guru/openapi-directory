...
> start_rollback_compose_deployment_upgrade(api_version, deployment_name, timeout=timeout)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Starts rolling back a compose deployment upgrade in the Service Fabric cluster.
        api_instance.start_rollback_compose_deployment_upgrade(api_version, deployment_name, timeout=timeout)
    except Exception as e:
        print("Exception when calling ComposeDeploymentApi->start_rollback_compose_deployment_upgrade: %s\n" % e)
```
