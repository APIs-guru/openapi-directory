...
# **get_node_info_list**
> PagedNodeInfoList get_node_info_list(api_version, continuation_token=continuation_token, node_status_filter=node_status_filter, max_results=max_results, timeout=timeout)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    node_status_filter = default # str | Allows filtering the nodes based on the NodeStatus. Only the nodes that are matching the specified filter value will be returned. The filter value can be one of the following. (optional) (default to default)
    ...
    try:
        # Gets the list of nodes in the Service Fabric cluster.
        api_response = api_instance.get_node_info_list(api_version, continuation_token=continuation_token, node_status_filter=node_status_filter, max_results=max_results, timeout=timeout)
 ...
 **node_status_filter** | **str**| Allows filtering the nodes based on the NodeStatus. Only the nodes that are matching the specified filter value will be returned. The filter value can be one of the following. | [optional] [default to default]
