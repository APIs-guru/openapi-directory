...
> ServiceTypeInfo get_service_type_info_by_name(api_version, application_type_name, application_type_version, service_type_name, timeout=timeout)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Gets the information about a specific service type that is supported by a provisioned application type in a Service Fabric cluster.
        api_response = api_instance.get_service_type_info_by_name(api_version, application_type_name, application_type_version, service_type_name, timeout=timeout)
        print("The response of ServiceTypeApi->get_service_type_info_by_name:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ServiceTypeApi->get_service_type_info_by_name: %s\n" % e)
```
