...
class ContainerCodePackageProperties(BaseModel):
    ...
    liveness_probe: Optional[List[Probe]] = Field(default=None, description="An array of liveness probes for a code package. It determines when to restart a code package.", alias="livenessProbe")
    name: StrictStr = Field(description="The name of the code package.")
    readiness_probe: Optional[List[Probe]] = Field(default=None, description="An array of readiness probes for a code package. It determines when to unpublish an endpoint.", alias="readinessProbe")
    ...
    __properties: ClassVar[List[str]] = ["commands", "diagnostics", "endpoints", "entrypoint", "environmentVariables", "image", "imageRegistryCredential", "instanceView", "labels", "livenessProbe", "name", "readinessProbe", "reliableCollectionsRefs", "resources", "settings", "volumeRefs", "volumes"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.labels:
            ...
            _dict['labels'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in liveness_probe (list)
        _items = []
        if self.liveness_probe:
            for _item_liveness_probe in self.liveness_probe:
                if _item_liveness_probe:
                    _items.append(_item_liveness_probe.to_dict())
            _dict['livenessProbe'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in readiness_probe (list)
        _items = []
        if self.readiness_probe:
            for _item_readiness_probe in self.readiness_probe:
                if _item_readiness_probe:
                    _items.append(_item_readiness_probe.to_dict())
            _dict['readinessProbe'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "livenessProbe": [Probe.from_dict(_item) for _item in obj["livenessProbe"]] if obj.get("livenessProbe") is not None else None,
            "name": obj.get("name"),
            "readinessProbe": [Probe.from_dict(_item) for _item in obj["readinessProbe"]] if obj.get("readinessProbe") is not None else None,
