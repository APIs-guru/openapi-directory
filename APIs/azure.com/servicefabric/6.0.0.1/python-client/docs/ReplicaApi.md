...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    partition_id = 'partition_id_example' # str | The identity of the partition.
    replica_id = 'replica_id_example' # str | The identifier of the replica.
    service_kind = Stateful # str | The kind of service replica (Stateless or Stateful) for which the health is being reported. Following are the possible values. - Stateless - Does not use Service Fabric to make its state highly available or reliable. The value is 1 - Stateful - Uses Service Fabric to make its state or part of its state highly available and reliable. The value is 2.  (default to Stateful)
