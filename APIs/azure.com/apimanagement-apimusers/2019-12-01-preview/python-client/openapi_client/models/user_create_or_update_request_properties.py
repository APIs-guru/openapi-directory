...
class UserCreateOrUpdateRequestProperties(BaseModel):
    ...
    """ # noqa: E501
    app_type: Optional[StrictStr] = Field(default=None, description="Determines the type of application which send the create user request. Default is old publisher portal.", alias="appType")
    ...
    __properties: ClassVar[List[str]] = ["appType", "confirmation", "email", "firstName", "lastName", "password"]
    ...
    def app_type_validate_enum(cls, value):
        ...
        if value not in set(['developerPortal']):
            raise ValueError("must be one of enum values ('developerPortal')")
        return value
