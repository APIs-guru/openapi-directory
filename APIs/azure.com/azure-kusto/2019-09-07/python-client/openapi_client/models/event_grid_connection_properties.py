...
class EventGridConnectionProperties(BaseModel):
    """
    ...
    data_format: DataFormat = Field(alias="dataFormat")
    event_hub_resource_id: StrictStr = Field(description="The resource ID where the event grid is configured to send events.", alias="eventHubResourceId")
    ...
    storage_account_resource_id: StrictStr = Field(description="The resource ID of the storage account where the data resides.", alias="storageAccountResourceId")
    table_name: StrictStr = Field(description="The table where the data should be ingested. Optionally the table information can be added to each message.", alias="tableName")
    __properties: ClassVar[List[str]] = ["consumerGroup", "dataFormat", "eventHubResourceId", "mappingRuleName", "storageAccountResourceId", "tableName"]
