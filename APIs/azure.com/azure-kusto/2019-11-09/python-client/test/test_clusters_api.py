# coding: utf-8

"""
    KustoManagementClient

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 2019-11-09
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.clusters_api import ClustersApi


class TestClustersApi(unittest.TestCase):
    """ClustersApi unit test stubs"""

    def setUp(self) -> None:
        self.api = ClustersApi()

    def tearDown(self) -> None:
        pass

    def test_clusters_check_name_availability(self) -> None:
        """Test case for clusters_check_name_availability

        """
        pass

    def test_clusters_create_or_update(self) -> None:
        """Test case for clusters_create_or_update

        """
        pass

    def test_clusters_delete(self) -> None:
        """Test case for clusters_delete

        """
        pass

    def test_clusters_detach_follower_databases(self) -> None:
        """Test case for clusters_detach_follower_databases

        """
        pass

    def test_clusters_diagnose_virtual_network(self) -> None:
        """Test case for clusters_diagnose_virtual_network

        """
        pass

    def test_clusters_get(self) -> None:
        """Test case for clusters_get

        """
        pass

    def test_clusters_list(self) -> None:
        """Test case for clusters_list

        """
        pass

    def test_clusters_list_by_resource_group(self) -> None:
        """Test case for clusters_list_by_resource_group

        """
        pass

    def test_clusters_list_follower_databases(self) -> None:
        """Test case for clusters_list_follower_databases

        """
        pass

    def test_clusters_list_skus_by_resource(self) -> None:
        """Test case for clusters_list_skus_by_resource

        """
        pass

    def test_clusters_start(self) -> None:
        """Test case for clusters_start

        """
        pass

    def test_clusters_stop(self) -> None:
        """Test case for clusters_stop

        """
        pass

    def test_clusters_update(self) -> None:
        """Test case for clusters_update

        """
        pass


if __name__ == '__main__':
    unittest.main()
