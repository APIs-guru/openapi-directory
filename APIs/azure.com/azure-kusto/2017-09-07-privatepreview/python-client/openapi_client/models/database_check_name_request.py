...
class DatabaseCheckNameRequest(BaseModel):
    """
    DatabaseCheckNameRequest
    """ # noqa: E501
    name: StrictStr = Field(description="Database name.")
    type: StrictStr = Field(description="The type of resource, Microsoft.Kusto/clusters/databases.")
    __properties: ClassVar[List[str]] = ["name", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Microsoft.Kusto/clusters/databases']):
            raise ValueError("must be one of enum values ('Microsoft.Kusto/clusters/databases')")
        return value
