# coding: utf-8

"""
    Cosmos DB

    Azure Cosmos DB Database Service Resource Provider REST API

    The version of the OpenAPI document: 2019-12-12
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.sql_container_get_properties_resource import SqlContainerGetPropertiesResource

class TestSqlContainerGetPropertiesResource(unittest.TestCase):
    """SqlContainerGetPropertiesResource unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> SqlContainerGetPropertiesResource:
        """Test SqlContainerGetPropertiesResource
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SqlContainerGetPropertiesResource`
        """
        model = SqlContainerGetPropertiesResource()
        if include_optional:
            return SqlContainerGetPropertiesResource(
                conflict_resolution_policy = openapi_client.models.conflict_resolution_policy.ConflictResolutionPolicy(
                    conflict_resolution_path = '', 
                    conflict_resolution_procedure = '', 
                    mode = 'LastWriterWins', ),
                default_ttl = 56,
                id = '',
                indexing_policy = openapi_client.models.indexing_policy.IndexingPolicy(
                    automatic = True, 
                    composite_indexes = [
                        [
                            openapi_client.models.composite_path.CompositePath(
                                order = 'Ascending', 
                                path = '', )
                            ]
                        ], 
                    excluded_paths = [
                        openapi_client.models.excluded_path.ExcludedPath(
                            path = '', )
                        ], 
                    included_paths = [
                        openapi_client.models.included_path.IncludedPath(
                            indexes = [
                                openapi_client.models.indexes.Indexes(
                                    data_type = 'String', 
                                    kind = 'Hash', 
                                    precision = 56, )
                                ], 
                            path = '', )
                        ], 
                    indexing_mode = 'Consistent', 
                    spatial_indexes = [
                        openapi_client.models.spatial_spec.SpatialSpec(
                            path = '', 
                            types = [
                                'Point'
                                ], )
                        ], ),
                partition_key = openapi_client.models.container_partition_key.ContainerPartitionKey(
                    kind = 'Hash', 
                    paths = [
                        ''
                        ], 
                    version = 1, ),
                unique_key_policy = openapi_client.models.unique_key_policy.UniqueKeyPolicy(
                    unique_keys = [
                        openapi_client.models.unique_key.UniqueKey(
                            paths = [
                                ''
                                ], )
                        ], ),
                etag = '',
                rid = '',
                ts = openapi_client.models._ts._ts()
            )
        else:
            return SqlContainerGetPropertiesResource(
                id = '',
        )
        """

    def testSqlContainerGetPropertiesResource(self):
        """Test SqlContainerGetPropertiesResource"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
