...
class ElasticPoolPerDatabaseMinPerformanceLevelCapability(BaseModel):
    """
    The minimum per-database performance level capability.
    """ # noqa: E501
    limit: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="The minimum performance level per database.")
    ...
    unit: Optional[StrictStr] = Field(default=None, description="Unit type used to measure performance level.")
    __properties: ClassVar[List[str]] = ["limit", "reason", "status", "unit"]
    ...
    def unit_validate_enum(cls, value):
        ...
        if value not in set(['DTU', 'VCores']):
            raise ValueError("must be one of enum values ('DTU', 'VCores')")
        return value
