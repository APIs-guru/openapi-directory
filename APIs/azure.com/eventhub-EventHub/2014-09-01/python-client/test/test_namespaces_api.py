# coding: utf-8

"""
    EventHubManagementClient

    Azure Event Hubs client

    The version of the OpenAPI document: 2014-09-01
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.namespaces_api import NamespacesApi


class TestNamespacesApi(unittest.TestCase):
    """NamespacesApi unit test stubs"""

    def setUp(self) -> None:
        self.api = NamespacesApi()

    def tearDown(self) -> None:
        pass

    def test_namespaces_check_name_availability(self) -> None:
        """Test case for namespaces_check_name_availability

        """
        pass

    def test_namespaces_check_name_space_availability(self) -> None:
        """Test case for namespaces_check_name_space_availability

        """
        pass

    def test_namespaces_create_or_update(self) -> None:
        """Test case for namespaces_create_or_update

        """
        pass

    def test_namespaces_create_or_update_authorization_rule(self) -> None:
        """Test case for namespaces_create_or_update_authorization_rule

        """
        pass

    def test_namespaces_delete(self) -> None:
        """Test case for namespaces_delete

        """
        pass

    def test_namespaces_delete_authorization_rule(self) -> None:
        """Test case for namespaces_delete_authorization_rule

        """
        pass

    def test_namespaces_get(self) -> None:
        """Test case for namespaces_get

        """
        pass

    def test_namespaces_get_authorization_rule(self) -> None:
        """Test case for namespaces_get_authorization_rule

        """
        pass

    def test_namespaces_list_authorization_rules(self) -> None:
        """Test case for namespaces_list_authorization_rules

        """
        pass

    def test_namespaces_list_by_resource_group(self) -> None:
        """Test case for namespaces_list_by_resource_group

        """
        pass

    def test_namespaces_list_by_subscription(self) -> None:
        """Test case for namespaces_list_by_subscription

        """
        pass

    def test_namespaces_list_post_authorization_rules(self) -> None:
        """Test case for namespaces_list_post_authorization_rules

        """
        pass

    def test_namespaces_post_authorization_rule(self) -> None:
        """Test case for namespaces_post_authorization_rule

        """
        pass

    def test_namespaces_update(self) -> None:
        """Test case for namespaces_update

        """
        pass


if __name__ == '__main__':
    unittest.main()
