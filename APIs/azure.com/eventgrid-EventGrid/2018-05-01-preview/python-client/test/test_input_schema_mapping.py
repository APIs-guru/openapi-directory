# coding: utf-8

"""
    EventGridManagementClient

    Azure EventGrid Management Client

    The version of the OpenAPI document: 2018-05-01-preview
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.input_schema_mapping import InputSchemaMapping

class TestInputSchemaMapping(unittest.TestCase):
    """InputSchemaMapping unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> InputSchemaMapping:
        """Test InputSchemaMapping
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `InputSchemaMapping`
        """
        model = InputSchemaMapping()
        if include_optional:
            return InputSchemaMapping(
                input_schema_mapping_type = 'Json'
            )
        else:
            return InputSchemaMapping(
        )
        """

    def testInputSchemaMapping(self):
        """Test InputSchemaMapping"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
