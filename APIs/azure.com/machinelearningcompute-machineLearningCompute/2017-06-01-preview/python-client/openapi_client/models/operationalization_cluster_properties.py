...
class OperationalizationClusterProperties(BaseModel):
    ...
    container_service: AcsClusterProperties = Field(alias="containerService")
    ...
    __properties: ClassVar[List[str]] = ["appInsights", "clusterType", "containerRegistry", "containerService", "createdOn", "description", "globalServiceConfiguration", "modifiedOn", "provisioningState", "storageAccount"]

    @field_validator('cluster_type')
    def cluster_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACS']):
            raise ValueError("must be one of enum values ('ACS')")
        return value
