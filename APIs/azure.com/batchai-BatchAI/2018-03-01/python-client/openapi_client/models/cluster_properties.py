...
class ClusterProperties(BaseModel):
    ...
    """ # noqa: E501
    allocation_state: Optional[StrictStr] = Field(default=None, description="Possible values are: steady and resizing. steady state indicates that the cluster is not resizing. There are no changes to the number of compute nodes in the cluster in progress. A cluster enters this state when it is created and when no operations are being performed on the cluster to change the number of compute nodes. resizing state indicates that the cluster is resizing; that is, compute nodes are being added to or removed from the cluster.", alias="allocationState")
    allocation_state_transition_time: Optional[datetime] = Field(default=None, alias="allocationStateTransitionTime")
    creation_time: Optional[datetime] = Field(default=None, alias="creationTime")
    current_node_count: Optional[StrictInt] = Field(default=None, alias="currentNodeCount")
    errors: Optional[List[BatchAIError]] = Field(default=None, description="This element contains all the errors encountered by various compute nodes during node setup.")
    ...
    provisioning_state: Optional[StrictStr] = Field(default=None, description="Possible value are: creating - Specifies that the cluster is being created. succeeded - Specifies that the cluster has been created successfully. failed - Specifies that the cluster creation has failed. deleting - Specifies that the cluster is being deleted.", alias="provisioningState")
    ...
    vm_priority: Optional[StrictStr] = Field(default='dedicated', description="The default value is dedicated. The node can get preempted while the task is running if lowpriority is chosen. This is best suited if the workload is checkpointing and can be restarted.", alias="vmPriority")
    vm_size: Optional[StrictStr] = Field(default=None, description="All virtual machines in a cluster are the same size. For information about available VM sizes for clusters using images from the Virtual Machines Marketplace (see Sizes for Virtual Machines (Linux) or Sizes for Virtual Machines (Windows). Batch AI service supports all Azure VM sizes except STANDARD_A0 and those with premium storage (STANDARD_GS, STANDARD_DS, and STANDARD_DSV2 series).", alias="vmSize")
