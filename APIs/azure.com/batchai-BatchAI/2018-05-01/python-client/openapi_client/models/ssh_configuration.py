...
class SshConfiguration(BaseModel):
    ...
    SSH configuration.
    """ # noqa: E501
    public_ips_to_allow: Optional[List[StrictStr]] = Field(default=None, description="List of source IP ranges to allow SSH connection from. The default value is '*' (all source IPs are allowed). Maximum number of IP ranges that can be specified is 400.", alias="publicIPsToAllow")
