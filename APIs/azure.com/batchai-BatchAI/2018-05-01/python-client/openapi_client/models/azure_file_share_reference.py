...
class AzureFileShareReference(BaseModel):
    ...
    azure_file_url: StrictStr = Field(description="URL to access the Azure File.", alias="azureFileUrl")
    credentials: AzureStorageCredentialsInfo
    directory_mode: Optional[StrictStr] = Field(default='0777', description="File mode for directories on the mounted file share. Default value: 0777.", alias="directoryMode")
    file_mode: Optional[StrictStr] = Field(default='0777', description="File mode for files on the mounted file share. Default value: 0777.", alias="fileMode")
    relative_mount_path: StrictStr = Field(description="The relative path on the compute node where the Azure File share will be mounted. Note that all cluster level file shares will be mounted under $AZ_BATCHAI_MOUNT_ROOT location and all job level file shares will be mounted under $AZ_BATCHAI_JOB_MOUNT_ROOT.", alias="relativeMountPath")
