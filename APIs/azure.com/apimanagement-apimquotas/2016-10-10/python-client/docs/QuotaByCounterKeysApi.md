...
> QuotaCounterCollection quota_by_counter_keys_list_by_service(subscription_id, resource_group_name, service_name, quota_counter_key, api_version)
 ...
 **quota_counter_key** | **str**| Quota counter key identifier. | 
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    api_version = 'api_version_example' # str | Version of the API to be used with the client request.
    subscription_id = 'subscription_id_example' # str | Subscription credentials which uniquely identify Microsoft Azure subscription. The subscription ID forms part of the URI for every service call.
    parameters = openapi_client.QuotaCounterValueContract() # QuotaCounterValueContract | The value of the quota counter to be applied to all quota counter periods.
