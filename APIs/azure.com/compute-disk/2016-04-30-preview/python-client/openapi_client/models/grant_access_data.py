...
class GrantAccessData(BaseModel):
    ...
    """ # noqa: E501
    access: StrictStr
    duration_in_seconds: StrictInt = Field(description="Time duration in seconds until the SAS access expires.", alias="durationInSeconds")
    __properties: ClassVar[List[str]] = ["access", "durationInSeconds"]

    @field_validator('access')
    def access_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['None', 'Read']):
            raise ValueError("must be one of enum values ('None', 'Read')")
        return value
