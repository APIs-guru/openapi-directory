...
class PeriodicTimerEventTrigger(BaseModel):
    """
    Trigger details.
    """ # noqa: E501
    ...
    kind: StrictStr = Field(description="Trigger Kind.")
    __properties: ClassVar[List[str]] = ["kind", "id", "name", "type"]

    @field_validator('kind')
    def kind_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FileEvent', 'PeriodicTimerEvent']):
            raise ValueError("must be one of enum values ('FileEvent', 'PeriodicTimerEvent')")
        return value
