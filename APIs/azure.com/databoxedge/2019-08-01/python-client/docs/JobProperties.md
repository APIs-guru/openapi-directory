...
**folder** | **str** | If only subfolders need to be refreshed, then the subfolder path inside the share or container. (The path is empty if there are no subfolders.) | [optional] 
...
**refreshed_entity_id** | **str** | ARM ID of the entity that was refreshed. | [optional] [readonly] 
