# coding: utf-8

"""
    StreamAnalyticsManagementClient

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 2016-03-01
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.streaming_job_properties_inputs_inner import StreamingJobPropertiesInputsInner

class TestStreamingJobPropertiesInputsInner(unittest.TestCase):
    """StreamingJobPropertiesInputsInner unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> StreamingJobPropertiesInputsInner:
        """Test StreamingJobPropertiesInputsInner
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `StreamingJobPropertiesInputsInner`
        """
        model = StreamingJobPropertiesInputsInner()
        if include_optional:
            return StreamingJobPropertiesInputsInner(
                properties = openapi_client.models.streaming_job_properties_inputs_inner_properties.StreamingJobProperties_inputs_inner_properties(
                    diagnostics = openapi_client.models.streaming_job_properties_inputs_inner_properties_diagnostics.StreamingJobProperties_inputs_inner_properties_diagnostics(
                        conditions = [
                            openapi_client.models.streaming_job_properties_inputs_inner_properties_diagnostics_conditions_inner.StreamingJobProperties_inputs_inner_properties_diagnostics_conditions_inner(
                                code = '', 
                                message = '', 
                                since = '', )
                            ], ), 
                    etag = '', 
                    serialization = openapi_client.models.streaming_job_properties_inputs_inner_properties_serialization.StreamingJobProperties_inputs_inner_properties_serialization(
                        type = '', ), 
                    type = '', )
            )
        else:
            return StreamingJobPropertiesInputsInner(
        )
        """

    def testStreamingJobPropertiesInputsInner(self):
        """Test StreamingJobPropertiesInputsInner"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
