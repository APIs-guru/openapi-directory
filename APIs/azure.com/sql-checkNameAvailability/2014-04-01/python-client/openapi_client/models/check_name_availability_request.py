...
class CheckNameAvailabilityRequest(BaseModel):
    ...
    """ # noqa: E501
    name: StrictStr = Field(description="The name whose availability is to be checked.")
    type: StrictStr = Field(description="The type of resource that is used as the scope of the availability check.")
    __properties: ClassVar[List[str]] = ["name", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Microsoft.Sql/servers']):
            raise ValueError("must be one of enum values ('Microsoft.Sql/servers')")
        return value
