# coding: utf-8

"""
    Azure Reservation

    This API describe Azure Reservation

    The version of the OpenAPI document: 2017-11-01
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.reservation_order_response import ReservationOrderResponse

class TestReservationOrderResponse(unittest.TestCase):
    """ReservationOrderResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ReservationOrderResponse:
        """Test ReservationOrderResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ReservationOrderResponse`
        """
        model = ReservationOrderResponse()
        if include_optional:
            return ReservationOrderResponse(
                etag = 56,
                id = '',
                name = '',
                properties = openapi_client.models.reservation_order_properties.ReservationOrderProperties(
                    created_date_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    display_name = '', 
                    expiry_date = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), 
                    original_quantity = 56, 
                    provisioning_state = 'Creating', 
                    request_date_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    reservations = [
                        openapi_client.models.reservation_response.ReservationResponse(
                            etag = 56, 
                            id = '', 
                            kind = 'Microsoft.Compute', 
                            location = 'westus', 
                            name = '', 
                            sku = openapi_client.models.sku_name.SkuName(
                                name = '', ), 
                            type = '', )
                        ], 
                    term = 'P1Y', ),
                type = ''
            )
        else:
            return ReservationOrderResponse(
        )
        """

    def testReservationOrderResponse(self):
        """Test ReservationOrderResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
