# coding: utf-8

"""
    Microsoft Storage Sync

    Microsoft Storage Sync Service API

    The version of the OpenAPI document: 2018-10-01
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.server_endpoint_health import ServerEndpointHealth

class TestServerEndpointHealth(unittest.TestCase):
    """ServerEndpointHealth unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ServerEndpointHealth:
        """Test ServerEndpointHealth
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ServerEndpointHealth`
        """
        model = ServerEndpointHealth()
        if include_optional:
            return ServerEndpointHealth(
                combined_health = 'Healthy',
                current_progress = openapi_client.models.sync_progress_status.SyncProgressStatus(
                    applied_bytes = 0, 
                    applied_item_count = 0, 
                    per_item_error_count = 0, 
                    progress_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    sync_direction = 'none', 
                    total_bytes = 0, 
                    total_item_count = 0, ),
                download_health = 'Healthy',
                download_status = openapi_client.models.sync_session_status.SyncSessionStatus(
                    last_sync_per_item_error_count = 0, 
                    last_sync_result = 56, 
                    last_sync_success_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    last_sync_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), ),
                last_updated_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                offline_data_transfer_status = 'InProgress',
                upload_health = 'Healthy',
                upload_status = openapi_client.models.sync_session_status.SyncSessionStatus(
                    last_sync_per_item_error_count = 0, 
                    last_sync_result = 56, 
                    last_sync_success_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    last_sync_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), )
            )
        else:
            return ServerEndpointHealth(
        )
        """

    def testServerEndpointHealth(self):
        """Test ServerEndpointHealth"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
