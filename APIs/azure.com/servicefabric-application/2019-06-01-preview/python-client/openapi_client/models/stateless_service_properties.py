...
class StatelessServiceProperties(BaseModel):
    ...
    service_package_activation_mode: Optional[StrictStr] = Field(default=None, description="The activation Mode of the service package", alias="servicePackageActivationMode")
    ...
    __properties: ClassVar[List[str]] = ["partitionDescription", "provisioningState", "serviceKind", "servicePackageActivationMode", "serviceTypeName", "correlationScheme", "defaultMoveCost", "placementConstraints", "serviceLoadMetrics", "servicePlacementPolicies"]

    @field_validator('service_package_activation_mode')
    def service_package_activation_mode_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['SharedProcess', 'ExclusiveProcess']):
            raise ValueError("must be one of enum values ('SharedProcess', 'ExclusiveProcess')")
        return value
