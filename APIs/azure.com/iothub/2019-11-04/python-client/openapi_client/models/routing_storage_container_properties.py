...
class RoutingStorageContainerProperties(BaseModel):
    ...
    encoding: Optional[StrictStr] = Field(default=None, description="Encoding that is used to serialize messages to blobs. Supported values are 'avro', 'avrodeflate', and 'JSON'. Default value is 'avro'.")
    ...
    def encoding_validate_enum(cls, value):
        ...
        if value not in set(['Avro', 'AvroDeflate', 'JSON']):
            raise ValueError("must be one of enum values ('Avro', 'AvroDeflate', 'JSON')")
        return value
