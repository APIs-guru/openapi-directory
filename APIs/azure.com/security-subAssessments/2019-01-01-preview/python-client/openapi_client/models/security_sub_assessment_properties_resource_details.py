...
class SecuritySubAssessmentPropertiesResourceDetails(BaseModel):
    """
    Details of the resource that was assessed
    """ # noqa: E501
    source: StrictStr = Field(description="The platform where the assessed resource resides")
    __properties: ClassVar[List[str]] = ["source"]

    @field_validator('source')
    def source_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Azure', 'OnPremise']):
            raise ValueError("must be one of enum values ('Azure', 'OnPremise')")
        return value
