...
class CheckNameAvailabilityResult(BaseModel):
    ...
    name_available: Optional[StrictBool] = Field(default=None, description="Required. True indicates name is valid and available. False indicates the name is invalid, unavailable, or both.", alias="nameAvailable")
    reason: Optional[StrictStr] = Field(default=None, description="Required if nameAvailable == false. Invalid indicates the name provided does not match the resource provider's naming requirements (incorrect length, unsupported characters, etc.) AlreadyExists indicates that the name is already in use and is therefore unavailable.")
