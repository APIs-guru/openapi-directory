...
class PipelineRunQueryOrderBy(BaseModel):
    """
    An object to provide order by options for listing pipeline runs.
    """ # noqa: E501
    order: StrictStr = Field(description="Sorting order of the parameter.")
    order_by: StrictStr = Field(description="Parameter name to be used for order by.", alias="orderBy")
    __properties: ClassVar[List[str]] = ["order", "orderBy"]
    ...
    def order_by_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['RunStart', 'RunEnd']):
            raise ValueError("must be one of enum values ('RunStart', 'RunEnd')")
        return value
