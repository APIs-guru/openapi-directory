# coding: utf-8

"""
    Azure Log Analytics Query Packs

    Azure Log Analytics API reference for Query Packs management.

    The version of the OpenAPI document: 2019-09-01-preview
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.query_packs_resource import QueryPacksResource

class TestQueryPacksResource(unittest.TestCase):
    """QueryPacksResource unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> QueryPacksResource:
        """Test QueryPacksResource
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `QueryPacksResource`
        """
        model = QueryPacksResource()
        if include_optional:
            return QueryPacksResource(
                id = '',
                location = '',
                name = '',
                tags = openapi_client.models.tags.tags(),
                type = ''
            )
        else:
            return QueryPacksResource(
                location = '',
        )
        """

    def testQueryPacksResource(self):
        """Test QueryPacksResource"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
