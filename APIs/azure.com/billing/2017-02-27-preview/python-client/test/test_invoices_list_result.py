# coding: utf-8

"""
    BillingClient

    Billing client provides access to billing resources for Azure Web-Direct subscriptions. Other subscription types which were not purchased directly through the Azure web portal are not supported through this preview API.

    The version of the OpenAPI document: 2017-02-27-preview
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.invoices_list_result import InvoicesListResult

class TestInvoicesListResult(unittest.TestCase):
    """InvoicesListResult unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> InvoicesListResult:
        """Test InvoicesListResult
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `InvoicesListResult`
        """
        model = InvoicesListResult()
        if include_optional:
            return InvoicesListResult(
                next_link = '',
                value = [
                    openapi_client.models.invoice.Invoice(
                        properties = openapi_client.models.invoice_properties.InvoiceProperties(
                            download_url = openapi_client.models.download_url.DownloadUrl(
                                expiry_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                                url = '', ), 
                            invoice_period_end_date = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), 
                            invoice_period_start_date = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), ), )
                    ]
            )
        else:
            return InvoicesListResult(
        )
        """

    def testInvoicesListResult(self):
        """Test InvoicesListResult"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
