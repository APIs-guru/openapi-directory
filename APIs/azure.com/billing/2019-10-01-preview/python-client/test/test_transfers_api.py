# coding: utf-8

"""
    BillingManagementClient

    Billing client provides access to billing resources for Azure subscriptions.

    The version of the OpenAPI document: 2019-10-01-preview
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.transfers_api import TransfersApi


class TestTransfersApi(unittest.TestCase):
    """TransfersApi unit test stubs"""

    def setUp(self) -> None:
        self.api = TransfersApi()

    def tearDown(self) -> None:
        pass

    def test_partner_transfers_cancel(self) -> None:
        """Test case for partner_transfers_cancel

        """
        pass

    def test_partner_transfers_get(self) -> None:
        """Test case for partner_transfers_get

        """
        pass

    def test_partner_transfers_initiate(self) -> None:
        """Test case for partner_transfers_initiate

        """
        pass

    def test_partner_transfers_list(self) -> None:
        """Test case for partner_transfers_list

        """
        pass

    def test_transfers_cancel(self) -> None:
        """Test case for transfers_cancel

        """
        pass

    def test_transfers_get(self) -> None:
        """Test case for transfers_get

        """
        pass

    def test_transfers_initiate(self) -> None:
        """Test case for transfers_initiate

        """
        pass

    def test_transfers_list(self) -> None:
        """Test case for transfers_list

        """
        pass


if __name__ == '__main__':
    unittest.main()
