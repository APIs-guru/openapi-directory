...
class AssessedNetworkAdapter(BaseModel):
    ...
    """ # noqa: E501
    display_name: Optional[StrictStr] = Field(default=None, description="User friendly name of the assessed network adapter.", alias="displayName")
    ...
    suitability_detail: Optional[StrictStr] = Field(default=None, description="If network adapter is not suitable for cloud, this explains the reasons.", alias="suitabilityDetail")
    ...
    __properties: ClassVar[List[str]] = ["displayName", "ipAddresses", "macAddress", "megabytesPerSecondReceived", "megabytesPerSecondTransmitted", "monthlyBandwidthCosts", "netGigabytesTransmittedPerMonth", "suitability", "suitabilityDetail", "suitabilityExplanation"]
    ...
    def suitability_detail_validate_enum(cls, value):
        ...
        if value not in set(['None', 'MegabytesOfDataTransmittedMissing', 'MegabytesOfDataTransmittedOutOfRange']):
            raise ValueError("must be one of enum values ('None', 'MegabytesOfDataTransmittedMissing', 'MegabytesOfDataTransmittedOutOfRange')")
        return value
    ...
    def suitability_explanation_validate_enum(cls, value):
        ...
        if value not in set(['Unknown', 'NotApplicable', 'InternalErrorOccurred']):
            raise ValueError("must be one of enum values ('Unknown', 'NotApplicable', 'InternalErrorOccurred')")
        return value
