# coding: utf-8

"""
    Azure Migrate V2

    Assess your workloads for Azure.

    The version of the OpenAPI document: 2019-10-01
    Contact: azuremigrate@microsoft.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.group_result_list import GroupResultList

class TestGroupResultList(unittest.TestCase):
    """GroupResultList unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> GroupResultList:
        """Test GroupResultList
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GroupResultList`
        """
        model = GroupResultList()
        if include_optional:
            return GroupResultList(
                value = [
                    openapi_client.models.group.Group(
                        e_tag = '', 
                        id = '', 
                        name = '', 
                        properties = openapi_client.models.group_properties.GroupProperties(
                            are_assessments_running = True, 
                            assessments = [
                                ''
                                ], 
                            created_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            group_status = 'Created', 
                            machine_count = 56, 
                            updated_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), ), 
                        type = '', )
                    ]
            )
        else:
            return GroupResultList(
        )
        """

    def testGroupResultList(self):
        """Test GroupResultList"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
