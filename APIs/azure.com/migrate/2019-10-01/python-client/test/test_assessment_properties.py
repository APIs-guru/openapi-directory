# coding: utf-8

"""
    Azure Migrate V2

    Assess your workloads for Azure.

    The version of the OpenAPI document: 2019-10-01
    Contact: azuremigrate@microsoft.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.assessment_properties import AssessmentProperties

class TestAssessmentProperties(unittest.TestCase):
    """AssessmentProperties unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AssessmentProperties:
        """Test AssessmentProperties
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AssessmentProperties`
        """
        model = AssessmentProperties()
        if include_optional:
            return AssessmentProperties(
                azure_disk_type = 'Unknown',
                azure_hybrid_use_benefit = 'Unknown',
                azure_location = 'Unknown',
                azure_offer_code = 'Unknown',
                azure_pricing_tier = 'Standard',
                azure_storage_redundancy = 'Unknown',
                azure_vm_families = [
                    'Unknown'
                    ],
                confidence_rating_in_percentage = 1.337,
                created_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                currency = 'Unknown',
                discount_percentage = 1.337,
                ea_subscription_id = '',
                monthly_bandwidth_cost = 1.337,
                monthly_compute_cost = 1.337,
                monthly_premium_storage_cost = 1.337,
                monthly_standard_ssd_storage_cost = 1.337,
                monthly_storage_cost = 1.337,
                number_of_machines = 56,
                percentile = 'Percentile50',
                perf_data_end_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                perf_data_start_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                prices_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                reserved_instance = 'None',
                scaling_factor = 1.337,
                sizing_criterion = 'PerformanceBased',
                stage = 'InProgress',
                status = 'Created',
                time_range = 'Day',
                updated_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                vm_uptime = openapi_client.models.vm_uptime.VmUptime(
                    days_per_month = 1.337, 
                    hours_per_day = 1.337, )
            )
        else:
            return AssessmentProperties(
                azure_disk_type = 'Unknown',
                azure_hybrid_use_benefit = 'Unknown',
                azure_location = 'Unknown',
                azure_offer_code = 'Unknown',
                azure_pricing_tier = 'Standard',
                azure_storage_redundancy = 'Unknown',
                azure_vm_families = [
                    'Unknown'
                    ],
                currency = 'Unknown',
                discount_percentage = 1.337,
                percentile = 'Percentile50',
                reserved_instance = 'None',
                scaling_factor = 1.337,
                sizing_criterion = 'PerformanceBased',
                stage = 'InProgress',
                time_range = 'Day',
                vm_uptime = openapi_client.models.vm_uptime.VmUptime(
                    days_per_month = 1.337, 
                    hours_per_day = 1.337, ),
        )
        """

    def testAssessmentProperties(self):
        """Test AssessmentProperties"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
