# coding: utf-8

"""
    AppServiceEnvironments API Client

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 2016-09-01
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.usage_collection import UsageCollection

class TestUsageCollection(unittest.TestCase):
    """UsageCollection unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> UsageCollection:
        """Test UsageCollection
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `UsageCollection`
        """
        model = UsageCollection()
        if include_optional:
            return UsageCollection(
                next_link = '',
                value = [
                    openapi_client.models.usage.Usage(
                        properties = openapi_client.models.properties.properties(
                            compute_mode = 'Shared', 
                            current_value = 56, 
                            display_name = '', 
                            limit = 56, 
                            name = '', 
                            next_reset_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            resource_name = '', 
                            site_mode = '', 
                            unit = '', ), )
                    ]
            )
        else:
            return UsageCollection(
                value = [
                    openapi_client.models.usage.Usage(
                        properties = openapi_client.models.properties.properties(
                            compute_mode = 'Shared', 
                            current_value = 56, 
                            display_name = '', 
                            limit = 56, 
                            name = '', 
                            next_reset_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            resource_name = '', 
                            site_mode = '', 
                            unit = '', ), )
                    ],
        )
        """

    def testUsageCollection(self):
        """Test UsageCollection"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
