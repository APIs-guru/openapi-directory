# coding: utf-8

"""
    DataShareManagementClient

    Creates a Microsoft.DataShare management client.

    The version of the OpenAPI document: 2019-11-01
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.consumer_invitation import ConsumerInvitation

class TestConsumerInvitation(unittest.TestCase):
    """ConsumerInvitation unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ConsumerInvitation:
        """Test ConsumerInvitation
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ConsumerInvitation`
        """
        model = ConsumerInvitation()
        if include_optional:
            return ConsumerInvitation(
                properties = openapi_client.models.consumer_invitation_properties.ConsumerInvitationProperties(
                    data_set_count = 56, 
                    description = '', 
                    invitation_id = '', 
                    invitation_status = 'Pending', 
                    location = '', 
                    provider_email = '', 
                    provider_name = '', 
                    provider_tenant_name = '', 
                    responded_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    sent_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    share_name = '', 
                    terms_of_use = '', 
                    user_email = '', 
                    user_name = '', ),
                id = '',
                name = '',
                type = ''
            )
        else:
            return ConsumerInvitation(
                properties = openapi_client.models.consumer_invitation_properties.ConsumerInvitationProperties(
                    data_set_count = 56, 
                    description = '', 
                    invitation_id = '', 
                    invitation_status = 'Pending', 
                    location = '', 
                    provider_email = '', 
                    provider_name = '', 
                    provider_tenant_name = '', 
                    responded_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    sent_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    share_name = '', 
                    terms_of_use = '', 
                    user_email = '', 
                    user_name = '', ),
        )
        """

    def testConsumerInvitation(self):
        """Test ConsumerInvitation"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
