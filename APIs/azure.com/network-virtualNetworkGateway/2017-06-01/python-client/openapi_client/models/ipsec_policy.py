...
class IpsecPolicy(BaseModel):
    ...
    pfs_group: StrictStr = Field(description="The DH Groups used in IKE Phase 2 for new child SA.", alias="pfsGroup")
    sa_data_size_kilobytes: StrictInt = Field(description="The IPSec Security Association (also called Quick Mode or Phase 2 SA) payload size in KB for a site to site VPN tunnel.", alias="saDataSizeKilobytes")
    sa_life_time_seconds: StrictInt = Field(description="The IPSec Security Association (also called Quick Mode or Phase 2 SA) lifetime in seconds for a site to site VPN tunnel.", alias="saLifeTimeSeconds")
    ...
    def ike_encryption_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DES', 'DES3', 'AES128', 'AES192', 'AES256']):
            raise ValueError("must be one of enum values ('DES', 'DES3', 'AES128', 'AES192', 'AES256')")
        return value

    @field_validator('ike_integrity')
    def ike_integrity_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['MD5', 'SHA1', 'SHA256', 'SHA384']):
            raise ValueError("must be one of enum values ('MD5', 'SHA1', 'SHA256', 'SHA384')")
        return value
    ...
    def pfs_group_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['None', 'PFS1', 'PFS2', 'PFS2048', 'ECP256', 'ECP384', 'PFS24']):
            raise ValueError("must be one of enum values ('None', 'PFS1', 'PFS2', 'PFS2048', 'ECP256', 'ECP384', 'PFS24')")
        return value
