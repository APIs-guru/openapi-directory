...
class WebhookPropertiesCreateParameters(BaseModel):
    """
    ...
    service_uri: StrictStr = Field(description="The service URI for the webhook to post notifications.", alias="serviceUri")
    ...
    def actions_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['push', 'delete']):
                raise ValueError("each list item must be one of ('push', 'delete')")
        return value
