...
class SecurityAssessmentMetadataProperties(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["assessmentType", "category", "description", "displayName", "implementationEffort", "policyDefinitionId", "preview", "remediationDescription", "severity", "threats", "userImpact"]

    @field_validator('assessment_type')
    def assessment_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['BuiltIn', 'CustomPolicy', 'CustomerManaged']):
            raise ValueError("must be one of enum values ('BuiltIn', 'CustomPolicy', 'CustomerManaged')")
        return value
