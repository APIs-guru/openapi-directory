...
class NetworkInterfacePropertiesFormat(BaseModel):
    """
    NetworkInterface properties. 
    """ # noqa: E501
    dns_settings: Optional[NetworkInterfaceDnsSettings] = Field(default=None, alias="dnsSettings")
    enable_ip_forwarding: Optional[StrictBool] = Field(default=None, description="Gets or sets whether IPForwarding is enabled on the NIC", alias="enableIPForwarding")
    ip_configurations: Optional[List[NetworkInterfaceIpConfiguration]] = Field(default=None, description="Gets or sets list of IPConfigurations of the NetworkInterface", alias="ipConfigurations")
    mac_address: Optional[StrictStr] = Field(default=None, description="Gets the MAC Address of the network interface", alias="macAddress")
    network_security_group: Optional[SubResource] = Field(default=None, alias="networkSecurityGroup")
    primary: Optional[StrictBool] = Field(default=None, description="Gets whether this is a primary NIC on a virtual machine")
    ...
    resource_guid: Optional[StrictStr] = Field(default=None, description="Gets or sets resource guid property of the network interface resource", alias="resourceGuid")
    virtual_machine: Optional[SubResource] = Field(default=None, alias="virtualMachine")
    __properties: ClassVar[List[str]] = ["dnsSettings", "enableIPForwarding", "ipConfigurations", "macAddress", "networkSecurityGroup", "primary", "provisioningState", "resourceGuid", "virtualMachine"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "dnsSettings": NetworkInterfaceDnsSettings.from_dict(obj["dnsSettings"]) if obj.get("dnsSettings") is not None else None,
            "enableIPForwarding": obj.get("enableIPForwarding"),
            "ipConfigurations": [NetworkInterfaceIpConfiguration.from_dict(_item) for _item in obj["ipConfigurations"]] if obj.get("ipConfigurations") is not None else None,
            ...
            "virtualMachine": SubResource.from_dict(obj["virtualMachine"]) if obj.get("virtualMachine") is not None else None
        })
        return _obj
