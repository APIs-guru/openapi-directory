# coding: utf-8

"""
    CdnManagementClient

    Use these APIs to manage Azure CDN resources through the Azure Resource Manager. You must make sure that requests made to these resources are secure. For more information, see https://msdn.microsoft.com/en-us/library/azure/dn790557.aspx.

    The version of the OpenAPI document: 2015-06-01
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.profile_properties_create_parameters import ProfilePropertiesCreateParameters

class TestProfilePropertiesCreateParameters(unittest.TestCase):
    """ProfilePropertiesCreateParameters unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ProfilePropertiesCreateParameters:
        """Test ProfilePropertiesCreateParameters
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ProfilePropertiesCreateParameters`
        """
        model = ProfilePropertiesCreateParameters()
        if include_optional:
            return ProfilePropertiesCreateParameters(
                sku = openapi_client.models.sku.Sku(
                    name = 'Standard', )
            )
        else:
            return ProfilePropertiesCreateParameters(
                sku = openapi_client.models.sku.Sku(
                    name = 'Standard', ),
        )
        """

    def testProfilePropertiesCreateParameters(self):
        """Test ProfilePropertiesCreateParameters"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
