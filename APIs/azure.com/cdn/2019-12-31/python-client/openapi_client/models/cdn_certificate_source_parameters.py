...
class CdnCertificateSourceParameters(BaseModel):
    """
    Defines the parameters for using CDN managed certificate for securing custom domain.
    """ # noqa: E501
    odata_type: StrictStr = Field(alias="@odata.type")
    certificate_type: StrictStr = Field(description="Type of certificate used", alias="certificateType")
    __properties: ClassVar[List[str]] = ["@odata.type", "certificateType"]

    @field_validator('odata_type')
    def odata_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['#Microsoft.Azure.Cdn.Models.CdnCertificateSourceParameters']):
            raise ValueError("must be one of enum values ('#Microsoft.Azure.Cdn.Models.CdnCertificateSourceParameters')")
        return value
