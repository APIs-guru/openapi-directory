...
class EndpointPropertiesUpdateParameters(BaseModel):
    ...
    geo_filters: Optional[List[GeoFilter]] = Field(default=None, description="List of rules defining user geo access within a CDN endpoint. Each geo filter defines an access rule to a specified path or content, e.g. block APAC for path /pictures/", alias="geoFilters")
    ...
    origin_host_header: Optional[StrictStr] = Field(default=None, description="The host header CDN sends along with content requests to origin. The default value is the host name of the origin.", alias="originHostHeader")
    origin_path: Optional[StrictStr] = Field(default=None, description="The path used when CDN sends request to origin.", alias="originPath")
    query_string_caching_behavior: Optional[QueryStringCachingBehavior] = Field(default=None, alias="queryStringCachingBehavior")
    __properties: ClassVar[List[str]] = ["contentTypesToCompress", "geoFilters", "isCompressionEnabled", "isHttpAllowed", "isHttpsAllowed", "optimizationType", "originHostHeader", "originPath", "queryStringCachingBehavior"]
