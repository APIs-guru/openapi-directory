...
class PoolProperties(BaseModel):
    ...
    Pool properties.
    """ # noqa: E501
    allocation_state: Optional[StrictStr] = Field(default=None, description="Values are:   Steady - The pool is not resizing. There are no changes to the number of nodes in the pool in progress. A pool enters this state when it is created and when no operations are being performed on the pool to change the number of dedicated nodes.  Resizing - The pool is resizing; that is, compute nodes are being added to or removed from the pool.  Stopping - The pool was resizing, but the user has requested that the resize be stopped, but the stop request has not yet been completed.", alias="allocationState")
    ...
    provisioning_state: Optional[StrictStr] = Field(default=None, description="Values are:   Succeeded - The pool is available to run tasks subject to the availability of compute nodes.  Deleting - The user has requested that the pool be deleted, but the delete operation has not yet completed.", alias="provisioningState")
