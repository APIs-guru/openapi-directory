# coding: utf-8

"""
    Azure Alerts Management Service Resource Provider

    Azure Alerts Management Service provides a single pane of glass of alerts across Azure Monitor.

    The version of the OpenAPI document: 2019-05-05-preview
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.default_api import DefaultApi


class TestDefaultApi(unittest.TestCase):
    """DefaultApi unit test stubs"""

    def setUp(self) -> None:
        self.api = DefaultApi()

    def tearDown(self) -> None:
        pass

    def test_action_rules_create_update(self) -> None:
        """Test case for action_rules_create_update

        Create/update an action rule
        """
        pass

    def test_action_rules_delete(self) -> None:
        """Test case for action_rules_delete

        Delete action rule
        """
        pass

    def test_action_rules_get_by_name(self) -> None:
        """Test case for action_rules_get_by_name

        Get action rule by name
        """
        pass

    def test_action_rules_list_by_resource_group(self) -> None:
        """Test case for action_rules_list_by_resource_group

        Get all action rules created in a resource group
        """
        pass

    def test_action_rules_list_by_subscription(self) -> None:
        """Test case for action_rules_list_by_subscription

        Get all action rule in a given subscription
        """
        pass

    def test_action_rules_update(self) -> None:
        """Test case for action_rules_update

        Patch action rule
        """
        pass

    def test_alerts_change_state(self) -> None:
        """Test case for alerts_change_state

        """
        pass

    def test_alerts_get_all(self) -> None:
        """Test case for alerts_get_all

        """
        pass

    def test_alerts_get_by_id(self) -> None:
        """Test case for alerts_get_by_id

        Get a specific alert.
        """
        pass

    def test_alerts_get_history(self) -> None:
        """Test case for alerts_get_history

        """
        pass

    def test_alerts_get_summary(self) -> None:
        """Test case for alerts_get_summary

        """
        pass

    def test_alerts_meta_data(self) -> None:
        """Test case for alerts_meta_data

        """
        pass

    def test_operations_list(self) -> None:
        """Test case for operations_list

        """
        pass

    def test_smart_groups_change_state(self) -> None:
        """Test case for smart_groups_change_state

        """
        pass

    def test_smart_groups_get_all(self) -> None:
        """Test case for smart_groups_get_all

        Get all Smart Groups within a specified subscription
        """
        pass

    def test_smart_groups_get_by_id(self) -> None:
        """Test case for smart_groups_get_by_id

        Get information related to a specific Smart Group.
        """
        pass

    def test_smart_groups_get_history(self) -> None:
        """Test case for smart_groups_get_history

        """
        pass


if __name__ == '__main__':
    unittest.main()
