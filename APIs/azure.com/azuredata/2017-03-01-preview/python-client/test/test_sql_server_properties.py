# coding: utf-8

"""
    AzureDataManagementClient

    The AzureData management API provides a RESTful set of web APIs to manage Azure Data Resources. For example, register, delete and retrieve a SQL Server, SQL Server registration.

    The version of the OpenAPI document: 2017-03-01-preview
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.sql_server_properties import SqlServerProperties

class TestSqlServerProperties(unittest.TestCase):
    """SqlServerProperties unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> SqlServerProperties:
        """Test SqlServerProperties
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SqlServerProperties`
        """
        model = SqlServerProperties()
        if include_optional:
            return SqlServerProperties(
                cores = 56,
                edition = '',
                property_bag = '',
                registration_id = '',
                version = ''
            )
        else:
            return SqlServerProperties(
        )
        """

    def testSqlServerProperties(self):
        """Test SqlServerProperties"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
