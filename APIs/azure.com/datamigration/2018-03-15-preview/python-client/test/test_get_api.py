# coding: utf-8

"""
    Azure Data Migration Service Resource Provider

    The Data Migration Service helps people migrate their data from on-premise database servers to Azure, or from older database software to newer software. The service manages one or more workers that are joined to a customer's virtual network, which is assumed to provide connectivity to their databases. To avoid frequent updates to the resource provider, data migration tasks are implemented by the resource provider in a generic way as task resources, each of which has a task type (which identifies the type of work to run), input, and output. The client is responsible for providing appropriate task type and inputs, which will be passed through unexamined to the machines that implement the functionality, and for understanding the output, which is passed back unexamined to the client.

    The version of the OpenAPI document: 2018-03-15-preview
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.get_api import GETApi


class TestGETApi(unittest.TestCase):
    """GETApi unit test stubs"""

    def setUp(self) -> None:
        self.api = GETApi()

    def tearDown(self) -> None:
        pass

    def test_operations_list_0(self) -> None:
        """Test case for operations_list_0

        Get available resource provider actions (operations)
        """
        pass

    def test_projects_get_1(self) -> None:
        """Test case for projects_get_1

        Get project information
        """
        pass

    def test_projects_list_1(self) -> None:
        """Test case for projects_list_1

        Get projects in a service
        """
        pass

    def test_resource_skus_list_skus_0(self) -> None:
        """Test case for resource_skus_list_skus_0

        Get supported SKUs
        """
        pass

    def test_services_get_1(self) -> None:
        """Test case for services_get_1

        Get DMS Service Instance
        """
        pass

    def test_services_list_1(self) -> None:
        """Test case for services_list_1

        Get services in subscription
        """
        pass

    def test_services_list_by_resource_group_1(self) -> None:
        """Test case for services_list_by_resource_group_1

        Get services in resource group
        """
        pass

    def test_services_list_skus_1(self) -> None:
        """Test case for services_list_skus_1

        Get compatible SKUs
        """
        pass

    def test_tasks_get_1(self) -> None:
        """Test case for tasks_get_1

        Get task information
        """
        pass

    def test_tasks_list_1(self) -> None:
        """Test case for tasks_list_1

        Get tasks in a service
        """
        pass

    def test_usages_list_0(self) -> None:
        """Test case for usages_list_0

        Get resource quotas and usage information
        """
        pass


if __name__ == '__main__':
    unittest.main()
