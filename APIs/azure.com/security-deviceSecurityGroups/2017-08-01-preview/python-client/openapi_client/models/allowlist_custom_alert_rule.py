...
class AllowlistCustomAlertRule(BaseModel):
    """
    A custom alert rule that checks if a value (depends on the custom alert type) is allowed
    """ # noqa: E501
    allowlist_values: List[StrictStr] = Field(description="The values to allow. The format of the values depends on the rule type.", alias="allowlistValues")
    ...
    def rule_type_validate_enum(cls, value):
        ...
        if value not in set(['ConnectionToIpNotAllowed', 'LocalUserNotAllowed', 'ProcessNotAllowed']):
            raise ValueError("must be one of enum values ('ConnectionToIpNotAllowed', 'LocalUserNotAllowed', 'ProcessNotAllowed')")
        return value
