...
class EventHubStreamInputDataSourceProperties(BaseModel):
    """
    The properties that are associated with a Event Hub input containing stream data.
    """ # noqa: E501
    consumer_group_name: Optional[StrictStr] = Field(default=None, description="The name of an Event Hub Consumer Group that should be used to read events from the Event Hub. Specifying distinct consumer group names for multiple inputs allows each of those inputs to receive the same events from the Event Hub. If not specified, the input uses the Event Hub’s default consumer group.", alias="consumerGroupName")
