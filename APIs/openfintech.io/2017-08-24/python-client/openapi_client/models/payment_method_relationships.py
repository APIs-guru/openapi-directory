...
class PaymentMethodRelationships(BaseModel):
    """
    PaymentMethodRelationships
    """ # noqa: E501
    currencies: Optional[PaymentMethodCurrencies] = None
    payment_processor: Optional[PaymentMethodProcessor] = Field(default=None, alias="payment-processor")
    __properties: ClassVar[List[str]] = ["currencies", "payment-processor"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.payment_processor:
            _dict['payment-processor'] = self.payment_processor.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "currencies": PaymentMethodCurrencies.from_dict(obj["currencies"]) if obj.get("currencies") is not None else None,
            "payment-processor": PaymentMethodProcessor.from_dict(obj["payment-processor"]) if obj.get("payment-processor") is not None else None
        })
        return _obj
