...
class VenuesEntityType(BaseModel):
    """
    VenuesEntityType
    """ # noqa: E501
    id: StrictStr = Field(description="String identifying the venue type.")
    label: StrictStr = Field(description="Label of the venue type. This value is translated according to the 'Accept-Language' header.")
    __properties: ClassVar[List[str]] = ["id", "label"]

    @field_validator('id')
    def id_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SAL', 'FES']):
            raise ValueError("must be one of enum values ('SAL', 'FES')")
        return value
