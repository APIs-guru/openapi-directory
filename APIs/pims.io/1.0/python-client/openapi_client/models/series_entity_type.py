...
class SeriesEntityType(BaseModel):
    """
    Type of the series
    """ # noqa: E501
    id: StrictStr = Field(description="String identifying the series type.")
    label: StrictStr = Field(description="Label of the series type. This value is translated according to the 'Accept-Language' header.")
    __properties: ClassVar[List[str]] = ["id", "label"]

    @field_validator('id')
    def id_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['TOU', 'LGS']):
            raise ValueError("must be one of enum values ('TOU', 'LGS')")
        return value
