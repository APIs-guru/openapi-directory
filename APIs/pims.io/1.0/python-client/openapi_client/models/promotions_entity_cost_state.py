...
class PromotionsEntityCostState(BaseModel):
    """
    State of the promotion cost. *This object may be omitted according to the customer configuration.*
    """ # noqa: E501
    id: StrictStr = Field(description="String identifying the state of the promotion cost.")
    label: StrictStr = Field(description="Label of the state of the promotion cost. This value is translated according to the 'Accept-Language' header.")
    __properties: ClassVar[List[str]] = ["id", "label"]

    @field_validator('id')
    def id_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PRE', 'ENG', 'FAC', 'PAY']):
            raise ValueError("must be one of enum values ('PRE', 'ENG', 'FAC', 'PAY')")
        return value
