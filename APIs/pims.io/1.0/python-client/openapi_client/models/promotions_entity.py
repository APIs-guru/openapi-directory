...
class PromotionsEntity(BaseModel):
    """
    PromotionsEntity
    """ # noqa: E501
    applied_to: List[PromotionsEntityAppliedToInner] = Field(description="List of events and/or series where the promotion is applied. A promotion can be applied on several events, and its costs can be split between those events.")
    comments: Optional[StrictStr] = Field(description="Comments on the promotion.")
    cost: PromotionsEntityCost
    end_date: Optional[date] = Field(description="Date the promotion ends. (If null, has the same value as start_date.)")
    file: Optional[StrictStr] = Field(default=None, description="File associated to the promotion.")
    id: StrictInt = Field(description="Unique ID of the promotion.")
    label: StrictStr = Field(description="Label of the promotion.")
    start_date: Optional[date] = Field(description="Date the promotion begins.")
    supplier: Optional[PromotionsEntitySupplier]
    type: PromotionsEntityType
    __properties: ClassVar[List[str]] = ["applied_to", "comments", "cost", "end_date", "file", "id", "label", "start_date", "supplier", "type"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in applied_to (list)
        _items = []
        if self.applied_to:
            for _item_applied_to in self.applied_to:
                if _item_applied_to:
                    _items.append(_item_applied_to.to_dict())
            _dict['applied_to'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "applied_to": [PromotionsEntityAppliedToInner.from_dict(_item) for _item in obj["applied_to"]] if obj.get("applied_to") is not None else None,
            "comments": obj.get("comments"),
            "cost": PromotionsEntityCost.from_dict(obj["cost"]) if obj.get("cost") is not None else None,
            "end_date": obj.get("end_date"),
            "file": obj.get("file"),
            "id": obj.get("id"),
            "label": obj.get("label"),
            "start_date": obj.get("start_date"),
            "supplier": PromotionsEntitySupplier.from_dict(obj["supplier"]) if obj.get("supplier") is not None else None,
            "type": PromotionsEntityType.from_dict(obj["type"]) if obj.get("type") is not None else None
        })
        return _obj
