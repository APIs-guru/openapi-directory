...
class EventsEntityContractType(BaseModel):
    """
    Contract type.
    """ # noqa: E501
    id: StrictStr = Field(description="String identifying the contract type.")
    label: StrictStr = Field(description="Label of the contract type. This value is translated according to the 'Accept-Language' header.")
    __properties: ClassVar[List[str]] = ["id", "label"]

    @field_validator('id')
    def id_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PRO', 'COP', 'PLO', 'COR', 'CES']):
            raise ValueError("must be one of enum values ('PRO', 'COP', 'PLO', 'COR', 'CES')")
        return value
