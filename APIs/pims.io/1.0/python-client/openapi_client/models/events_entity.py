...
class EventsEntity(BaseModel):
    ...
    EventsEntity
    """ # noqa: E501
    break_even: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="Value of the break-even for the event.")
    cancellation_date: Optional[date] = Field(default=None, description="Date the event was canceled.")
    contract: EventsEntityContract
    costing_capacity: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="Costing capacity of the event.")
    creation_timestamp: StrictInt = Field(description="Timestamp for when the venue was created in the customer's database.")
    currency: Optional[StrictStr] = Field(default=None, description="Currency of the prices.")
    datetime: StrictStr = Field(description="Datetime of the event (relative to the timezone of the venue).")
    free: StrictBool = Field(description="Whether this event is free or not.")
    general_sales_date: Optional[date] = Field(default=None, description="Date the event went on general sales (relative to the timezone of the venue).")
    id: StrictInt = Field(description="Unique ID of the event.")
    input_type: EventsEntityInputType
    label: StrictStr = Field(description="Label of the event.")
    last_update_timestamp: StrictInt = Field(description="Timestamp for when the venue was last updated in the customer's database.")
    max_capacity: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="Maximum capacity for the venue in which the event occurs.")
    presales_date: Optional[date] = Field(default=None, description="Date the event went on presales (relative to the timezone of the venue).")
    series_id: Optional[StrictInt] = Field(description="ID of the series the event belongs to.")
    sold_out_date: Optional[date] = Field(default=None, description="Date the event was sold out.")
    venue: VenuesEntity
    __properties: ClassVar[List[str]] = ["break_even", "cancellation_date", "contract", "costing_capacity", "creation_timestamp", "currency", "datetime", "free", "general_sales_date", "id", "input_type", "label", "last_update_timestamp", "max_capacity", "presales_date", "series_id", "sold_out_date", "venue"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.input_type:
            _dict['input_type'] = self.input_type.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "break_even": obj.get("break_even"),
            "cancellation_date": obj.get("cancellation_date"),
            "contract": EventsEntityContract.from_dict(obj["contract"]) if obj.get("contract") is not None else None,
            "costing_capacity": obj.get("costing_capacity"),
            "creation_timestamp": obj.get("creation_timestamp"),
            "currency": obj.get("currency"),
            "datetime": obj.get("datetime"),
            "free": obj.get("free"),
            "general_sales_date": obj.get("general_sales_date"),
            "id": obj.get("id"),
            "input_type": EventsEntityInputType.from_dict(obj["input_type"]) if obj.get("input_type") is not None else None,
            "label": obj.get("label"),
            "last_update_timestamp": obj.get("last_update_timestamp"),
            "max_capacity": obj.get("max_capacity"),
            "presales_date": obj.get("presales_date"),
            "series_id": obj.get("series_id"),
            "sold_out_date": obj.get("sold_out_date"),
            "venue": VenuesEntity.from_dict(obj["venue"]) if obj.get("venue") is not None else None
        })
        return _obj
