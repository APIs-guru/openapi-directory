...
class PromotionsEntityCostType(BaseModel):
    """
    Type of the promotion cost.
    """ # noqa: E501
    id: StrictStr = Field(description="String identifying the type of the promotion cost.")
    label: StrictStr = Field(description="Label of the type of the promotion cost. This value is translated according to the 'Accept-Language' header.")
    __properties: ClassVar[List[str]] = ["id", "label"]

    @field_validator('id')
    def id_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PAY', 'ECH', 'GRA']):
            raise ValueError("must be one of enum values ('PAY', 'ECH', 'GRA')")
        return value
