...
class TicketCountsDetailedEntity(BaseModel):
    """
    TicketCountsDetailedEntity
    """ # noqa: E501
    approved: StrictBool = Field(description="Defines whether this ticket count is approved or not.")
    comment: Optional[StrictStr] = Field(description="Comment for the ticket count.")
    var_date: date = Field(description="Date of the ticket count.", alias="date")
    event_channels: Optional[List[TicketCountsDetailedEntityEventChannelsInner]] = Field(default=None, description="Array of event channels where the sales were made.")
    final: StrictBool = Field(description="Whether this ticket count is the last and final one of its event or not. If it is, it means that no further ticket counts will be added for the event it belongs to.")
    id: StrictInt = Field(description="Unique ID of the ticket count.")
    __properties: ClassVar[List[str]] = ["approved", "comment", "date", "event_channels", "final", "id"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in event_channels (list)
        _items = []
        if self.event_channels:
            for _item_event_channels in self.event_channels:
                if _item_event_channels:
                    _items.append(_item_event_channels.to_dict())
            _dict['event_channels'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "date": obj.get("date"),
            "event_channels": [TicketCountsDetailedEntityEventChannelsInner.from_dict(_item) for _item in obj["event_channels"]] if obj.get("event_channels") is not None else None,
            "final": obj.get("final"),
            "id": obj.get("id")
        })
        return _obj
