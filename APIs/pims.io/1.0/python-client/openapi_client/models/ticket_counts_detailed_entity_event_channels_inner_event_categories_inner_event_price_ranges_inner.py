...
class TicketCountsDetailedEntityEventChannelsInnerEventCategoriesInnerEventPriceRangesInner(BaseModel):
    """
    TicketCountsDetailedEntityEventChannelsInnerEventCategoriesInnerEventPriceRangesInner
    """ # noqa: E501
    currency: StrictStr = Field(description="Currency of the gross (<a href='https://en.wikipedia.org/wiki/ISO_4217#Active_codes'>ISO 4212 alphabetic code</a>).")
    gross: Optional[Union[Annotated[float, Field(strict=True, ge=0)], Annotated[int, Field(strict=True, ge=0)]]] = Field(description="Gross (= income for the sold tickets for the current channel/category/price range, including VAT but excluding all commissions).")
    id: StrictInt = Field(description="ID of the event price range.")
    reservations: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="Detailed ticket reservations (= number of reserved tickets for the current channel/category/price range). *This field may be omitted according to the customer configuration.*")
    sales: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(description="Detailed ticket sales (= number of sold tickets for the current channel/category/price range).")
    __properties: ClassVar[List[str]] = ["currency", "gross", "id", "reservations", "sales"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if gross (nullable) is None
        # and model_fields_set contains the field
        if self.gross is None and "gross" in self.model_fields_set:
            _dict['gross'] = None

        # set to None if reservations (nullable) is None
        # and model_fields_set contains the field
        if self.reservations is None and "reservations" in self.model_fields_set:
            _dict['reservations'] = None
