...
class SeriesEntity(BaseModel):
    ...
    SeriesEntity
    """ # noqa: E501
    contract: Optional[SeriesEntityContract] = None
    costing_capacity: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="Value of the costing capacity.")
    creation_timestamp: StrictInt = Field(description="Timestamp for when the series was created in the customer's database.")
    first_date: date = Field(description="Date of the first event in the series.")
    id: StrictInt = Field(description="Unique ID of the series.")
    label: StrictStr = Field(description="Label of the series.")
    last_date: date = Field(description="Date of the last event in the series.")
    last_update_timestamp: StrictInt = Field(description="Timestamp for when the series was last updated in the customer's database.")
    type: SeriesEntityType
    venue: Optional[VenuesEntity] = None
    __properties: ClassVar[List[str]] = ["contract", "costing_capacity", "creation_timestamp", "first_date", "id", "label", "last_date", "last_update_timestamp", "type", "venue"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if costing_capacity (nullable) is None
        # and model_fields_set contains the field
        if self.costing_capacity is None and "costing_capacity" in self.model_fields_set:
            _dict['costing_capacity'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "contract": SeriesEntityContract.from_dict(obj["contract"]) if obj.get("contract") is not None else None,
            ...
            "type": SeriesEntityType.from_dict(obj["type"]) if obj.get("type") is not None else None,
