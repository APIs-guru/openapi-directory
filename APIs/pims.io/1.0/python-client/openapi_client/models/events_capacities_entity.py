...
class EventsCapacitiesEntity(BaseModel):
    """
    EventsCapacitiesEntity
    """ # noqa: E501
    var_date: date = Field(description="Date from which the capacity is active.", alias="date")
    event_categories: Optional[List[EventsCapacitiesEntityEventCategoriesInner]] = Field(default=None, description="Array of categories with their detailed capacities.")
    id: StrictInt = Field(description="Unique ID of the event capacity.")
    __properties: ClassVar[List[str]] = ["date", "event_categories", "id"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in event_categories (list)
        _items = []
        if self.event_categories:
            for _item_event_categories in self.event_categories:
                if _item_event_categories:
                    _items.append(_item_event_categories.to_dict())
            _dict['event_categories'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "date": obj.get("date"),
            "event_categories": [EventsCapacitiesEntityEventCategoriesInner.from_dict(_item) for _item in obj["event_categories"]] if obj.get("event_categories") is not None else None,
            "id": obj.get("id")
        })
        return _obj
