...
class EventsCategoriesEntity(BaseModel):
    """
    EventsCategoriesEntity
    """ # noqa: E501
    category: CategoriesEntity
    event_price_ranges: Optional[List[EventsCategoriesEntityEventPriceRangesInner]] = Field(default=None, description="Array of event price ranges.")
    id: StrictInt = Field(description="Unique ID of the event category.")
    ignored: StrictBool = Field(description="Defines whether this event category is ignored or not.")
    __properties: ClassVar[List[str]] = ["category", "event_price_ranges", "id", "ignored"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in event_price_ranges (list)
        _items = []
        if self.event_price_ranges:
            for _item_event_price_ranges in self.event_price_ranges:
                if _item_event_price_ranges:
                    _items.append(_item_event_price_ranges.to_dict())
            _dict['event_price_ranges'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "category": CategoriesEntity.from_dict(obj["category"]) if obj.get("category") is not None else None,
            "event_price_ranges": [EventsCategoriesEntityEventPriceRangesInner.from_dict(_item) for _item in obj["event_price_ranges"]] if obj.get("event_price_ranges") is not None else None,
            "id": obj.get("id"),
