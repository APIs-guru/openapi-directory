...
class EventsCapacitiesEntityEventCategoriesInner(BaseModel):
    """
    EventsCapacitiesEntityEventCategoriesInner
    """ # noqa: E501
    comps: Optional[StrictInt] = Field(description="Number of comps in the category.")
    holds: Optional[StrictInt] = Field(description="Number of *holds* in the category. <span class=\"definition\">Holds</span> are seats/places that are not in sale at the date of the capacity, but will eventually be later.")
    id: StrictInt = Field(description="Unique ID of the event category.")
    kills: Optional[StrictInt] = Field(description="Number of *kills* in the category. <span class=\"definition\">Kills</span> are seats/places that will not be sold for technical reasons.")
    sellable_capacity: StrictInt = Field(description="Number of sellable seats/places in the category. This is calculated by the formula: `total_capacity - kills - comps - holds`.")
    total_capacity: StrictInt = Field(description="Total number of seats/places in the category.")
    __properties: ClassVar[List[str]] = ["comps", "holds", "id", "kills", "sellable_capacity", "total_capacity"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if comps (nullable) is None
        # and model_fields_set contains the field
        if self.comps is None and "comps" in self.model_fields_set:
            _dict['comps'] = None

        # set to None if holds (nullable) is None
        # and model_fields_set contains the field
        if self.holds is None and "holds" in self.model_fields_set:
            _dict['holds'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "id": obj.get("id"),
            "kills": obj.get("kills"),
            "sellable_capacity": obj.get("sellable_capacity"),
