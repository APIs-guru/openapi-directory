...
class EventsEntityInputType(BaseModel):
    """
    Input type of the event.
    """ # noqa: E501
    id: StrictStr = Field(description="String identifying the input type of the event.")
    label: StrictStr = Field(description="Label of the input type. This value is translated according to the 'Accept-Language' header.")
    __properties: ClassVar[List[str]] = ["id", "label"]

    @field_validator('id')
    def id_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DET', 'GLO']):
            raise ValueError("must be one of enum values ('DET', 'GLO')")
        return value
