...
class EventsCategoriesEntityEventPriceRangesInner(BaseModel):
    """
    EventsCategoriesEntityEventPriceRangesInner
    """ # noqa: E501
    base_price: Union[Annotated[float, Field(strict=True, ge=0)], Annotated[int, Field(strict=True, ge=0)]] = Field(description="Base price of the event price range (i.e. including VAT but excluding all commissions).")
    currency: StrictStr = Field(description="Currency of the prices.")
    id: StrictInt = Field(description="ID of the event price range.")
    ignored: Optional[StrictBool] = Field(default=None, description="Defines whether this event price range is ignored or not.")
    price_range: PriceRangesEntity
    public_price: Optional[Union[Annotated[float, Field(strict=True, ge=0)], Annotated[int, Field(strict=True, ge=0)]]] = Field(description="Public price of the event price range (i.e. including VAT and all commissions).")
    __properties: ClassVar[List[str]] = ["base_price", "currency", "id", "ignored", "price_range", "public_price"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if public_price (nullable) is None
        # and model_fields_set contains the field
        if self.public_price is None and "public_price" in self.model_fields_set:
            _dict['public_price'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "price_range": PriceRangesEntity.from_dict(obj["price_range"]) if obj.get("price_range") is not None else None,
            "public_price": obj.get("public_price")
        })
        return _obj
