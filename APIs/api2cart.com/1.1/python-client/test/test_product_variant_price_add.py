# coding: utf-8

"""
    Swagger API2Cart

    API2Cart

    The version of the OpenAPI document: 1.1
    Contact: contact@api2cart.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.product_variant_price_add import ProductVariantPriceAdd

class TestProductVariantPriceAdd(unittest.TestCase):
    """ProductVariantPriceAdd unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ProductVariantPriceAdd:
        """Test ProductVariantPriceAdd
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ProductVariantPriceAdd`
        """
        model = ProductVariantPriceAdd()
        if include_optional:
            return ProductVariantPriceAdd(
                group_prices = [
                    openapi_client.models.product_add_group_prices_inner.ProductAdd_group_prices_inner(
                        group_id = '', 
                        price = 1.337, )
                    ],
                id = '',
                product_id = ''
            )
        else:
            return ProductVariantPriceAdd(
                group_prices = [
                    openapi_client.models.product_add_group_prices_inner.ProductAdd_group_prices_inner(
                        group_id = '', 
                        price = 1.337, )
                    ],
        )
        """

    def testProductVariantPriceAdd(self):
        """Test ProductVariantPriceAdd"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
