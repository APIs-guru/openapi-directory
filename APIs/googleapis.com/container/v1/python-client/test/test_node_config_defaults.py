# coding: utf-8

"""
    Kubernetes Engine API

    Builds and manages container-based applications, powered by the open source Kubernetes technology.

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.node_config_defaults import NodeConfigDefaults

class TestNodeConfigDefaults(unittest.TestCase):
    """NodeConfigDefaults unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> NodeConfigDefaults:
        """Test NodeConfigDefaults
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `NodeConfigDefaults`
        """
        model = NodeConfigDefaults()
        if include_optional:
            return NodeConfigDefaults(
                gcfs_config = openapi_client.models.gcfs_config.GcfsConfig(
                    enabled = True, ),
                logging_config = openapi_client.models.node_pool_logging_config.NodePoolLoggingConfig(
                    variant_config = openapi_client.models.logging_variant_config.LoggingVariantConfig(
                        variant = 'VARIANT_UNSPECIFIED', ), )
            )
        else:
            return NodeConfigDefaults(
        )
        """

    def testNodeConfigDefaults(self):
        """Test NodeConfigDefaults"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
