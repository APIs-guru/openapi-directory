...
class SasPortalAssignment(BaseModel):
    ...
    """ # noqa: E501
    members: Optional[List[StrictStr]] = Field(default=None, description="The identities the role is assigned to. It can have the following values: * `{user_email}`: An email address that represents a specific Google account. For example: `alice@gmail.com`. * `{group_email}`: An email address that represents a Google group. For example, `viewers@gmail.com`.")
    role: Optional[StrictStr] = Field(default=None, description="Required. Role that is assigned to `members`.")
