...
class EncryptionInfo(BaseModel):
    ...
    encryption_type: Optional[StrictStr] = Field(default=None, description="Output only. The type of encryption used to protect this resource.", alias="encryptionType")
    kms_key_version: Optional[StrictStr] = Field(default=None, description="Output only. The version of the Cloud KMS key specified in the parent cluster that is in use for the data underlying this table.", alias="kmsKeyVersion")
    ...
    def encryption_type_validate_enum(cls, value):
        ...
        if value not in set(['ENCRYPTION_TYPE_UNSPECIFIED', 'GOOGLE_DEFAULT_ENCRYPTION', 'CUSTOMER_MANAGED_ENCRYPTION']):
            raise ValueError("must be one of enum values ('ENCRYPTION_TYPE_UNSPECIFIED', 'GOOGLE_DEFAULT_ENCRYPTION', 'CUSTOMER_MANAGED_ENCRYPTION')")
        return value
