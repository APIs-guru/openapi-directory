...
class GooglePrivacyDlpV2DataRiskLevel(BaseModel):
    ...
    """ # noqa: E501
    score: Optional[StrictStr] = Field(default=None, description="The score applied to the resource.")
    __properties: ClassVar[List[str]] = ["score"]

    @field_validator('score')
    def score_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['RISK_SCORE_UNSPECIFIED', 'RISK_LOW', 'RISK_MODERATE', 'RISK_HIGH']):
            raise ValueError("must be one of enum values ('RISK_SCORE_UNSPECIFIED', 'RISK_LOW', 'RISK_MODERATE', 'RISK_HIGH')")
        return value
