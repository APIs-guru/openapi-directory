...
class MailCountResult(BaseModel):
    ...
    """ # noqa: E501
    account_count_errors: Optional[List[AccountCountError]] = Field(default=None, description="Errors occurred when querying these accounts.", alias="accountCountErrors")
    ...
    non_queryable_accounts: Optional[List[StrictStr]] = Field(default=None, description="When **DataScope** is **HELD_DATA** and when account emails are passed in explicitly, the list of accounts in the request that are not queried because they are not on hold in the matter. For other data scopes, this field is not set.", alias="nonQueryableAccounts")
