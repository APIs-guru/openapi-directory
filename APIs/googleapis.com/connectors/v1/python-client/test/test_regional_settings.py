# coding: utf-8

"""
    Connectors API

    Enables users to create and manage connections to Google Cloud services and third-party business applications using the Connectors interface.

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.regional_settings import RegionalSettings

class TestRegionalSettings(unittest.TestCase):
    """RegionalSettings unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> RegionalSettings:
        """Test RegionalSettings
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `RegionalSettings`
        """
        model = RegionalSettings()
        if include_optional:
            return RegionalSettings(
                encryption_config = openapi_client.models.encryption_config.EncryptionConfig(
                    encryption_type = 'ENCRYPTION_TYPE_UNSPECIFIED', 
                    kms_key_name = '', ),
                name = '',
                network_config = openapi_client.models.network_config.NetworkConfig(
                    egress_ips = [
                        ''
                        ], 
                    egress_mode = 'NETWORK_EGRESS_MODE_UNSPECIFIED', ),
                provisioned = True
            )
        else:
            return RegionalSettings(
        )
        """

    def testRegionalSettings(self):
        """Test RegionalSettings"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
