# coding: utf-8

"""
    Cloud Monitoring API

    Manages your Cloud Monitoring data and configurations.

    The version of the OpenAPI document: v3
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.metric_range import MetricRange

class TestMetricRange(unittest.TestCase):
    """MetricRange unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> MetricRange:
        """Test MetricRange
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `MetricRange`
        """
        model = MetricRange()
        if include_optional:
            return MetricRange(
                range = openapi_client.models.google_monitoring_v3_range.GoogleMonitoringV3Range(
                    max = 1.337, 
                    min = 1.337, ),
                time_series = ''
            )
        else:
            return MetricRange(
        )
        """

    def testMetricRange(self):
        """Test MetricRange"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
