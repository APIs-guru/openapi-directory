...
class GuestConfigDetails(BaseModel):
    ...
    selinux_mode: Optional[StrictStr] = Field(default=None, description="Security-Enhanced Linux (SELinux) mode.", alias="selinuxMode")
    __properties: ClassVar[List[str]] = ["fstab", "hosts", "issue", "nfsExports", "selinuxMode"]

    @field_validator('selinux_mode')
    def selinux_mode_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['SE_LINUX_MODE_UNSPECIFIED', 'SE_LINUX_MODE_DISABLED', 'SE_LINUX_MODE_PERMISSIVE', 'SE_LINUX_MODE_ENFORCING']):
            raise ValueError("must be one of enum values ('SE_LINUX_MODE_UNSPECIFIED', 'SE_LINUX_MODE_DISABLED', 'SE_LINUX_MODE_PERMISSIVE', 'SE_LINUX_MODE_ENFORCING')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "selinuxMode": obj.get("selinuxMode")
        })
        return _obj
