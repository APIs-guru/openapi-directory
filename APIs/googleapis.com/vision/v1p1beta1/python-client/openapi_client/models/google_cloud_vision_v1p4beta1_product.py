...
class GoogleCloudVisionV1p4beta1Product(BaseModel):
    ...
    product_labels: Optional[List[GoogleCloudVisionV1p4beta1ProductKeyValue]] = Field(default=None, description="Key-value pairs that can be attached to a product. At query time, constraints can be specified based on the product_labels. Note that integer values can be provided as strings, e.g. \"1199\". Only strings with integer values can match a range-based restriction which is to be supported soon. Multiple values can be assigned to the same key. One product may have up to 500 product_labels. Notice that the total number of distinct product_labels over all products in one ProductSet cannot exceed 1M, otherwise the product search pipeline will refuse to work for that ProductSet.", alias="productLabels")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "productLabels": [GoogleCloudVisionV1p4beta1ProductKeyValue.from_dict(_item) for _item in obj["productLabels"]] if obj.get("productLabels") is not None else None
        })
        return _obj
