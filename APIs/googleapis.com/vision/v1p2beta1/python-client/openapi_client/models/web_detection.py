...
class WebDetection(BaseModel):
    ...
    """ # noqa: E501
    best_guess_labels: Optional[List[WebLabel]] = Field(default=None, description="The service's best guess as to the topic of the request image. Inferred from similar images on the open web.", alias="bestGuessLabels")
    full_matching_images: Optional[List[WebImage]] = Field(default=None, description="Fully matching images from the Internet. Can include resized copies of the query image.", alias="fullMatchingImages")
    pages_with_matching_images: Optional[List[WebPage]] = Field(default=None, description="Web pages containing the matching images from the Internet.", alias="pagesWithMatchingImages")
    partial_matching_images: Optional[List[WebImage]] = Field(default=None, description="Partial matching images from the Internet. Those images are similar enough to share some key-point features. For example an original image will likely have partial matching for its crops.", alias="partialMatchingImages")
    visually_similar_images: Optional[List[WebImage]] = Field(default=None, description="The visually similar image results.", alias="visuallySimilarImages")
    web_entities: Optional[List[WebEntity]] = Field(default=None, description="Deduced entities from similar images on the Internet.", alias="webEntities")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "bestGuessLabels": [WebLabel.from_dict(_item) for _item in obj["bestGuessLabels"]] if obj.get("bestGuessLabels") is not None else None,
            "fullMatchingImages": [WebImage.from_dict(_item) for _item in obj["fullMatchingImages"]] if obj.get("fullMatchingImages") is not None else None,
            "pagesWithMatchingImages": [WebPage.from_dict(_item) for _item in obj["pagesWithMatchingImages"]] if obj.get("pagesWithMatchingImages") is not None else None,
            "partialMatchingImages": [WebImage.from_dict(_item) for _item in obj["partialMatchingImages"]] if obj.get("partialMatchingImages") is not None else None,
            "visuallySimilarImages": [WebImage.from_dict(_item) for _item in obj["visuallySimilarImages"]] if obj.get("visuallySimilarImages") is not None else None,
            "webEntities": [WebEntity.from_dict(_item) for _item in obj["webEntities"]] if obj.get("webEntities") is not None else None
        })
        return _obj
