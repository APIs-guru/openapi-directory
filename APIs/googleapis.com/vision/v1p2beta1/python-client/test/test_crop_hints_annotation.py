# coding: utf-8

"""
    Cloud Vision API

    Integrates Google Vision features, including image labeling, face, logo, and landmark detection, optical character recognition (OCR), and detection of explicit content, into applications.

    The version of the OpenAPI document: v1p2beta1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.crop_hints_annotation import CropHintsAnnotation

class TestCropHintsAnnotation(unittest.TestCase):
    """CropHintsAnnotation unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CropHintsAnnotation:
        """Test CropHintsAnnotation
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CropHintsAnnotation`
        """
        model = CropHintsAnnotation()
        if include_optional:
            return CropHintsAnnotation(
                crop_hints = [
                    openapi_client.models.crop_hint.CropHint(
                        bounding_poly = openapi_client.models.bounding_poly.BoundingPoly(
                            normalized_vertices = [
                                openapi_client.models.normalized_vertex.NormalizedVertex(
                                    x = 1.337, 
                                    y = 1.337, )
                                ], 
                            vertices = [
                                openapi_client.models.vertex.Vertex(
                                    x = 56, 
                                    y = 56, )
                                ], ), 
                        confidence = 1.337, 
                        importance_fraction = 1.337, )
                    ]
            )
        else:
            return CropHintsAnnotation(
        )
        """

    def testCropHintsAnnotation(self):
        """Test CropHintsAnnotation"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
