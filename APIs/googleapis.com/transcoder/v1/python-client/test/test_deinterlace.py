# coding: utf-8

"""
    Transcoder API

    This API converts video files into formats suitable for consumer distribution. For more information, see the Transcoder API overview. 

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.deinterlace import Deinterlace

class TestDeinterlace(unittest.TestCase):
    """Deinterlace unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> Deinterlace:
        """Test Deinterlace
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `Deinterlace`
        """
        model = Deinterlace()
        if include_optional:
            return Deinterlace(
                bwdif = openapi_client.models.bwdif_config.BwdifConfig(
                    deinterlace_all_frames = True, 
                    mode = '', 
                    parity = '', ),
                yadif = openapi_client.models.yadif_config.YadifConfig(
                    deinterlace_all_frames = True, 
                    disable_spatial_interlacing = True, 
                    mode = '', 
                    parity = '', )
            )
        else:
            return Deinterlace(
        )
        """

    def testDeinterlace(self):
        """Test Deinterlace"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
