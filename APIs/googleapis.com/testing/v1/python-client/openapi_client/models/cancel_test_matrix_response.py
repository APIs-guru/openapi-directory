...
class CancelTestMatrixResponse(BaseModel):
    ...
    """ # noqa: E501
    test_state: Optional[StrictStr] = Field(default=None, description="The current rolled-up state of the test matrix. If this state is already final, then the cancelation request will have no effect.", alias="testState")
    __properties: ClassVar[List[str]] = ["testState"]

    @field_validator('test_state')
    def test_state_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
