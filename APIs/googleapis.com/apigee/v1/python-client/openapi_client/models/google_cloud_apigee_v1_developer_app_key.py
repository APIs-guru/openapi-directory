...
class GoogleCloudApigeeV1DeveloperAppKey(BaseModel):
    ...
    """ # noqa: E501
    api_products: Optional[List[Any]] = Field(default=None, description="List of API products for which the credential can be used. **Note**: Do not specify the list of API products when creating a consumer key and secret for a developer app. Instead, use the UpdateDeveloperAppKey API to make the association after the consumer key and secret are created.", alias="apiProducts")
    ...
    expires_at: Optional[StrictStr] = Field(default=None, description="Time the developer app expires in milliseconds since epoch.", alias="expiresAt")
    expires_in_seconds: Optional[StrictStr] = Field(default=None, description="Input only. Expiration time, in seconds, for the consumer key. If not set or left to the default value of `-1`, the API key never expires. The expiration time can't be updated after it is set.", alias="expiresInSeconds")
    issued_at: Optional[StrictStr] = Field(default=None, description="Time the developer app was created in milliseconds since epoch.", alias="issuedAt")
