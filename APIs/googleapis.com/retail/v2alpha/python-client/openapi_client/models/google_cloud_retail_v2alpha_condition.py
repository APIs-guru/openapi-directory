...
class GoogleCloudRetailV2alphaCondition(BaseModel):
    """
    Metadata that is used to define a condition that triggers an action. A valid condition must specify at least one of 'query_terms' or 'products_filter'. If multiple fields are specified, the condition is met if all the fields are satisfied e.g. if a set of query terms and product_filter are set, then only items matching the product_filter for requests with a query matching the query terms wil get boosted.
    """ # noqa: E501
    active_time_range: Optional[List[GoogleCloudRetailV2alphaConditionTimeRange]] = Field(default=None, description="Range of time(s) specifying when Condition is active. Condition true if any time range matches.", alias="activeTimeRange")
    ...
    query_terms: Optional[List[GoogleCloudRetailV2alphaConditionQueryTerm]] = Field(default=None, description="A list (up to 10 entries) of terms to match the query on. If not specified, match all queries. If many query terms are specified, the condition is matched if any of the terms is a match (i.e. using the OR operator).", alias="queryTerms")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "activeTimeRange": [GoogleCloudRetailV2alphaConditionTimeRange.from_dict(_item) for _item in obj["activeTimeRange"]] if obj.get("activeTimeRange") is not None else None,
            "pageCategories": obj.get("pageCategories"),
            "queryTerms": [GoogleCloudRetailV2alphaConditionQueryTerm.from_dict(_item) for _item in obj["queryTerms"]] if obj.get("queryTerms") is not None else None
        })
        return _obj
