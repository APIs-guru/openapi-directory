...
class GoogleCloudDialogflowV2SmartReplyMetricsTopNMetrics(BaseModel):
    """
    Evaluation metrics when retrieving `n` smart replies with the model.
    """ # noqa: E501
    n: Optional[StrictInt] = Field(default=None, description="Number of retrieved smart replies. For example, when `n` is 3, this evaluation contains metrics for when Dialogflow retrieves 3 smart replies with the model.")
    recall: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Defined as `number of queries whose top n smart replies have at least one similar (token match similarity above the defined threshold) reply as the real reply` divided by `number of queries with at least one smart reply`. Value ranges from 0.0 to 1.0 inclusive.")
    __properties: ClassVar[List[str]] = ["n", "recall"]
