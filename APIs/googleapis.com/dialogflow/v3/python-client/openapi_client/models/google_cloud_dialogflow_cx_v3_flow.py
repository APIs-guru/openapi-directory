...
class GoogleCloudDialogflowCxV3Flow(BaseModel):
    """
    ...
    event_handlers: Optional[List[GoogleCloudDialogflowCxV3EventHandler]] = Field(default=None, description="A flow's event handlers serve two purposes: * They are responsible for handling events (e.g. no match, webhook errors) in the flow. * They are inherited by every page's event handlers, which can be used to handle common events regardless of the current page. Event handlers defined in the page have higher priority than those defined in the flow. Unlike transition_routes, these handlers are evaluated on a first-match basis. The first one that matches the event get executed, with the rest being ignored.", alias="eventHandlers")
    ...
    nlu_settings: Optional[GoogleCloudDialogflowCxV3NluSettings] = Field(default=None, alias="nluSettings")
    ...
    transition_routes: Optional[List[GoogleCloudDialogflowCxV3TransitionRoute]] = Field(default=None, description="A flow's transition routes serve two purposes: * They are responsible for matching the user's first utterances in the flow. * They are inherited by every page's transition routes and can support use cases such as the user saying \"help\" or \"can I talk to a human?\", which can be handled in a common way regardless of the current page. Transition routes defined in the page have higher priority than those defined in the flow. TransitionRoutes are evalauted in the following order: * TransitionRoutes with intent specified. * TransitionRoutes with only condition specified. TransitionRoutes with intent specified are inherited by pages in the flow.", alias="transitionRoutes")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "name": obj.get("name"),
            "nluSettings": GoogleCloudDialogflowCxV3NluSettings.from_dict(obj["nluSettings"]) if obj.get("nluSettings") is not None else None,
