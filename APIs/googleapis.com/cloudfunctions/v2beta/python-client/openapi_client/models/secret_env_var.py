...
class SecretEnvVar(BaseModel):
    ...
    project_id: Optional[StrictStr] = Field(default=None, description="Project identifier (preferably project number but can also be the project ID) of the project that contains the secret. If not set, it is assumed that the secret is in the same project as the function.", alias="projectId")
