...
class ListStepAccessibilityClustersResponse(BaseModel):
    ...
    """ # noqa: E501
    clusters: Optional[List[SuggestionClusterProto]] = Field(default=None, description="A sequence of accessibility suggestions, grouped into clusters. Within the sequence, clusters that belong to the same SuggestionCategory should be adjacent. Within each category, clusters should be ordered by their SuggestionPriority (ERRORs first). The categories should be ordered by their highest priority cluster.")
    name: Optional[StrictStr] = Field(default=None, description="A full resource name of the step. For example, projects/my-project/histories/bh.1234567890abcdef/executions/ 1234567890123456789/steps/bs.1234567890abcdef Always presents.")
    __properties: ClassVar[List[str]] = ["clusters", "name"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "clusters": [SuggestionClusterProto.from_dict(_item) for _item in obj["clusters"]] if obj.get("clusters") is not None else None,
            "name": obj.get("name")
        })
        return _obj
