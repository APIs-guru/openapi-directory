# coding: utf-8

"""
    Cloud Tool Results API

    API to publish and access results from developer tools.

    The version of the OpenAPI document: v1beta3
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.test_case import TestCase

class TestTestCase(unittest.TestCase):
    """TestCase unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> TestCase:
        """Test TestCase
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `TestCase`
        """
        model = TestCase()
        if include_optional:
            return TestCase(
                elapsed_time = openapi_client.models.duration.Duration(
                    nanos = 56, 
                    seconds = '', ),
                end_time = openapi_client.models.timestamp.Timestamp(
                    nanos = 56, 
                    seconds = '', ),
                skipped_message = '',
                stack_traces = [
                    openapi_client.models.stack_trace.StackTrace(
                        exception = '', )
                    ],
                start_time = openapi_client.models.timestamp.Timestamp(
                    nanos = 56, 
                    seconds = '', ),
                status = 'passed',
                test_case_id = '',
                test_case_reference = openapi_client.models.test_case_reference.TestCaseReference(
                    class_name = '', 
                    name = '', 
                    test_suite_name = '', ),
                tool_outputs = [
                    openapi_client.models.tool_output_reference.ToolOutputReference(
                        creation_time = openapi_client.models.timestamp.Timestamp(
                            nanos = 56, 
                            seconds = '', ), 
                        output = openapi_client.models.file_reference.FileReference(
                            file_uri = '', ), 
                        test_case = openapi_client.models.test_case_reference.TestCaseReference(
                            class_name = '', 
                            name = '', 
                            test_suite_name = '', ), )
                    ]
            )
        else:
            return TestCase(
        )
        """

    def testTestCase(self):
        """Test TestCase"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
