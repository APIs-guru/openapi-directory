# coding: utf-8

"""
    Cloud Tool Results API

    API to publish and access results from developer tools.

    The version of the OpenAPI document: v1beta3
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.app_start_time import AppStartTime

class TestAppStartTime(unittest.TestCase):
    """AppStartTime unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AppStartTime:
        """Test AppStartTime
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AppStartTime`
        """
        model = AppStartTime()
        if include_optional:
            return AppStartTime(
                fully_drawn_time = openapi_client.models.duration.Duration(
                    nanos = 56, 
                    seconds = '', ),
                initial_display_time = openapi_client.models.duration.Duration(
                    nanos = 56, 
                    seconds = '', )
            )
        else:
            return AppStartTime(
        )
        """

    def testAppStartTime(self):
        """Test AppStartTime"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
