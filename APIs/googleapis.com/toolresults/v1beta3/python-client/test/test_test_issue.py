# coding: utf-8

"""
    Cloud Tool Results API

    API to publish and access results from developer tools.

    The version of the OpenAPI document: v1beta3
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.test_issue import TestIssue

class TestTestIssue(unittest.TestCase):
    """TestIssue unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> TestIssue:
        """Test TestIssue
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `TestIssue`
        """
        model = TestIssue()
        if include_optional:
            return TestIssue(
                category = 'unspecifiedCategory',
                error_message = '',
                severity = 'unspecifiedSeverity',
                stack_trace = openapi_client.models.stack_trace.StackTrace(
                    exception = '', ),
                type = 'unspecifiedType',
                warning = openapi_client.models.any.Any(
                    type_url = '', 
                    value = 'YQ==', )
            )
        else:
            return TestIssue(
        )
        """

    def testTestIssue(self):
        """Test TestIssue"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
