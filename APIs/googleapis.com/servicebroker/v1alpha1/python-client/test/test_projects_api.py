# coding: utf-8

"""
    Service Broker

    The Google Cloud Platform Service Broker API provides Google hosted implementation of the Open Service Broker API (https://www.openservicebrokerapi.org/). 

    The version of the OpenAPI document: v1alpha1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.projects_api import ProjectsApi


class TestProjectsApi(unittest.TestCase):
    """ProjectsApi unit test stubs"""

    def setUp(self) -> None:
        self.api = ProjectsApi()

    def tearDown(self) -> None:
        pass

    def test_servicebroker_projects_brokers_instances_service_bindings_list(self) -> None:
        """Test case for servicebroker_projects_brokers_instances_service_bindings_list

        """
        pass

    def test_servicebroker_projects_brokers_service_instances_list(self) -> None:
        """Test case for servicebroker_projects_brokers_service_instances_list

        """
        pass

    def test_servicebroker_projects_brokers_v2_catalog_list(self) -> None:
        """Test case for servicebroker_projects_brokers_v2_catalog_list

        """
        pass

    def test_servicebroker_projects_brokers_v2_service_instances_create(self) -> None:
        """Test case for servicebroker_projects_brokers_v2_service_instances_create

        """
        pass

    def test_servicebroker_projects_brokers_v2_service_instances_delete(self) -> None:
        """Test case for servicebroker_projects_brokers_v2_service_instances_delete

        """
        pass

    def test_servicebroker_projects_brokers_v2_service_instances_get(self) -> None:
        """Test case for servicebroker_projects_brokers_v2_service_instances_get

        """
        pass

    def test_servicebroker_projects_brokers_v2_service_instances_get_last_operation(self) -> None:
        """Test case for servicebroker_projects_brokers_v2_service_instances_get_last_operation

        """
        pass

    def test_servicebroker_projects_brokers_v2_service_instances_patch(self) -> None:
        """Test case for servicebroker_projects_brokers_v2_service_instances_patch

        """
        pass

    def test_servicebroker_projects_brokers_v2_service_instances_service_bindings_create(self) -> None:
        """Test case for servicebroker_projects_brokers_v2_service_instances_service_bindings_create

        """
        pass

    def test_servicebroker_projects_brokers_v2_service_instances_service_bindings_get(self) -> None:
        """Test case for servicebroker_projects_brokers_v2_service_instances_service_bindings_get

        """
        pass

    def test_servicebroker_projects_brokers_v2_service_instances_service_bindings_get_last_operation(self) -> None:
        """Test case for servicebroker_projects_brokers_v2_service_instances_service_bindings_get_last_operation

        """
        pass


if __name__ == '__main__':
    unittest.main()
