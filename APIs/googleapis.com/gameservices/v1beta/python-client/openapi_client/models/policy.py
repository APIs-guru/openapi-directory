...
class Policy(BaseModel):
    ...
    """ # noqa: E501
    audit_configs: Optional[List[AuditConfig]] = Field(default=None, description="Specifies cloud audit logging configuration for this policy.", alias="auditConfigs")
    bindings: Optional[List[Binding]] = Field(default=None, description="Associates a list of `members`, or principals, with a `role`. Optionally, may specify a `condition` that determines how and when the `bindings` are applied. Each of the `bindings` must contain at least one principal. The `bindings` in a `Policy` can refer to up to 1,500 principals; up to 250 of these principals can be Google groups. Each occurrence of a principal counts towards these limits. For example, if the `bindings` grant 50 different roles to `user:alice@example.com`, and not to any other principal, then you can add another 1,450 principals to the `bindings` in the `Policy`.")
    ...
    rules: Optional[List[Rule]] = Field(default=None, description="If more than one rule is specified, the rules are applied in the following manner: - All matching LOG rules are always applied. - If any DENY/DENY_WITH_LOG rule matches, permission is denied. Logging will be applied if one or more matching rule requires logging. - Otherwise, if any ALLOW/ALLOW_WITH_LOG rule matches, permission is granted. Logging will be applied if one or more matching rule requires logging. - Otherwise, if no rule applies, permission is denied.")
    ...
    __properties: ClassVar[List[str]] = ["auditConfigs", "bindings", "etag", "rules", "version"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "auditConfigs": [AuditConfig.from_dict(_item) for _item in obj["auditConfigs"]] if obj.get("auditConfigs") is not None else None,
