...
class Membership(BaseModel):
    ...
    external_id: Optional[StrictStr] = Field(default=None, description="Optional. An externally-generated and managed ID for this Membership. This ID may be modified after creation, but this is not recommended. For GKE clusters, external_id is managed by the Hub API and updates will be ignored. The ID must match the regex: `a-zA-Z0-9*` If this Membership represents a Kubernetes cluster, this value should be set to the UID of the `kube-system` namespace object.", alias="externalId")
    infrastructure_type: Optional[StrictStr] = Field(default=None, description="Optional. The infrastructure type this Membership is running on.", alias="infrastructureType")
    ...
    __properties: ClassVar[List[str]] = ["authority", "createTime", "deleteTime", "description", "endpoint", "externalId", "infrastructureType", "labels", "lastConnectionTime", "monitoringConfig", "name", "state", "uniqueId", "updateTime"]

    @field_validator('infrastructure_type')
    def infrastructure_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['INFRASTRUCTURE_TYPE_UNSPECIFIED', 'ON_PREM', 'MULTI_CLOUD']):
            raise ValueError("must be one of enum values ('INFRASTRUCTURE_TYPE_UNSPECIFIED', 'ON_PREM', 'MULTI_CLOUD')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "infrastructureType": obj.get("infrastructureType"),
