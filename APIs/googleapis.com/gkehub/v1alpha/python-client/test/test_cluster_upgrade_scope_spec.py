# coding: utf-8

"""
    GKE Hub API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: v1alpha
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.cluster_upgrade_scope_spec import ClusterUpgradeScopeSpec

class TestClusterUpgradeScopeSpec(unittest.TestCase):
    """ClusterUpgradeScopeSpec unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ClusterUpgradeScopeSpec:
        """Test ClusterUpgradeScopeSpec
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ClusterUpgradeScopeSpec`
        """
        model = ClusterUpgradeScopeSpec()
        if include_optional:
            return ClusterUpgradeScopeSpec(
                gke_upgrade_overrides = [
                    openapi_client.models.cluster_upgrade_gke_upgrade_override.ClusterUpgradeGKEUpgradeOverride(
                        post_conditions = openapi_client.models.cluster_upgrade_post_conditions.ClusterUpgradePostConditions(
                            soaking = '', ), 
                        upgrade = openapi_client.models.cluster_upgrade_gke_upgrade.ClusterUpgradeGKEUpgrade(
                            name = '', 
                            version = '', ), )
                    ],
                post_conditions = openapi_client.models.cluster_upgrade_post_conditions.ClusterUpgradePostConditions(
                    soaking = '', ),
                upstream_scopes = [
                    ''
                    ]
            )
        else:
            return ClusterUpgradeScopeSpec(
        )
        """

    def testClusterUpgradeScopeSpec(self):
        """Test ClusterUpgradeScopeSpec"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
