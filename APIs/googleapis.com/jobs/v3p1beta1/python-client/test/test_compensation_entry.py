# coding: utf-8

"""
    Cloud Talent Solution API

    Cloud Talent Solution provides the capability to create, read, update, and delete job postings, as well as search jobs based on keywords and filters. 

    The version of the OpenAPI document: v3p1beta1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.compensation_entry import CompensationEntry

class TestCompensationEntry(unittest.TestCase):
    """CompensationEntry unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CompensationEntry:
        """Test CompensationEntry
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CompensationEntry`
        """
        model = CompensationEntry()
        if include_optional:
            return CompensationEntry(
                amount = openapi_client.models.money.Money(
                    currency_code = '', 
                    nanos = 56, 
                    units = '', ),
                description = '',
                expected_units_per_year = 1.337,
                range = openapi_client.models.compensation_range.CompensationRange(
                    max_compensation = openapi_client.models.money.Money(
                        currency_code = '', 
                        nanos = 56, 
                        units = '', ), 
                    min_compensation = openapi_client.models.money.Money(
                        currency_code = '', 
                        nanos = 56, 
                        units = '', ), ),
                type = 'COMPENSATION_TYPE_UNSPECIFIED',
                unit = 'COMPENSATION_UNIT_UNSPECIFIED'
            )
        else:
            return CompensationEntry(
        )
        """

    def testCompensationEntry(self):
        """Test CompensationEntry"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
