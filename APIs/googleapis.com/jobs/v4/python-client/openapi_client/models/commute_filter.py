...
class CommuteFilter(BaseModel):
    ...
    """ # noqa: E501
    allow_imprecise_addresses: Optional[StrictBool] = Field(default=None, description="If `true`, jobs without street level addresses may also be returned. For city level addresses, the city center is used. For state and coarser level addresses, text matching is used. If this field is set to `false` or isn't specified, only jobs that include street level addresses will be returned by commute search.", alias="allowImpreciseAddresses")
    commute_method: Optional[StrictStr] = Field(default=None, description="Required. The method of transportation to calculate the commute time for.", alias="commuteMethod")
    departure_time: Optional[TimeOfDay] = Field(default=None, alias="departureTime")
    road_traffic: Optional[StrictStr] = Field(default=None, description="Specifies the traffic density to use when calculating commute time.", alias="roadTraffic")
    ...
    def commute_method_validate_enum(cls, value):
        ...
        if value not in set(['COMMUTE_METHOD_UNSPECIFIED', 'DRIVING', 'TRANSIT', 'WALKING', 'CYCLING', 'TRANSIT_ACCESSIBLE']):
            raise ValueError("must be one of enum values ('COMMUTE_METHOD_UNSPECIFIED', 'DRIVING', 'TRANSIT', 'WALKING', 'CYCLING', 'TRANSIT_ACCESSIBLE')")
        return value
