...
class GoogleChromePolicyVersionsV1BatchInheritOrgUnitPoliciesRequest(BaseModel):
    ...
    """ # noqa: E501
    requests: Optional[List[GoogleChromePolicyVersionsV1InheritOrgUnitPolicyRequest]] = Field(default=None, description="List of policies that have to inherit their values as defined by the `requests`. All requests in the list must follow these restrictions: 1. All schemas in the list must have the same root namespace. 2. All `policyTargetKey.targetResource` values must point to an org unit resource. 3. All `policyTargetKey` values must have the same key names in the ` additionalTargetKeys`. This also means if one of the targets has an empty `additionalTargetKeys` map, all of the targets must have an empty `additionalTargetKeys` map. 4. No two modification requests can reference the same `policySchema` + ` policyTargetKey` pair. ")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GoogleChromePolicyVersionsV1BatchInheritOrgUnitPoliciesRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GoogleChromePolicyVersionsV1BatchInheritOrgUnitPoliciesRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "requests": [GoogleChromePolicyVersionsV1InheritOrgUnitPolicyRequest.from_dict(_item) for _item in obj["requests"]] if obj.get("requests") is not None else None
        })
        return _obj
