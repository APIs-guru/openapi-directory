# coding: utf-8

"""
    Content API for Shopping

    Manage your product listings and accounts for Google Shopping

    The version of the OpenAPI document: v2.1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.regional_inventory import RegionalInventory

class TestRegionalInventory(unittest.TestCase):
    """RegionalInventory unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> RegionalInventory:
        """Test RegionalInventory
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `RegionalInventory`
        """
        model = RegionalInventory()
        if include_optional:
            return RegionalInventory(
                availability = '',
                custom_attributes = [
                    openapi_client.models.custom_attribute.CustomAttribute(
                        group_values = [
                            openapi_client.models.custom_attribute.CustomAttribute(
                                name = '', 
                                value = '', )
                            ], 
                        name = '', 
                        value = '', )
                    ],
                kind = '',
                price = openapi_client.models.price.Price(
                    currency = '', 
                    value = '', ),
                region_id = '',
                sale_price = openapi_client.models.price.Price(
                    currency = '', 
                    value = '', ),
                sale_price_effective_date = ''
            )
        else:
            return RegionalInventory(
        )
        """

    def testRegionalInventory(self):
        """Test RegionalInventory"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
