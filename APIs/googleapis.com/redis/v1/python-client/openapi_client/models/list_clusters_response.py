...
class ListClustersResponse(BaseModel):
    ...
    """ # noqa: E501
    clusters: Optional[List[Cluster]] = Field(default=None, description="A list of Redis clusters in the project in the specified location, or across all locations. If the `location_id` in the parent field of the request is \"-\", all regions available to the project are queried, and the results aggregated. If in such an aggregated query a location is unavailable, a placeholder Redis entry is included in the response with the `name` field set to a value of the form `projects/{project_id}/locations/{location_id}/clusters/`- and the `status` field set to ERROR and `status_message` field set to \"location not available for ListClusters\".")
    ...
    __properties: ClassVar[List[str]] = ["clusters", "nextPageToken", "unreachable"]
