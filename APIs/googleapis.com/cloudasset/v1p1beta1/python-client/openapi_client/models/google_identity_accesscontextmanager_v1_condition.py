...
class GoogleIdentityAccesscontextmanagerV1Condition(BaseModel):
    ...
    negate: Optional[StrictBool] = Field(default=None, description="Whether to negate the Condition. If true, the Condition becomes a NAND over its non-empty fields. Any non-empty field criteria evaluating to false will result in the Condition to be satisfied. Defaults to false.")
    ...
    vpc_network_sources: Optional[List[GoogleIdentityAccesscontextmanagerV1VpcNetworkSource]] = Field(default=None, description="The request must originate from one of the provided VPC networks in Google Cloud. Cannot specify this field together with `ip_subnetworks`.", alias="vpcNetworkSources")
    __properties: ClassVar[List[str]] = ["devicePolicy", "ipSubnetworks", "members", "negate", "regions", "requiredAccessLevels", "vpcNetworkSources"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in vpc_network_sources (list)
        _items = []
        if self.vpc_network_sources:
            for _item_vpc_network_sources in self.vpc_network_sources:
                if _item_vpc_network_sources:
                    _items.append(_item_vpc_network_sources.to_dict())
            _dict['vpcNetworkSources'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "vpcNetworkSources": [GoogleIdentityAccesscontextmanagerV1VpcNetworkSource.from_dict(_item) for _item in obj["vpcNetworkSources"]] if obj.get("vpcNetworkSources") is not None else None
        })
        return _obj
