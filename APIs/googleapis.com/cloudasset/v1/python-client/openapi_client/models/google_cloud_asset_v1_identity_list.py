...
class GoogleCloudAssetV1IdentityList(BaseModel):
    """
    The identities and group edges.
    """ # noqa: E501
    group_edges: Optional[List[GoogleCloudAssetV1Edge]] = Field(default=None, description="Group identity edges of the graph starting from the binding's group members to any node of the identities. The Edge.source_node contains a group, such as `group:parent@google.com`. The Edge.target_node contains a member of the group, such as `group:child@google.com` or `user:foo@google.com`. This field is present only if the output_group_edges option is enabled in request.", alias="groupEdges")
    identities: Optional[List[GoogleCloudAssetV1Identity]] = Field(default=None, description="Only the identities that match one of the following conditions will be presented: - The identity_selector, if it is specified in request; - Otherwise, identities reachable from the policy binding's members.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "groupEdges": [GoogleCloudAssetV1Edge.from_dict(_item) for _item in obj["groupEdges"]] if obj.get("groupEdges") is not None else None,
            "identities": [GoogleCloudAssetV1Identity.from_dict(_item) for _item in obj["identities"]] if obj.get("identities") is not None else None
        })
        return _obj
