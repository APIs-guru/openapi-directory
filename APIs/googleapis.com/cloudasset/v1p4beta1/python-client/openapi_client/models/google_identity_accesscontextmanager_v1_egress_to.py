...
class GoogleIdentityAccesscontextmanagerV1EgressTo(BaseModel):
    ...
    """ # noqa: E501
    external_resources: Optional[List[StrictStr]] = Field(default=None, description="A list of external resources that are allowed to be accessed. A request matches if it contains an external resource in this list (Example: s3://bucket/path). Currently '*' is not allowed.", alias="externalResources")
