...
class BatchTranslateDocumentRequest(BaseModel):
    ...
    source_language_code: Optional[StrictStr] = Field(default=None, description="Required. The BCP-47 language code of the input document if known, for example, \"en-US\" or \"sr-Latn\". Supported language codes are listed in [Language Support](https://cloud.google.com/translate/docs/languages).", alias="sourceLanguageCode")
    target_language_codes: Optional[List[StrictStr]] = Field(default=None, description="Required. The BCP-47 language code to use for translation of the input document. Specify up to 10 language codes here.", alias="targetLanguageCodes")
