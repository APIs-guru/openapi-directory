# coding: utf-8

"""
    Cloud Debugger API (Deprecated)

    Examines the call stack and variables of a running application without stopping or slowing it down. (Deprecated) 

    The version of the OpenAPI document: v2
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.cloud_workspace_source_context import CloudWorkspaceSourceContext

class TestCloudWorkspaceSourceContext(unittest.TestCase):
    """CloudWorkspaceSourceContext unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CloudWorkspaceSourceContext:
        """Test CloudWorkspaceSourceContext
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CloudWorkspaceSourceContext`
        """
        model = CloudWorkspaceSourceContext()
        if include_optional:
            return CloudWorkspaceSourceContext(
                snapshot_id = '',
                workspace_id = openapi_client.models.cloud_workspace_id.CloudWorkspaceId(
                    name = '', 
                    repo_id = openapi_client.models.repo_id.RepoId(
                        project_repo_id = openapi_client.models.project_repo_id.ProjectRepoId(
                            project_id = '', 
                            repo_name = '', ), 
                        uid = '', ), )
            )
        else:
            return CloudWorkspaceSourceContext(
        )
        """

    def testCloudWorkspaceSourceContext(self):
        """Test CloudWorkspaceSourceContext"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
