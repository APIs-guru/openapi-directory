# coding: utf-8

"""
    Display & Video 360 API

    Display & Video 360 API allows users to automate complex Display & Video 360 workflows, such as creating insertion orders and setting targeting options for individual line items.

    The version of the OpenAPI document: v2
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.partner_cost import PartnerCost

class TestPartnerCost(unittest.TestCase):
    """PartnerCost unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PartnerCost:
        """Test PartnerCost
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PartnerCost`
        """
        model = PartnerCost()
        if include_optional:
            return PartnerCost(
                cost_type = 'PARTNER_COST_TYPE_UNSPECIFIED',
                fee_amount = '',
                fee_percentage_millis = '',
                fee_type = 'PARTNER_COST_FEE_TYPE_UNSPECIFIED',
                invoice_type = 'PARTNER_COST_INVOICE_TYPE_UNSPECIFIED'
            )
        else:
            return PartnerCost(
        )
        """

    def testPartnerCost(self):
        """Test PartnerCost"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
