# coding: utf-8

"""
    Vertex AI API

    Train high-quality custom machine learning models with minimal machine learning expertise and effort.

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.google_cloud_aiplatform_v1_pipeline_task_executor_detail import GoogleCloudAiplatformV1PipelineTaskExecutorDetail

class TestGoogleCloudAiplatformV1PipelineTaskExecutorDetail(unittest.TestCase):
    """GoogleCloudAiplatformV1PipelineTaskExecutorDetail unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> GoogleCloudAiplatformV1PipelineTaskExecutorDetail:
        """Test GoogleCloudAiplatformV1PipelineTaskExecutorDetail
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GoogleCloudAiplatformV1PipelineTaskExecutorDetail`
        """
        model = GoogleCloudAiplatformV1PipelineTaskExecutorDetail()
        if include_optional:
            return GoogleCloudAiplatformV1PipelineTaskExecutorDetail(
                container_detail = openapi_client.models.google_cloud_aiplatform_v1_pipeline_task_executor_detail_container_detail.GoogleCloudAiplatformV1PipelineTaskExecutorDetailContainerDetail(
                    failed_main_jobs = [
                        ''
                        ], 
                    failed_pre_caching_check_jobs = [
                        ''
                        ], 
                    main_job = '', 
                    pre_caching_check_job = '', ),
                custom_job_detail = openapi_client.models.google_cloud_aiplatform_v1_pipeline_task_executor_detail_custom_job_detail.GoogleCloudAiplatformV1PipelineTaskExecutorDetailCustomJobDetail(
                    failed_jobs = [
                        ''
                        ], 
                    job = '', )
            )
        else:
            return GoogleCloudAiplatformV1PipelineTaskExecutorDetail(
        )
        """

    def testGoogleCloudAiplatformV1PipelineTaskExecutorDetail(self):
        """Test GoogleCloudAiplatformV1PipelineTaskExecutorDetail"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
