# coding: utf-8

"""
    Vertex AI API

    Train high-quality custom machine learning models with minimal machine learning expertise and effort.

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.google_cloud_aiplatform_v1_export_feature_values_request import GoogleCloudAiplatformV1ExportFeatureValuesRequest

class TestGoogleCloudAiplatformV1ExportFeatureValuesRequest(unittest.TestCase):
    """GoogleCloudAiplatformV1ExportFeatureValuesRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> GoogleCloudAiplatformV1ExportFeatureValuesRequest:
        """Test GoogleCloudAiplatformV1ExportFeatureValuesRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GoogleCloudAiplatformV1ExportFeatureValuesRequest`
        """
        model = GoogleCloudAiplatformV1ExportFeatureValuesRequest()
        if include_optional:
            return GoogleCloudAiplatformV1ExportFeatureValuesRequest(
                destination = openapi_client.models.google_cloud_aiplatform_v1_feature_value_destination.GoogleCloudAiplatformV1FeatureValueDestination(
                    bigquery_destination = openapi_client.models.google_cloud_aiplatform_v1_big_query_destination.GoogleCloudAiplatformV1BigQueryDestination(
                        output_uri = '', ), 
                    csv_destination = openapi_client.models.google_cloud_aiplatform_v1_csv_destination.GoogleCloudAiplatformV1CsvDestination(
                        gcs_destination = openapi_client.models.google_cloud_aiplatform_v1_gcs_destination.GoogleCloudAiplatformV1GcsDestination(
                            output_uri_prefix = '', ), ), 
                    tfrecord_destination = openapi_client.models.google_cloud_aiplatform_v1_tf_record_destination.GoogleCloudAiplatformV1TFRecordDestination(), ),
                feature_selector = openapi_client.models.google_cloud_aiplatform_v1_feature_selector.GoogleCloudAiplatformV1FeatureSelector(
                    id_matcher = openapi_client.models.google_cloud_aiplatform_v1_id_matcher.GoogleCloudAiplatformV1IdMatcher(
                        ids = [
                            ''
                            ], ), ),
                full_export = openapi_client.models.google_cloud_aiplatform_v1_export_feature_values_request_full_export.GoogleCloudAiplatformV1ExportFeatureValuesRequestFullExport(
                    end_time = '', 
                    start_time = '', ),
                settings = [
                    openapi_client.models.google_cloud_aiplatform_v1_destination_feature_setting.GoogleCloudAiplatformV1DestinationFeatureSetting(
                        destination_field = '', 
                        feature_id = '', )
                    ],
                snapshot_export = openapi_client.models.google_cloud_aiplatform_v1_export_feature_values_request_snapshot_export.GoogleCloudAiplatformV1ExportFeatureValuesRequestSnapshotExport(
                    snapshot_time = '', 
                    start_time = '', )
            )
        else:
            return GoogleCloudAiplatformV1ExportFeatureValuesRequest(
        )
        """

    def testGoogleCloudAiplatformV1ExportFeatureValuesRequest(self):
        """Test GoogleCloudAiplatformV1ExportFeatureValuesRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
