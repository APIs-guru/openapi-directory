...
class GoogleCloudAiplatformV1Part(BaseModel):
    """
    A datatype containing media that is part of a multi-part `Content` message. A `Part` consists of data which has an associated datatype. A `Part` can only contain one of the accepted types in `Part.data`. A `Part` must have a fixed IANA MIME type identifying the type and subtype of the media if `inline_data` or `file_data` field is filled with raw bytes.
    """ # noqa: E501
    file_data: Optional[GoogleCloudAiplatformV1FileData] = Field(default=None, alias="fileData")
    function_call: Optional[GoogleCloudAiplatformV1FunctionCall] = Field(default=None, alias="functionCall")
    function_response: Optional[GoogleCloudAiplatformV1FunctionResponse] = Field(default=None, alias="functionResponse")
    inline_data: Optional[GoogleCloudAiplatformV1Blob] = Field(default=None, alias="inlineData")
    text: Optional[StrictStr] = Field(default=None, description="Optional. Text part (can be code).")
    video_metadata: Optional[GoogleCloudAiplatformV1VideoMetadata] = Field(default=None, alias="videoMetadata")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "fileData": GoogleCloudAiplatformV1FileData.from_dict(obj["fileData"]) if obj.get("fileData") is not None else None,
            "functionCall": GoogleCloudAiplatformV1FunctionCall.from_dict(obj["functionCall"]) if obj.get("functionCall") is not None else None,
            "functionResponse": GoogleCloudAiplatformV1FunctionResponse.from_dict(obj["functionResponse"]) if obj.get("functionResponse") is not None else None,
            "inlineData": GoogleCloudAiplatformV1Blob.from_dict(obj["inlineData"]) if obj.get("inlineData") is not None else None,
            "text": obj.get("text"),
            "videoMetadata": GoogleCloudAiplatformV1VideoMetadata.from_dict(obj["videoMetadata"]) if obj.get("videoMetadata") is not None else None
        })
        return _obj
