...
class CapacityCommitment(BaseModel):
    ...
    renewal_plan: Optional[StrictStr] = Field(default=None, description="The plan this capacity commitment is converted to after commitment_end_time passes. Once the plan is changed, committed period is extended according to commitment plan. Only applicable for ANNUAL commitments.", alias="renewalPlan")
    ...
    __properties: ClassVar[List[str]] = ["commitmentEndTime", "commitmentStartTime", "failureStatus", "multiRegionAuxiliary", "name", "plan", "renewalPlan", "slotCount", "state"]
    ...
    def plan_validate_enum(cls, value):
        ...
        if value not in set(['COMMITMENT_PLAN_UNSPECIFIED', 'FLEX', 'TRIAL', 'MONTHLY', 'ANNUAL']):
            raise ValueError("must be one of enum values ('COMMITMENT_PLAN_UNSPECIFIED', 'FLEX', 'TRIAL', 'MONTHLY', 'ANNUAL')")
        return value
