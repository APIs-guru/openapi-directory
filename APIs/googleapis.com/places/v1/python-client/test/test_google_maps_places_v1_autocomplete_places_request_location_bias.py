# coding: utf-8

"""
    Places API (New)

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.google_maps_places_v1_autocomplete_places_request_location_bias import GoogleMapsPlacesV1AutocompletePlacesRequestLocationBias

class TestGoogleMapsPlacesV1AutocompletePlacesRequestLocationBias(unittest.TestCase):
    """GoogleMapsPlacesV1AutocompletePlacesRequestLocationBias unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> GoogleMapsPlacesV1AutocompletePlacesRequestLocationBias:
        """Test GoogleMapsPlacesV1AutocompletePlacesRequestLocationBias
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GoogleMapsPlacesV1AutocompletePlacesRequestLocationBias`
        """
        model = GoogleMapsPlacesV1AutocompletePlacesRequestLocationBias()
        if include_optional:
            return GoogleMapsPlacesV1AutocompletePlacesRequestLocationBias(
                circle = openapi_client.models.google_maps_places_v1_circle.GoogleMapsPlacesV1Circle(
                    center = openapi_client.models.google_type_lat_lng.GoogleTypeLatLng(
                        latitude = 1.337, 
                        longitude = 1.337, ), 
                    radius = 1.337, ),
                rectangle = openapi_client.models.google_geo_type_viewport.GoogleGeoTypeViewport(
                    high = openapi_client.models.google_type_lat_lng.GoogleTypeLatLng(
                        latitude = 1.337, 
                        longitude = 1.337, ), 
                    low = openapi_client.models.google_type_lat_lng.GoogleTypeLatLng(
                        latitude = 1.337, 
                        longitude = 1.337, ), )
            )
        else:
            return GoogleMapsPlacesV1AutocompletePlacesRequestLocationBias(
        )
        """

    def testGoogleMapsPlacesV1AutocompletePlacesRequestLocationBias(self):
        """Test GoogleMapsPlacesV1AutocompletePlacesRequestLocationBias"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
