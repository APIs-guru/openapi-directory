# coding: utf-8

"""
    OS Config API

    OS management tools that can be used for patch management, patch compliance, and configuration management on VM instances.

    The version of the OpenAPI document: v1alpha
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.projects_api import ProjectsApi


class TestProjectsApi(unittest.TestCase):
    """ProjectsApi unit test stubs"""

    def setUp(self) -> None:
        self.api = ProjectsApi()

    def tearDown(self) -> None:
        pass

    def test_osconfig_projects_locations_instance_os_policies_compliances_list(self) -> None:
        """Test case for osconfig_projects_locations_instance_os_policies_compliances_list

        """
        pass

    def test_osconfig_projects_locations_instances_inventories_list(self) -> None:
        """Test case for osconfig_projects_locations_instances_inventories_list

        """
        pass

    def test_osconfig_projects_locations_instances_os_policy_assignments_reports_list(self) -> None:
        """Test case for osconfig_projects_locations_instances_os_policy_assignments_reports_list

        """
        pass

    def test_osconfig_projects_locations_instances_vulnerability_reports_list(self) -> None:
        """Test case for osconfig_projects_locations_instances_vulnerability_reports_list

        """
        pass

    def test_osconfig_projects_locations_os_policy_assignments_create(self) -> None:
        """Test case for osconfig_projects_locations_os_policy_assignments_create

        """
        pass

    def test_osconfig_projects_locations_os_policy_assignments_delete(self) -> None:
        """Test case for osconfig_projects_locations_os_policy_assignments_delete

        """
        pass

    def test_osconfig_projects_locations_os_policy_assignments_list(self) -> None:
        """Test case for osconfig_projects_locations_os_policy_assignments_list

        """
        pass

    def test_osconfig_projects_locations_os_policy_assignments_list_revisions(self) -> None:
        """Test case for osconfig_projects_locations_os_policy_assignments_list_revisions

        """
        pass

    def test_osconfig_projects_locations_os_policy_assignments_operations_cancel(self) -> None:
        """Test case for osconfig_projects_locations_os_policy_assignments_operations_cancel

        """
        pass

    def test_osconfig_projects_locations_os_policy_assignments_operations_get(self) -> None:
        """Test case for osconfig_projects_locations_os_policy_assignments_operations_get

        """
        pass

    def test_osconfig_projects_locations_os_policy_assignments_patch(self) -> None:
        """Test case for osconfig_projects_locations_os_policy_assignments_patch

        """
        pass


if __name__ == '__main__':
    unittest.main()
