...
class ApigatewayPolicy(BaseModel):
    """
    ...
    """ # noqa: E501
    audit_configs: Optional[List[ApigatewayAuditConfig]] = Field(default=None, description="Specifies cloud audit logging configuration for this policy.", alias="auditConfigs")
    bindings: Optional[List[ApigatewayBinding]] = Field(default=None, description="Associates a list of `members`, or principals, with a `role`. Optionally, may specify a `condition` that determines how and when the `bindings` are applied. Each of the `bindings` must contain at least one principal. The `bindings` in a `Policy` can refer to up to 1,500 principals; up to 250 of these principals can be Google groups. Each occurrence of a principal counts towards these limits. For example, if the `bindings` grant 50 different roles to `user:alice@example.com`, and not to any other principal, then you can add another 1,450 principals to the `bindings` in the `Policy`.")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ApigatewayPolicy from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ApigatewayPolicy from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "auditConfigs": [ApigatewayAuditConfig.from_dict(_item) for _item in obj["auditConfigs"]] if obj.get("auditConfigs") is not None else None,
            "bindings": [ApigatewayBinding.from_dict(_item) for _item in obj["bindings"]] if obj.get("bindings") is not None else None,
            "etag": obj.get("etag"),
