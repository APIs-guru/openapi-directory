...
**exit_code** | **int** | Output only. Return code from running the step. | [optional] 
...
**status** | **str** | Output only. Status of the build step. At this time, build step status is only updated on build completion; step status is not updated in real-time as the build progresses. | [optional] 
