...
class ContaineranalysisGoogleDevtoolsCloudbuildV1GitSource(BaseModel):
    ...
    revision: Optional[StrictStr] = Field(default=None, description="The revision to fetch from the Git repository such as a branch, a tag, a commit SHA, or any Git ref. Cloud Build uses `git fetch` to fetch the revision from the Git repository; therefore make sure that the string you provide for `revision` is parsable by the command. For information on string values accepted by `git fetch`, see https://git-scm.com/docs/gitrevisions#_specifying_revisions. For information on `git fetch`, see https://git-scm.com/docs/git-fetch.")
    url: Optional[StrictStr] = Field(default=None, description="Location of the Git repo to build. This will be used as a `git remote`, see https://git-scm.com/docs/git-remote.")
    __properties: ClassVar[List[str]] = ["dir", "revision", "url"]
