...
class JobStatus(BaseModel):
    ...
    errors: Optional[List[ErrorProto]] = Field(default=None, description="Output only. The first errors encountered during the running of the job. The final message includes the number of errors that caused the process to stop. Errors here do not necessarily mean that the job has not completed or was unsuccessful.")
    state: Optional[StrictStr] = Field(default=None, description="Output only. Running state of the job. Valid states include 'PENDING', 'RUNNING', and 'DONE'.")
    __properties: ClassVar[List[str]] = ["errorResult", "errors", "state"]
