...
class SynthesizeSpeechRequest(BaseModel):
    ...
    enable_time_pointing: Optional[List[StrictStr]] = Field(default=None, description="Whether and what timepoints are returned in the response.", alias="enableTimePointing")
    input: Optional[SynthesisInput] = None
    voice: Optional[VoiceSelectionParams] = None
    __properties: ClassVar[List[str]] = ["audioConfig", "enableTimePointing", "input", "voice"]

    @field_validator('enable_time_pointing')
    def enable_time_pointing_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['TIMEPOINT_TYPE_UNSPECIFIED', 'SSML_MARK']):
                raise ValueError("each list item must be one of ('TIMEPOINT_TYPE_UNSPECIFIED', 'SSML_MARK')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "enableTimePointing": obj.get("enableTimePointing"),
