...
class GoogleCloudDiscoveryengineV1alphaTrainCustomModelResponse(BaseModel):
    """
    ...
    error_samples: Optional[List[GoogleRpcStatus]] = Field(default=None, description="A sample of errors encountered while processing the data.", alias="errorSamples")
    model_status: Optional[StrictStr] = Field(default=None, description="The trained model status. Possible values are: * **bad-data**: The training data quality is bad. * **no-improvement**: Tuning didn't improve performance. Won't deploy. * **in-progress**: Model training is in progress. * **ready**: The model is ready for serving.", alias="modelStatus")
    __properties: ClassVar[List[str]] = ["errorConfig", "errorSamples", "modelStatus"]
