...
class FirstDerivativeElevationGrid(BaseModel):
    ...
    """ # noqa: E501
    altitude_multiplier: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="A multiplier applied to the altitude fields below to extract the actual altitudes in meters from the elevation grid.", alias="altitudeMultiplier")
    rows: Optional[List[Row]] = Field(default=None, description="Rows of points containing altitude data making up the elevation grid. Each row is the same length. Rows are ordered from north to south. E.g: rows[0] is the north-most row, and rows[n] is the south-most row.")
    __properties: ClassVar[List[str]] = ["altitudeMultiplier", "rows"]
