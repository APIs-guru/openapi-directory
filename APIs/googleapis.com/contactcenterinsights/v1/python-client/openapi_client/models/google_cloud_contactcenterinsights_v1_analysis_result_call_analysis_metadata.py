...
class GoogleCloudContactcenterinsightsV1AnalysisResultCallAnalysisMetadata(BaseModel):
    ...
    """ # noqa: E501
    annotations: Optional[List[GoogleCloudContactcenterinsightsV1CallAnnotation]] = Field(default=None, description="A list of call annotations that apply to this call.")
    entities: Optional[Dict[str, GoogleCloudContactcenterinsightsV1Entity]] = Field(default=None, description="All the entities in the call.")
    intents: Optional[Dict[str, GoogleCloudContactcenterinsightsV1Intent]] = Field(default=None, description="All the matched intents in the call.")
    issue_model_result: Optional[GoogleCloudContactcenterinsightsV1IssueModelResult] = Field(default=None, alias="issueModelResult")
    phrase_matchers: Optional[Dict[str, GoogleCloudContactcenterinsightsV1PhraseMatchData]] = Field(default=None, description="All the matched phrase matchers in the call.", alias="phraseMatchers")
    sentiments: Optional[List[GoogleCloudContactcenterinsightsV1ConversationLevelSentiment]] = Field(default=None, description="Overall conversation-level sentiment for each channel of the call.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "annotations": [GoogleCloudContactcenterinsightsV1CallAnnotation.from_dict(_item) for _item in obj["annotations"]] if obj.get("annotations") is not None else None,
            "entities": dict(
                (_k, GoogleCloudContactcenterinsightsV1Entity.from_dict(_v))
                for _k, _v in obj["entities"].items()
            )
            if obj.get("entities") is not None
            else None,
            "intents": dict(
                (_k, GoogleCloudContactcenterinsightsV1Intent.from_dict(_v))
                for _k, _v in obj["intents"].items()
            )
            if obj.get("intents") is not None
            else None,
            "issueModelResult": GoogleCloudContactcenterinsightsV1IssueModelResult.from_dict(obj["issueModelResult"]) if obj.get("issueModelResult") is not None else None,
            "phraseMatchers": dict(
                (_k, GoogleCloudContactcenterinsightsV1PhraseMatchData.from_dict(_v))
                for _k, _v in obj["phraseMatchers"].items()
            )
            if obj.get("phraseMatchers") is not None
            else None,
            "sentiments": [GoogleCloudContactcenterinsightsV1ConversationLevelSentiment.from_dict(_item) for _item in obj["sentiments"]] if obj.get("sentiments") is not None else None
        })
        return _obj
