...
class ExtraLinkCollection(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        _items = []
        if self.extra_links:
            for _item_extra_links in self.extra_links:
                if _item_extra_links:
                    _items.append(_item_extra_links.to_dict())
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "extra_links": [ExtraLink.from_dict(_item) for _item in obj["extra_links"]] if obj.get("extra_links") is not None else None
        })
        return _obj
