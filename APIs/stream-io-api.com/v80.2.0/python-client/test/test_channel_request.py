# coding: utf-8

"""
    Stream Chat API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: v80.2.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.channel_request import ChannelRequest

class TestChannelRequest(unittest.TestCase):
    """ChannelRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ChannelRequest:
        """Test ChannelRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ChannelRequest`
        """
        model = ChannelRequest()
        if include_optional:
            return ChannelRequest(
                auto_translation_enabled = True,
                auto_translation_language = '',
                config_overrides = openapi_client.models.channel_config_request.ChannelConfigRequest(
                    blocklist = '', 
                    blocklist_behavior = 'flag', 
                    commands = [
                        ''
                        ], 
                    grants = {
                        'key' : [
                            ''
                            ]
                        }, 
                    max_message_length = 56, 
                    quotes = True, 
                    reactions = True, 
                    replies = True, 
                    typing_events = True, 
                    uploads = True, 
                    url_enrichment = True, ),
                created_by = { },
                disabled = True,
                frozen = True,
                members = [
                    openapi_client.models.channel_member.Channel member(
                        ban_expires = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        banned = True, 
                        channel_role = '', 
                        created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        deleted_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        invite_accepted_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        invite_rejected_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        invited = True, 
                        is_moderator = True, 
                        role = 'member', 
                        shadow_banned = True, 
                        updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        user = { }, 
                        user_id = '', )
                    ],
                own_capabilities = [
                    56
                    ],
                team = '',
                truncated_at = [
                    56
                    ],
                truncated_by = [
                    56
                    ],
                truncated_by_id = ''
            )
        else:
            return ChannelRequest(
        )
        """

    def testChannelRequest(self):
        """Test ChannelRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
