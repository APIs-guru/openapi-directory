# coding: utf-8

"""
    Stream Chat API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: v80.2.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.channel_response_request import ChannelResponseRequest

class TestChannelResponseRequest(unittest.TestCase):
    """ChannelResponseRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ChannelResponseRequest:
        """Test ChannelResponseRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ChannelResponseRequest`
        """
        model = ChannelResponseRequest()
        if include_optional:
            return ChannelResponseRequest(
                auto_translation_enabled = True,
                auto_translation_language = '',
                cid = '',
                config = openapi_client.models.channel_config_with_info_request.ChannelConfigWithInfoRequest(
                    automod = 'disabled', 
                    automod_behavior = 'flag', 
                    automod_thresholds = openapi_client.models.auto_moderation_thresholds.Auto moderation thresholds(
                        explicit = openapi_client.models.label_thresholds_request.LabelThresholdsRequest(
                            block = 0, 
                            flag = 0, ), 
                        spam = openapi_client.models.label_thresholds_request.LabelThresholdsRequest(
                            block = 0, 
                            flag = 0, ), 
                        toxic = , ), 
                    blocklist = '', 
                    blocklist_behavior = 'flag', 
                    commands = [
                        openapi_client.models.command.Command(
                            args = '', 
                            description = '', 
                            name = '', 
                            set = '', )
                        ], 
                    connect_events = True, 
                    created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    custom_events = True, 
                    grants = {
                        'key' : [
                            ''
                            ]
                        }, 
                    max_message_length = 56, 
                    message_retention = '', 
                    mutes = True, 
                    name = '', 
                    push_notifications = True, 
                    quotes = True, 
                    reactions = True, 
                    read_events = True, 
                    reminders = True, 
                    replies = True, 
                    search = True, 
                    typing_events = True, 
                    updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    uploads = True, 
                    url_enrichment = True, ),
                cooldown = 56,
                created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                created_by = { },
                deleted_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                disabled = True,
                frozen = True,
                hidden = True,
                hide_messages_before = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                id = '',
                last_message_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                member_count = 56,
                members = [
                    openapi_client.models.channel_member.Channel member(
                        ban_expires = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        banned = True, 
                        channel_role = '', 
                        created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        deleted_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        invite_accepted_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        invite_rejected_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        invited = True, 
                        is_moderator = True, 
                        role = 'member', 
                        shadow_banned = True, 
                        updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        user = { }, 
                        user_id = '', )
                    ],
                mute_expires_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                muted = True,
                own_capabilities = [
                    ''
                    ],
                team = '',
                truncated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                truncated_by = { },
                type = '',
                updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f')
            )
        else:
            return ChannelResponseRequest(
        )
        """

    def testChannelResponseRequest(self):
        """Test ChannelResponseRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
