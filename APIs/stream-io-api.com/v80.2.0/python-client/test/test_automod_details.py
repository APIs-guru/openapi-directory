# coding: utf-8

"""
    Stream Chat API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: v80.2.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.automod_details import AutomodDetails

class TestAutomodDetails(unittest.TestCase):
    """AutomodDetails unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AutomodDetails:
        """Test AutomodDetails
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AutomodDetails`
        """
        model = AutomodDetails()
        if include_optional:
            return AutomodDetails(
                action = '',
                image_labels = [
                    ''
                    ],
                message_details = openapi_client.models.flag_message_details.FlagMessageDetails(
                    pin_changed = True, 
                    should_enrich = True, 
                    skip_push = True, 
                    updated_by_id = '', ),
                original_message_type = '',
                result = openapi_client.models.message_moderation_result.MessageModerationResult(
                    action = '', 
                    ai_moderation_response = openapi_client.models.moderation_response.ModerationResponse(
                        action = '', 
                        automod_response = null, 
                        explicit = 1.337, 
                        spam = 1.337, 
                        toxic = 1.337, ), 
                    blocked_word = '', 
                    blocklist_name = '', 
                    created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    message_id = '', 
                    moderated_by = '', 
                    moderation_thresholds = openapi_client.models.auto_moderation_thresholds.Auto moderation thresholds(
                        explicit = openapi_client.models.label_thresholds.LabelThresholds(
                            block = 0, 
                            flag = 0, ), 
                        spam = openapi_client.models.label_thresholds.LabelThresholds(
                            block = 0, 
                            flag = 0, ), 
                        toxic = , ), 
                    updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    user_bad_karma = True, 
                    user_karma = 1.337, )
            )
        else:
            return AutomodDetails(
        )
        """

    def testAutomodDetails(self):
        """Test AutomodDetails"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
