...
class BetaBuildLocalizationCreateRequestData(BaseModel):
    """
    ...
    type: StrictStr
    __properties: ClassVar[List[str]] = ["attributes", "relationships", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['betaBuildLocalizations']):
            raise ValueError("must be one of enum values ('betaBuildLocalizations')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "attributes": BetaBuildLocalizationCreateRequestDataAttributes.from_dict(obj["attributes"]) if obj.get("attributes") is not None else None,
            "relationships": BetaAppReviewSubmissionCreateRequestDataRelationships.from_dict(obj["relationships"]) if obj.get("relationships") is not None else None,
            "type": obj.get("type")
        })
        return _obj
