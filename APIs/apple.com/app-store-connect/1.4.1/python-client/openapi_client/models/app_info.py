...
class AppInfo(BaseModel):
    ...
    AppInfo
    """ # noqa: E501
    attributes: Optional[AppInfoAttributes] = None
    id: StrictStr
    links: ResourceLinks
    relationships: Optional[AppInfoRelationships] = None
    type: StrictStr
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['appInfos']):
            raise ValueError("must be one of enum values ('appInfos')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "attributes": AppInfoAttributes.from_dict(obj["attributes"]) if obj.get("attributes") is not None else None,
            "id": obj.get("id"),
            "links": ResourceLinks.from_dict(obj["links"]) if obj.get("links") is not None else None,
            "relationships": AppInfoRelationships.from_dict(obj["relationships"]) if obj.get("relationships") is not None else None,
            "type": obj.get("type")
        })
        return _obj
