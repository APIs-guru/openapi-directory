  ...
  pullDoc(
    ...
    return client
      .request({
        ...
        const res: operations.PullDocResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  pullUri(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.PullUriResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/xml`)) {
                const resBody: string = JSON.stringify(httpRes?.data, null, 0);
