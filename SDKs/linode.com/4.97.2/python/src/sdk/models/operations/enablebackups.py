from dataclasses import dataclass, field
from typing import Any,List,Optional
from dataclasses_json import dataclass_json
from sdk.models import shared


@dataclass
class EnableBackupsPathParams:
    linode_id: int = field(default=None, metadata={'path_param': { 'field_name': 'linodeId', 'style': 'simple', 'explode': False }})
    

@dataclass
class EnableBackupsSecurityOption1:
    personal_access_token: shared.SchemePersonalAccessToken = field(default=None, metadata={'security': { 'scheme': True, 'type': 'http', 'sub_type': 'bearer' }})
    

@dataclass
class EnableBackupsSecurityOption2:
    oauth: shared.SchemeOauth = field(default=None, metadata={'security': { 'scheme': True, 'type': 'oauth2' }})
    

@dataclass
class EnableBackupsSecurity:
    option1: Optional[EnableBackupsSecurityOption1] = field(default=None, metadata={'security': { 'option': True }})
    option2: Optional[EnableBackupsSecurityOption2] = field(default=None, metadata={'security': { 'option': True }})
    

@dataclass
class EnableBackupsRequest:
    path_params: EnableBackupsPathParams = field(default=None)
    security: EnableBackupsSecurity = field(default=None)
    

@dataclass_json
@dataclass
class EnableBackupsDefaultApplicationJSON:
    errors: Optional[List[shared.ErrorObject]] = field(default=None, metadata={'dataclasses_json': { 'field_name': 'errors' }})
    

@dataclass
class EnableBackupsResponse:
    content_type: str = field(default=None)
    status_code: int = field(default=None)
    enable_backups_200_application_json_object: Optional[dict[str, Any]] = field(default=None)
    enable_backups_default_application_json_object: Optional[EnableBackupsDefaultApplicationJSON] = field(default=None)
    
