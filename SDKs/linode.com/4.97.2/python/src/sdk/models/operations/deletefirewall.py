from dataclasses import dataclass, field
from typing import Any,List,Optional
from dataclasses_json import dataclass_json
from sdk.models import shared
DELETE_FIREWALL_SERVERS = [
	"https://api.linode.com/v4",
]


@dataclass
class DeleteFirewallPathParams:
    firewall_id: int = field(default=None, metadata={'path_param': { 'field_name': 'firewallId', 'style': 'simple', 'explode': False }})
    

@dataclass
class DeleteFirewallSecurityOption1:
    personal_access_token: shared.SchemePersonalAccessToken = field(default=None, metadata={'security': { 'scheme': True, 'type': 'http', 'sub_type': 'bearer' }})
    

@dataclass
class DeleteFirewallSecurityOption2:
    oauth: shared.SchemeOauth = field(default=None, metadata={'security': { 'scheme': True, 'type': 'oauth2' }})
    

@dataclass
class DeleteFirewallSecurity:
    option1: Optional[DeleteFirewallSecurityOption1] = field(default=None, metadata={'security': { 'option': True }})
    option2: Optional[DeleteFirewallSecurityOption2] = field(default=None, metadata={'security': { 'option': True }})
    

@dataclass
class DeleteFirewallRequest:
    server_url: Optional[str] = field(default=None)
    path_params: DeleteFirewallPathParams = field(default=None)
    security: DeleteFirewallSecurity = field(default=None)
    

@dataclass_json
@dataclass
class DeleteFirewallDefaultApplicationJSON:
    errors: Optional[List[shared.ErrorObject]] = field(default=None, metadata={'dataclasses_json': { 'field_name': 'errors' }})
    

@dataclass
class DeleteFirewallResponse:
    content_type: str = field(default=None)
    status_code: int = field(default=None)
    delete_firewall_200_application_json_object: Optional[dict[str, Any]] = field(default=None)
    delete_firewall_default_application_json_object: Optional[DeleteFirewallDefaultApplicationJSON] = field(default=None)
    
