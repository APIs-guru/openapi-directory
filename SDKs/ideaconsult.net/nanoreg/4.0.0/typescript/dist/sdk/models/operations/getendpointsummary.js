var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { SpeakeasyMetadata, SpeakeasyBase } from "../../../internal/utils";
import * as shared from "../shared";
var GetEndpointSummaryPathParams = /** @class */ (function (_super) {
    __extends(GetEndpointSummaryPathParams, _super);
    function GetEndpointSummaryPathParams() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        SpeakeasyMetadata({ data: "pathParam, style=simple;explode=false;name=db" }),
        __metadata("design:type", String)
    ], GetEndpointSummaryPathParams.prototype, "db", void 0);
    return GetEndpointSummaryPathParams;
}(SpeakeasyBase));
export { GetEndpointSummaryPathParams };
export var GetEndpointSummaryTopEnum;
(function (GetEndpointSummaryTopEnum) {
    GetEndpointSummaryTopEnum["PChem"] = "P-CHEM";
    GetEndpointSummaryTopEnum["Ecotox"] = "ECOTOX";
    GetEndpointSummaryTopEnum["EnvFate"] = "ENV FATE";
    GetEndpointSummaryTopEnum["Tox"] = "TOX";
    GetEndpointSummaryTopEnum["Exposure"] = "EXPOSURE";
})(GetEndpointSummaryTopEnum || (GetEndpointSummaryTopEnum = {}));
var GetEndpointSummaryQueryParams = /** @class */ (function (_super) {
    __extends(GetEndpointSummaryQueryParams, _super);
    function GetEndpointSummaryQueryParams() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=category" }),
        __metadata("design:type", String)
    ], GetEndpointSummaryQueryParams.prototype, "category", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=top" }),
        __metadata("design:type", String)
    ], GetEndpointSummaryQueryParams.prototype, "top", void 0);
    return GetEndpointSummaryQueryParams;
}(SpeakeasyBase));
export { GetEndpointSummaryQueryParams };
var GetEndpointSummaryRequest = /** @class */ (function (_super) {
    __extends(GetEndpointSummaryRequest, _super);
    function GetEndpointSummaryRequest() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        SpeakeasyMetadata(),
        __metadata("design:type", GetEndpointSummaryPathParams)
    ], GetEndpointSummaryRequest.prototype, "pathParams", void 0);
    __decorate([
        SpeakeasyMetadata(),
        __metadata("design:type", GetEndpointSummaryQueryParams)
    ], GetEndpointSummaryRequest.prototype, "queryParams", void 0);
    return GetEndpointSummaryRequest;
}(SpeakeasyBase));
export { GetEndpointSummaryRequest };
var GetEndpointSummaryResponse = /** @class */ (function (_super) {
    __extends(GetEndpointSummaryResponse, _super);
    function GetEndpointSummaryResponse() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        SpeakeasyMetadata(),
        __metadata("design:type", String)
    ], GetEndpointSummaryResponse.prototype, "contentType", void 0);
    __decorate([
        SpeakeasyMetadata(),
        __metadata("design:type", shared.Facet)
    ], GetEndpointSummaryResponse.prototype, "facet", void 0);
    __decorate([
        SpeakeasyMetadata(),
        __metadata("design:type", Number)
    ], GetEndpointSummaryResponse.prototype, "statusCode", void 0);
    return GetEndpointSummaryResponse;
}(SpeakeasyBase));
export { GetEndpointSummaryResponse };
