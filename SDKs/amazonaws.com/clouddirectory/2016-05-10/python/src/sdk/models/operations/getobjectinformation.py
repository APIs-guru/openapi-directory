...
class GetObjectInformationHeaders:
    ...
    x_amz_consistency_level: Optional[GetObjectInformationXAmzConsistencyLevelEnum] = field(default=None, metadata={'header': { 'field_name': 'x-amz-consistency-level', 'style': 'simple', 'explode': False }})
...
class GetObjectInformationRequestBody:
    object_reference: GetObjectInformationRequestBodyObjectReference = field(metadata={'dataclasses_json': { 'letter_case': utils.field_name('ObjectReference') }})
    

@dataclass
class GetObjectInformationRequest:
    headers: GetObjectInformationHeaders = field()
    request: GetObjectInformationRequestBody = field(metadata={'request': { 'media_type': 'application/json' }})
