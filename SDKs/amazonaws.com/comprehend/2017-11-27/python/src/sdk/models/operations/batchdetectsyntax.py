...
class BatchDetectSyntaxHeaders:
    ...
    x_amz_target: BatchDetectSyntaxXAmzTargetEnum = field(default=None, metadata={'header': { 'field_name': 'X-Amz-Target', 'style': 'simple', 'explode': False }})
    

@dataclass
class BatchDetectSyntaxRequest:
    headers: BatchDetectSyntaxHeaders = field(default=None)
    request: shared.BatchDetectSyntaxRequest = field(default=None, metadata={'request': { 'media_type': 'application/json' }})
    

@dataclass
class BatchDetectSyntaxResponse:
    batch_detect_syntax_response: Optional[shared.BatchDetectSyntaxResponse] = field(default=None)
