...
class UpdateUserAttributesHeaders:
    x_amz_target: UpdateUserAttributesXAmzTargetEnum = field(metadata={'header': { 'field_name': 'X-Amz-Target', 'style': 'simple', 'explode': False }})
...
class UpdateUserAttributesRequest:
    headers: UpdateUserAttributesHeaders = field()
    request: shared.UpdateUserAttributesRequest = field(metadata={'request': { 'media_type': 'application/json' }})
...
class UpdateUserAttributesResponse:
    ...
    update_user_attributes_response: Optional[shared.UpdateUserAttributesResponse] = field(default=None)
