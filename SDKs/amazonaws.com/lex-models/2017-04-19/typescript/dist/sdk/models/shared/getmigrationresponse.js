var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { SpeakeasyMetadata, SpeakeasyBase } from "../../../internal/utils";
import { MigrationAlert } from "./migrationalert";
import { MigrationStatusEnum } from "./migrationstatusenum";
import { MigrationStrategyEnum } from "./migrationstrategyenum";
import { LocaleEnum } from "./localeenum";
var GetMigrationResponse = /** @class */ (function (_super) {
    __extends(GetMigrationResponse, _super);
    function GetMigrationResponse() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        SpeakeasyMetadata({ data: "json, name=alerts", elemType: MigrationAlert }),
        __metadata("design:type", Array)
    ], GetMigrationResponse.prototype, "alerts", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "json, name=migrationId" }),
        __metadata("design:type", String)
    ], GetMigrationResponse.prototype, "migrationId", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "json, name=migrationStatus" }),
        __metadata("design:type", String)
    ], GetMigrationResponse.prototype, "migrationStatus", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "json, name=migrationStrategy" }),
        __metadata("design:type", String)
    ], GetMigrationResponse.prototype, "migrationStrategy", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "json, name=migrationTimestamp" }),
        __metadata("design:type", Date)
    ], GetMigrationResponse.prototype, "migrationTimestamp", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "json, name=v1BotLocale" }),
        __metadata("design:type", String)
    ], GetMigrationResponse.prototype, "v1BotLocale", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "json, name=v1BotName" }),
        __metadata("design:type", String)
    ], GetMigrationResponse.prototype, "v1BotName", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "json, name=v1BotVersion" }),
        __metadata("design:type", String)
    ], GetMigrationResponse.prototype, "v1BotVersion", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "json, name=v2BotId" }),
        __metadata("design:type", String)
    ], GetMigrationResponse.prototype, "v2BotId", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "json, name=v2BotRole" }),
        __metadata("design:type", String)
    ], GetMigrationResponse.prototype, "v2BotRole", void 0);
    return GetMigrationResponse;
}(SpeakeasyBase));
export { GetMigrationResponse };
