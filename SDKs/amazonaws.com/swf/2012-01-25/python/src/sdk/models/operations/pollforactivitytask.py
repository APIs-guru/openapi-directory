...
class PollForActivityTaskHeaders:
    ...
    x_amz_target: PollForActivityTaskXAmzTargetEnum = field(default=None, metadata={'header': { 'field_name': 'X-Amz-Target', 'style': 'simple', 'explode': False }})
    

@dataclass
class PollForActivityTaskRequest:
    headers: PollForActivityTaskHeaders = field(default=None)
    request: shared.PollForActivityTaskInput = field(default=None, metadata={'request': { 'media_type': 'application/json' }})
    

@dataclass
class PollForActivityTaskResponse:
    activity_task: Optional[shared.ActivityTask] = field(default=None)
