  ...
  getTimePeriod(
    ...
    return client
      .request({
        ...
        const res: operations.GetTimePeriodResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  getTimePeriods(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GetTimePeriodsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
