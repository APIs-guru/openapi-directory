  ...
  getWindowsHosting(
    ...
    return client
      .request({
        ...
        const res: operations.GetWindowsHostingResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  getWindowsHostings(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GetWindowsHostingsResponse = {statusCode: httpRes.status, contentType: contentType, headers: utils.GetHeadersFromResponse(httpRes.headers)};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
