import { SpeakeasyBase } from "../../../internal/utils/utils";
export declare class ProductsApiDeleteProduct2PathParams extends SpeakeasyBase {
    serial: string;
}
export declare class ProductsApiDeleteProduct2Headers extends SpeakeasyBase {
    xApiKey: string;
}
export declare class ProductsApiDeleteProduct2Request extends SpeakeasyBase {
    pathParams: ProductsApiDeleteProduct2PathParams;
    headers: ProductsApiDeleteProduct2Headers;
}
export declare class ProductsApiDeleteProduct2Response extends SpeakeasyBase {
    contentType: string;
    statusCode: number;
}
