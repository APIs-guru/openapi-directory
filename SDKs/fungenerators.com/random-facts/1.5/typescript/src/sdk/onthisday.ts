  ...
  getFactOnthisdayBorn(
    ...
    return client
      .request({
        ...
        const res: operations.GetFactOnthisdayBornResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  getFactOnthisdayDied(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GetFactOnthisdayDiedResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            break;
          case httpRes?.status == 401:
  ...
  getFactOnthisdayEvent(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GetFactOnthisdayEventResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            break;
          case httpRes?.status == 401:
