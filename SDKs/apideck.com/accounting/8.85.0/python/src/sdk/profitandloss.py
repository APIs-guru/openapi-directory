...
class ProfitAndLoss:
    ...
    def profit_and_loss_one(self, request: operations.ProfitAndLossOneRequest) -> operations.ProfitAndLossOneResponse:
        ...
        url = base_url.removesuffix("/") + "/accounting/profit-and-loss"
        ...
        res = operations.ProfitAndLossOneResponse(status_code=r.status_code, content_type=content_type)
        
        if r.status_code == 200:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[shared.GetProfitAndLossResponse])
                res.get_profit_and_loss_response = out
