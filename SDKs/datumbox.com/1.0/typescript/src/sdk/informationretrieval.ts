  ...
  keywordExtraction(
    ...
    return client
      .request({
        ...
        const res: operations.KeywordExtractionResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  textExtraction(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.TextExtractionResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            break;
        }
