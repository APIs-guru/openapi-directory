  ...
  requestAuditNodeUserData(
    ...
    return client
      .request({
        ...
        const res: operations.RequestAuditNodeUserDataResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  requestLogEventsAsJson(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.RequestLogEventsAsJsonResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
  ...
  requestLogOperations(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.RequestLogOperationsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
