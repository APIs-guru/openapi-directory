...
export class Git {
  ...
  **/
  gitGetAllRefs(
    req: operations.GitGetAllRefsRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.GitGetAllRefsResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.GitGetAllRefsRequest(req);
    }
    
    const baseURL: string = this._serverURL;
    const url: string = utils.GenerateURL(baseURL, "/repos/{owner}/{repo}/git/refs/{namespace}", req.pathParams);
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GitGetAllRefsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
