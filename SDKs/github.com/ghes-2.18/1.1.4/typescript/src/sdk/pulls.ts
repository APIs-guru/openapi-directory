...
export class Pulls {
  ...
  **/
  pullsCreateReviewCommentAlternative(
    req: operations.PullsCreateReviewCommentAlternativeRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.PullsCreateReviewCommentAlternativeResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.PullsCreateReviewCommentAlternativeResponse = {statusCode: httpRes.status, contentType: contentType, headers: utils.GetHeadersFromResponse(httpRes.headers)};
        switch (true) {
          case httpRes?.status == 201:
            if (utils.MatchContentType(contentType, `application/json`)) {
                res.legacyReviewComment = httpRes?.data;
            }
            break;
        }
