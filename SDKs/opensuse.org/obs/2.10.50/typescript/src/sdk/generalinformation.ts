  ...
  getArchitectures(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GetArchitecturesResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/xml; charset=utf-8`)) {
                const resBody: string = JSON.stringify(httpRes?.data, null, 0);
  ...
  getArchitecturesArchitectureName(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GetArchitecturesArchitectureNameResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/xml; charset=utf-8`)) {
                const resBody: string = JSON.stringify(httpRes?.data, null, 0);
