var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { SpeakeasyMetadata, SpeakeasyBase } from "../../../internal/utils";
import * as shared from "../shared";
export var ListCallsDirectionEnum;
(function (ListCallsDirectionEnum) {
    ListCallsDirectionEnum["Inbound"] = "INBOUND";
    ListCallsDirectionEnum["Outbound"] = "OUTBOUND";
})(ListCallsDirectionEnum || (ListCallsDirectionEnum = {}));
export var ListCallsOrderEnum;
(function (ListCallsOrderEnum) {
    ListCallsOrderEnum["Desc"] = "DESC";
    ListCallsOrderEnum["Asc"] = "ASC";
})(ListCallsOrderEnum || (ListCallsOrderEnum = {}));
export var ListCallsStatesEnum;
(function (ListCallsStatesEnum) {
    ListCallsStatesEnum["Initializing"] = "INITIALIZING";
    ListCallsStatesEnum["Ringing"] = "RINGING";
    ListCallsStatesEnum["Active"] = "ACTIVE";
    ListCallsStatesEnum["Held"] = "HELD";
    ListCallsStatesEnum["RemoteHeld"] = "REMOTE_HELD";
})(ListCallsStatesEnum || (ListCallsStatesEnum = {}));
var ListCallsQueryParams = /** @class */ (function (_super) {
    __extends(ListCallsQueryParams, _super);
    function ListCallsQueryParams() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=direction" }),
        __metadata("design:type", String)
    ], ListCallsQueryParams.prototype, "direction", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=fromDate" }),
        __metadata("design:type", Number)
    ], ListCallsQueryParams.prototype, "fromDate", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=offset" }),
        __metadata("design:type", Number)
    ], ListCallsQueryParams.prototype, "offset", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=order" }),
        __metadata("design:type", String)
    ], ListCallsQueryParams.prototype, "order", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=size" }),
        __metadata("design:type", Number)
    ], ListCallsQueryParams.prototype, "size", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=sort" }),
        __metadata("design:type", String)
    ], ListCallsQueryParams.prototype, "sort", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=states" }),
        __metadata("design:type", String)
    ], ListCallsQueryParams.prototype, "states", void 0);
    __decorate([
        SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=toDate" }),
        __metadata("design:type", Number)
    ], ListCallsQueryParams.prototype, "toDate", void 0);
    return ListCallsQueryParams;
}(SpeakeasyBase));
export { ListCallsQueryParams };
var ListCallsRequest = /** @class */ (function (_super) {
    __extends(ListCallsRequest, _super);
    function ListCallsRequest() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        SpeakeasyMetadata(),
        __metadata("design:type", ListCallsQueryParams)
    ], ListCallsRequest.prototype, "queryParams", void 0);
    return ListCallsRequest;
}(SpeakeasyBase));
export { ListCallsRequest };
var ListCallsResponse = /** @class */ (function (_super) {
    __extends(ListCallsResponse, _super);
    function ListCallsResponse() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        SpeakeasyMetadata({ elemType: shared.Call }),
        __metadata("design:type", Array)
    ], ListCallsResponse.prototype, "calls", void 0);
    __decorate([
        SpeakeasyMetadata(),
        __metadata("design:type", String)
    ], ListCallsResponse.prototype, "contentType", void 0);
    __decorate([
        SpeakeasyMetadata(),
        __metadata("design:type", shared.ErrorResponse)
    ], ListCallsResponse.prototype, "errorResponse", void 0);
    __decorate([
        SpeakeasyMetadata(),
        __metadata("design:type", Number)
    ], ListCallsResponse.prototype, "statusCode", void 0);
    return ListCallsResponse;
}(SpeakeasyBase));
export { ListCallsResponse };
