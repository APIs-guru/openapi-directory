from dataclasses import dataclass, field
from datetime import date, datetime
from marshmallow import fields
import dateutil.parser
from typing import Optional
from sdk.models import shared


@dataclass
class PriceOffersPathParams:
    destination: str = field(metadata={'path_param': { 'field_name': 'destination', 'style': 'simple', 'explode': False }})
    origin: str = field(metadata={'path_param': { 'field_name': 'origin', 'style': 'simple', 'explode': False }})
    

@dataclass
class PriceOffersQueryParams:
    departure_date: str = field(metadata={'query_param': { 'field_name': 'departureDate', 'style': 'form', 'explode': True }})
    return_date: str = field(metadata={'query_param': { 'field_name': 'returnDate', 'style': 'form', 'explode': True }})
    service: Optional[str] = field(default=None, metadata={'query_param': { 'field_name': 'service', 'style': 'form', 'explode': True }})
    

@dataclass
class PriceOffersSecurity:
    auth: shared.SchemeAuth = field(metadata={'security': { 'scheme': True, 'type': 'oauth2' }})
    

@dataclass
class PriceOffersRequest:
    path_params: PriceOffersPathParams = field()
    query_params: PriceOffersQueryParams = field()
    security: PriceOffersSecurity = field()
    

@dataclass
class PriceOffersResponse:
    content_type: str = field()
    status_code: int = field()
    price_offers_200_application_json_string: Optional[str] = field(default=None)
    
