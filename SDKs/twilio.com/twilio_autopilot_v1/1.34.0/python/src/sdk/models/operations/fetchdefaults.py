from dataclasses import dataclass, field
from typing import Optional
from sdk.models import shared
FETCH_DEFAULTS_SERVERS = [
	"https://autopilot.twilio.com",
]


@dataclass
class FetchDefaultsPathParams:
    assistant_sid: str = field(default=None, metadata={'path_param': { 'field_name': 'AssistantSid', 'style': 'simple', 'explode': False }})
    

@dataclass
class FetchDefaultsSecurity:
    account_sid_auth_token: shared.SchemeAccountSidAuthToken = field(default=None, metadata={'security': { 'scheme': True, 'type': 'http', 'sub_type': 'basic' }})
    

@dataclass
class FetchDefaultsRequest:
    server_url: Optional[str] = field(default=None)
    path_params: FetchDefaultsPathParams = field(default=None)
    security: FetchDefaultsSecurity = field(default=None)
    

@dataclass
class FetchDefaultsResponses:
    autopilot_v1_assistant_defaults: Optional[shared.AutopilotV1AssistantDefaults] = field(default=None)
    

@dataclass
class FetchDefaultsResponse:
    content_type: str = field(default=None)
    responses: dict[int, dict[str, FetchDefaultsResponses]] = field(default=None)
    status_code: int = field(default=None)
    
