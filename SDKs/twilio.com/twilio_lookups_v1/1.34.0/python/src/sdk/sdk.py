...
class SDK:
    ...
    def fetch_phone_number(self, request: operations.FetchPhoneNumberRequest) -> operations.FetchPhoneNumberResponse:
        ...
        url = utils.generate_url(base_url, "/v1/PhoneNumbers/{PhoneNumber}", request.path_params)
        ...
        if r.status_code == 200:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[shared.LookupsV1PhoneNumber])
                res.responses[r.status_code][content_type] = operations.FetchPhoneNumberResponses(lookups_v1_phone_number=out)
