...
func (s *Addresses) AddressSearch(ctx context.Context, request operations.AddressSearchRequest) (*operations.AddressSearchResponse, error) {
	...
	res := &operations.AddressSearchResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(contentType, `application/json`):
			var out *shared.AddressSearchResponseSchema
...
//
// Royal Mail (Postcode Address File), Ordnance Survey (Code-Point Open).
func (s *Addresses) UpdrnToAddress(ctx context.Context, request operations.UpdrnToAddressRequest) (*operations.UpdrnToAddressResponse, error) {
	baseURL := s._serverURL
	url := utils.GenerateURL(ctx, baseURL, "/addresses/{udprn}", request.PathParams)
	...
	res := &operations.UpdrnToAddressResponse{
		StatusCode:  int64(httpRes.StatusCode),
		ContentType: contentType,
	}
	switch {
	case httpRes.StatusCode == 200:
		switch {
		case utils.MatchContentType(contentType, `application/json`):
			var out *shared.UdprnToAddressResponseSchema
			if err := utils.UnmarshalJsonFromResponseBody(httpRes.Body, &out); err != nil {
				return nil, err
			}
