...
export class Connect {
  ...
  **/
  sqlConnectGenerateEphemeral(
    ...
    const url: string = utils.GenerateURL(baseURL, "/v1/projects/{project}/instances/{instance}:generateEphemeralCert", req.pathParams);
    ...
    return client
      .request({
        ...
        const res: operations.SqlConnectGenerateEphemeralResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  sqlConnectGet(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.GenerateURL(baseURL, "/v1/projects/{project}/instances/{instance}/connectSettings", req.pathParams);
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.SqlConnectGetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
