import { SpeakeasyBase } from "../../../internal/utils";
import * as shared from "../shared";
export declare class ContactcenterinsightsProjectsLocationsViewsPatchPathParams extends SpeakeasyBase {
    name: string;
}
export declare class ContactcenterinsightsProjectsLocationsViewsPatchQueryParams extends SpeakeasyBase {
    dollarXgafv?: shared.XgafvEnum;
    accessToken?: string;
    alt?: shared.AltEnum;
    callback?: string;
    fields?: string;
    key?: string;
    oauthToken?: string;
    prettyPrint?: boolean;
    quotaUser?: string;
    updateMask?: string;
    uploadType?: string;
    uploadProtocol?: string;
}
export declare class ContactcenterinsightsProjectsLocationsViewsPatchSecurity extends SpeakeasyBase {
    oauth2: shared.SchemeOauth2;
    oauth2c: shared.SchemeOauth2c;
}
export declare class ContactcenterinsightsProjectsLocationsViewsPatchRequest extends SpeakeasyBase {
    pathParams: ContactcenterinsightsProjectsLocationsViewsPatchPathParams;
    queryParams: ContactcenterinsightsProjectsLocationsViewsPatchQueryParams;
    request?: shared.GoogleCloudContactcenterinsightsV1ViewInput;
    security: ContactcenterinsightsProjectsLocationsViewsPatchSecurity;
}
export declare class ContactcenterinsightsProjectsLocationsViewsPatchResponse extends SpeakeasyBase {
    contentType: string;
    googleCloudContactcenterinsightsV1View?: shared.GoogleCloudContactcenterinsightsV1View;
    statusCode: number;
}
