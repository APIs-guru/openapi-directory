...
export class Speech {
  ...
  **/
  speechSpeechLongrunningrecognize(
    ...
    const url: string = baseURL.replace(/\/$/, "") + "/v1/speech:longrunningrecognize";
    ...
    return client
      .request({
        ...
        const res: operations.SpeechSpeechLongrunningrecognizeResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  speechSpeechRecognize(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.SpeechSpeechRecognizeResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
                res.recognizeResponse = httpRes?.data;
            }
            break;
        }
