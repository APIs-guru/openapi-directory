import { SpeakeasyBase } from "../../../internal/utils";
import * as shared from "../shared";
export declare class CloudassetQueryAssetsPathParams extends SpeakeasyBase {
    parent: string;
}
export declare class CloudassetQueryAssetsQueryParams extends SpeakeasyBase {
    dollarXgafv?: shared.XgafvEnum;
    accessToken?: string;
    alt?: shared.AltEnum;
    callback?: string;
    fields?: string;
    key?: string;
    oauthToken?: string;
    prettyPrint?: boolean;
    quotaUser?: string;
    uploadType?: string;
    uploadProtocol?: string;
}
export declare class CloudassetQueryAssetsSecurity extends SpeakeasyBase {
    oauth2: shared.SchemeOauth2;
    oauth2c: shared.SchemeOauth2c;
}
export declare class CloudassetQueryAssetsRequest extends SpeakeasyBase {
    pathParams: CloudassetQueryAssetsPathParams;
    queryParams: CloudassetQueryAssetsQueryParams;
    request?: shared.QueryAssetsRequest;
    security: CloudassetQueryAssetsSecurity;
}
export declare class CloudassetQueryAssetsResponse extends SpeakeasyBase {
    contentType: string;
    queryAssetsResponse?: shared.QueryAssetsResponse;
    statusCode: number;
}
