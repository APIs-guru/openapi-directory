...
export class V1p4beta1 {
  ...
  **/
  cloudassetAnalyzeIamPolicy(
    ...
    const url: string = utils.GenerateURL(baseURL, "/v1p4beta1/{parent}:analyzeIamPolicy", req.pathParams);
  ...
  **/
  cloudassetExportIamPolicyAnalysis(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.GenerateURL(baseURL, "/v1p4beta1/{parent}:exportIamPolicyAnalysis", req.pathParams);
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.CloudassetExportIamPolicyAnalysisResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
