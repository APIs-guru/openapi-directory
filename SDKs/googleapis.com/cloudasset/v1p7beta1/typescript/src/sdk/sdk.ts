...
export class SDK {
  ...
  CloudassetExportAssets(
    ...
    const url: string = utils.GenerateURL(baseURL, "/v1p7beta1/{parent}:exportAssets", req.pathParams);
    ...
    return client
      ...
      .then((httpRes: AxiosResponse) => {
        ...
        let res: operations.CloudassetExportAssetsResponse = {statusCode: httpRes.status, contentType: contentType};
  ...
  CloudassetOperationsGet(
    ...
    let baseURL: string = this.serverURL;
    const url: string = utils.GenerateURL(baseURL, "/v1p7beta1/{name}", req.pathParams);
    ...
    return client
      ...
      .then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        let res: operations.CloudassetOperationsGetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (httpRes?.status) {
          case 200:
            if (MatchContentType(contentType, `application/json`)) {
                res.operation = httpRes?.data;
