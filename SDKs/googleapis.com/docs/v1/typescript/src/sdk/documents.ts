...
export class Documents {
  ...
  **/
  docsDocumentsBatchUpdate(
    ...
    const url: string = utils.GenerateURL(baseURL, "/v1/documents/{documentId}:batchUpdate", req.pathParams);
    ...
    return client
      .request({
        ...
        const res: operations.DocsDocumentsBatchUpdateResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
                res.batchUpdateDocumentResponse = httpRes?.data;
            }
            break;
        }
  ...
  docsDocumentsCreate(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.DocsDocumentsCreateResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
  ...
  **/
  docsDocumentsGet(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.GenerateURL(baseURL, "/v1/documents/{documentId}", req.pathParams);
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.DocsDocumentsGetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
                res.document = httpRes?.data;
