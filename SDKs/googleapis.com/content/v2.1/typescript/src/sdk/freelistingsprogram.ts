  ...
  contentFreelistingsprogramGet(
    ...
    return client
      .request({
        ...
        const res: operations.ContentFreelistingsprogramGetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  contentFreelistingsprogramRequestreview(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.ContentFreelistingsprogramRequestreviewResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            break;
        }
