  ...
  contentShoppingadsprogramGet(
    ...
    return client
      .request({
        ...
        const res: operations.ContentShoppingadsprogramGetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  contentShoppingadsprogramRequestreview(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.ContentShoppingadsprogramRequestreviewResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            break;
        }
