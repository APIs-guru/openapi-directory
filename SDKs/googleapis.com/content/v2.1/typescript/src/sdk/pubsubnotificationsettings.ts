  ...
  contentPubsubnotificationsettingsGet(
    ...
    return client
      .request({
        ...
        const res: operations.ContentPubsubnotificationsettingsGetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  contentPubsubnotificationsettingsUpdate(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.ContentPubsubnotificationsettingsUpdateResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
                res.pubsubNotificationSettings = httpRes?.data;
