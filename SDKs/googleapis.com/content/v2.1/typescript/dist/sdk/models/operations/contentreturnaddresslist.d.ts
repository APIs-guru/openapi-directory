import { SpeakeasyBase } from "../../../internal/utils";
import * as shared from "../shared";
export declare class ContentReturnaddressListPathParams extends SpeakeasyBase {
    merchantId: string;
}
export declare class ContentReturnaddressListQueryParams extends SpeakeasyBase {
    dollarXgafv?: shared.XgafvEnum;
    accessToken?: string;
    alt?: shared.AltEnum;
    callback?: string;
    country?: string;
    fields?: string;
    key?: string;
    maxResults?: number;
    oauthToken?: string;
    pageToken?: string;
    prettyPrint?: boolean;
    quotaUser?: string;
    uploadType?: string;
    uploadProtocol?: string;
}
export declare class ContentReturnaddressListSecurity extends SpeakeasyBase {
    oauth2: shared.SchemeOauth2;
    oauth2c: shared.SchemeOauth2c;
}
export declare class ContentReturnaddressListRequest extends SpeakeasyBase {
    pathParams: ContentReturnaddressListPathParams;
    queryParams: ContentReturnaddressListQueryParams;
    security: ContentReturnaddressListSecurity;
}
export declare class ContentReturnaddressListResponse extends SpeakeasyBase {
    contentType: string;
    returnaddressListResponse?: shared.ReturnaddressListResponse;
    statusCode: number;
}
