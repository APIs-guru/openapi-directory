  ...
  youtubeWatermarksSet(
    ...
    return client
      .request({
        ...
        const res: operations.YoutubeWatermarksSetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  youtubeWatermarksUnset(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.YoutubeWatermarksUnsetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            break;
        }
