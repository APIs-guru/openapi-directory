  ...
  youtubeLiveChatBansDelete(
    ...
    return client
      .request({
        ...
        const res: operations.YoutubeLiveChatBansDeleteResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  youtubeLiveChatBansInsert(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.YoutubeLiveChatBansInsertResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
