...
class Projects:
    ...
    def managedidentities_projects_locations_global_domains_domain_join_machine(self, request: operations.ManagedidentitiesProjectsLocationsGlobalDomainsDomainJoinMachineRequest) -> operations.ManagedidentitiesProjectsLocationsGlobalDomainsDomainJoinMachineResponse:
        r"""DomainJoinMachine API joins a Compute Engine VM to the domain
        """
        
        base_url = self._server_url
        
        url = utils.generate_url(base_url, "/v1beta1/{domain}:domainJoinMachine", request.path_params)
        ...
        res = operations.ManagedidentitiesProjectsLocationsGlobalDomainsDomainJoinMachineResponse(status_code=r.status_code, content_type=content_type)
        
        if r.status_code == 200:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[shared.DomainJoinMachineResponse])
