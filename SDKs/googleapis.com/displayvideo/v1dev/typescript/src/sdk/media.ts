  ...
  displayvideoMediaDownload(
    ...
    return client
      .request({
        ...
        const res: operations.DisplayvideoMediaDownloadResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  displayvideoMediaUpload(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.DisplayvideoMediaUploadResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
                res.googleBytestreamMedia = httpRes?.data;
