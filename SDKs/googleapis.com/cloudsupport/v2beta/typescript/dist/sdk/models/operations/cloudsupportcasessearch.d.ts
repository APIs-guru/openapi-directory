import { SpeakeasyBase } from "../../../internal/utils";
import * as shared from "../shared";
export declare class CloudsupportCasesSearchQueryParams extends SpeakeasyBase {
    dollarXgafv?: shared.XgafvEnum;
    accessToken?: string;
    alt?: shared.AltEnum;
    callback?: string;
    fields?: string;
    key?: string;
    oauthToken?: string;
    pageSize?: number;
    pageToken?: string;
    prettyPrint?: boolean;
    query?: string;
    quotaUser?: string;
    uploadType?: string;
    uploadProtocol?: string;
}
export declare class CloudsupportCasesSearchSecurity extends SpeakeasyBase {
    oauth2: shared.SchemeOauth2;
    oauth2c: shared.SchemeOauth2c;
}
export declare class CloudsupportCasesSearchRequest extends SpeakeasyBase {
    queryParams: CloudsupportCasesSearchQueryParams;
    security: CloudsupportCasesSearchSecurity;
}
export declare class CloudsupportCasesSearchResponse extends SpeakeasyBase {
    contentType: string;
    searchCasesResponse?: shared.SearchCasesResponse;
    statusCode: number;
}
