  ...
  dfareportingOrderDocumentsGet(
    ...
    return client
      .request({
        ...
        const res: operations.DfareportingOrderDocumentsGetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  dfareportingOrderDocumentsList(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.DfareportingOrderDocumentsListResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
