  ...
  cloudidentityOrgUnitsMembershipsList(
    ...
    return client
      .request({
        ...
        const res: operations.CloudidentityOrgUnitsMembershipsListResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  cloudidentityOrgUnitsMembershipsMove(
    ...
    return client
      .request({
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.CloudidentityOrgUnitsMembershipsMoveResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.MatchContentType(contentType, `application/json`)) {
