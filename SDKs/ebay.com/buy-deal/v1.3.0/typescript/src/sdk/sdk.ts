...
export class SDK {
  ...
  GetDealItems(
  ...
  ): Promise<operations.GetDealItemsResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.GetDealItemsRequest(req);
    }
    
    let baseURL: string = this.serverURL;
    const url: string = baseURL.replace(/\/$/, "") + "/deal_item";
    ...
    return client
      ...
      .then((httpRes: AxiosResponse) => {
        ...
        let res: operations.GetDealItemsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (httpRes?.status) {
          case 200:
            if (MatchContentType(contentType, `application/json`)) {
                res.dealItemSearchResponse = httpRes?.data;
            }
            break;
          case 400:
            break;
  ...
  GetEventItems(
    ...
    return client
      ...
      .then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        let res: operations.GetEventItemsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (httpRes?.status) {
          case 200:
            if (MatchContentType(contentType, `application/json`)) {
